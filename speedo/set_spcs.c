 ...
 */
static boolean sp_setup_consts(SPD_PROTO_DECL2
 ...
 */
static fix31 sp_setup_offset(SPD_PROTO_DECL2 fix31 input_offset)	/* Multiplier in input format */
{
 ...
 * matrix from external to internal form.
 */
static fix15 sp_setup_mult(SPD_PROTO_DECL2 fix31 input_mult)	/* Multiplier in input format */
{
	fix15 imshift;						/* Right shift to internal format */
 ...
 */
boolean sp_set_specs(SPD_PROTO_DECL2 const specs_t *specsarg, const buff_t *font)	/* Bundle of conversion specifications */
{
...
void sp_type_tcb(SPD_PROTO_DECL2 tcb_t *ptcb)	/* Pointer to transformation control block */
{
	fix15 x_trans_type;
	fix15 y_trans_type;
	fix15 xx_mult;
 ...
 * Returns the decrypted value read as a signed integer.
 */
fix15 sp_read_word_u(SPD_PROTO_DECL2 ufix8 *pointer)	/* Pointer to first byte of unencrypted 2-byte integer */
{
	fix15 tmpfix15;
