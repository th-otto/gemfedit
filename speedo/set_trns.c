...
static ufix8 FONTFAR *sp_setup_pix_table(ufix8 FONTFAR * pointer, boolean short_form, fix15 no_X_ctrl_zones,
										 fix15 no_Y_ctrl_zones);
static ufix8 FONTFAR *sp_setup_int_table(ufix8 FONTFAR * pointer, fix15 no_X_int_zones,
 ...
 */
FUNCTION void scale_tcb(tcb_t GLOBALFAR * ptcb,	/* Transformation control block */
...
FUNCTION ufix8 FONTFAR *sp_skip_interpolation_table(ufix8 FONTFAR * pointer,	/* Pointer to next byte in char data */
...
FUNCTION ufix8 FONTFAR *sp_skip_control_zone(ufix8 FONTFAR * pointer,	/* Pointer to next byte in char data */
										  ufix8 format)	/* Character format byte */
{
	fix15 i, n;
	ufix16 tmpufix16;
 ...
 */
FUNCTION ufix8 FONTFAR *plaid_tcb(ufix8 FONTFAR * pointer,	/* Pointer to next byte in char data */
								  ufix8 format)	/* Character format byte */
{
	fix15 i, n;
 ...
 * data.
 */
FUNCTION ufix8 FONTFAR *read_oru_table(ufix8 FONTFAR * pointer)	/* Pointer to first byte in controlled coord table */
{
	fix15 i, j, k, n;
	boolean zero_not_in;
	boolean zero_added;
									 ...
									 ufix16 constr_nr, fix31 x_scale, fix31 x_offset, fix31 ppo, fix15 setwidth_pix)
{
	...
	zone_pix = (fix15) (((SQUEEZE_MULT(x_scale, sp_globals.c_pix[constr_nr])) + sp_globals.pixrnd) & sp_globals.pixfix);
 ...
 * data.
 */
FUNCTION static ufix8 FONTFAR *sp_setup_pix_table(ufix8 FONTFAR * pointer,	/* Pointer to first byte in control zone table */
...
#if INCL_SQUEEZING
	squeezed_x = sp_calculate_x_scale(&x_scale, &x_offset, no_X_ctrl_zones);
	squeezed_y = sp_calculate_y_scale(&y_top_scale, &y_bottom_scale, (n + 1), no_Y_ctrl_zones);
#if INCL_ISW
...
/* check for overflow */
	...
	em_top_pix = ((fix15) (((fix31) EM_TOP * yppo0) >> sp_globals.mpshift) + sp_globals.pixrnd) & sp_globals.pixfix;
#endif
...
/* check for overflow */
	...
	if (!squeezed_x && ((imported_width = sp_globals.import_setwidth_act) == TRUE))
	{
		...
		for (i = 0; i < (no_X_ctrl_zones + 1); i++)
			if (sp_plaid.orus[i] >= 0)
			{
				sp_plaid.pix[i] = (SQUEEZE_MULT(sp_plaid.pix[i], isw_scale) + sp_globals.pixrnd) & sp_globals.pixfix;
				break;
			}
	...
	for (i = 0;; i++)					/* For X and Y control zones... */
	{
		for (j = 0; j < n; j++)			/* For each zone in X or Y... */
		{
			...
			constr_nr = constr_org + NEXT_BYTES(pointer, tmpufix16);	/* Read constraint number */
#if INCL_SQUEEZING
			if (i == 0 && squeezed_x)
			{
				sp_calculate_x_pix(start_edge, end_edge, constr_nr, x_scale, x_offset, ppo, setwidth_pix);
				  ...
				  L1:
					...
					{					/*  get the non-xformed - xformed zone, in right direction */
						whole_zone = (((fix31) sp_plaid.orus[end_edge] -
									   (fix31) sp_plaid.orus[start_edge]) * ppo) / (1 << sp_globals.mpshift);
 ...
 * data.
 */
FUNCTION static ufix8 FONTFAR *sp_setup_int_table(ufix8 FONTFAR * pointer,	/* Pointer to first byte in interpolation zone table */
