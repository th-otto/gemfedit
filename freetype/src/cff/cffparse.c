...
FT_LOCAL_DEF(FT_Error) cff_parser_init(CFF_Parser parser,
	FT_UInt code,
	void *object, FT_Library library, FT_UInt stackSize, FT_UShort num_designs, FT_UShort num_axes)
{
...
static FT_Long cff_parse_integer(FT_Byte * start, FT_Byte * limit)
{
	...
	} else if (v == 29)
	{
		...
		val = (FT_Int32) (((FT_UInt32) p[0] << 24) |
						  ((FT_UInt32) p[1] << 16) | ((FT_UInt32) p[2] << 8) | (FT_UInt32) p[3]);
	} else if (v < 247)
	{
		val = v - 139;
	} else if (v < 251)
...
static FT_Fixed cff_parse_real(FT_Byte * start, FT_Byte * limit, FT_Long power_ten, FT_Long * scaling)
{
	...
	FT_Int sign = 0;
	FT_Int exponent_sign = 0;
	FT_Int have_overflow = 0;
...
FT_LOCAL_DEF(FT_Long) cff_parse_num(CFF_Parser parser, FT_Byte ** d)
{
	if (**d == 30)
	{
...
#if 0
		return (FT_Short) (((((FT_UInt32) * (d[0] + 1) << 24) |
							 ((FT_UInt32) * (d[0] + 2) << 16) |
							 ((FT_UInt32) * (d[0] + 3) << 8) | (FT_UInt32) * (d[0] + 4)) + 0x8000U) >> 16);
#else
		return (FT_Short) (((((FT_UInt32) * (d[0] + 1) << 16) |
							 ((FT_UInt32) * (d[0] + 2) << 8) | (FT_UInt32) * (d[0] + 3)) + 0x80U) >> 8);
#endif
	}
	...
	if (parser->top >= parser->stack + 6)
	{
		...
		*upm = (FT_ULong) power_tens[-max_scaling];
		...
		FT_TRACE4((" [%f %f %f %f %f %f]\n",
				   ...
				   (double) offset->x / *upm / 65536, (double) offset->y / *upm / 65536));
	}
...
static FT_Error cff_parse_multiple_master(CFF_Parser parser)
{
	...
	if (ft_trace_levels[FT_COMPONENT] < 4)
		FT_TRACE1(("Multiple Master CFFs not supported yet," " handling first master design only\n"));
	else
		FT_TRACE1((" (not supported yet," " handling first master design only)\n"));
#endif
	...
	if (parser->top >= parser->stack + 5)
	{
		...
		{
			FT_ERROR(("cff_parse_multiple_master:" " Invalid number of designs\n"));
			error = FT_THROW(Invalid_File_Format);
		} else
		{
...
#ifdef FT_DEBUG_LEVEL_TRACE
#define CFF_FIELD_TRACE_ID(id) , id
#else
#define CFF_FIELD_TRACE_ID(id)
#endif
...
#include "cfftoken.h"
	...
	{ 0, 0, 0, 0, 0, 0, 0 CFF_FIELD_TRACE_ID(0) }
};


FT_LOCAL_DEF(FT_Error) cff_parser_run(CFF_Parser parser, FT_Byte * start, FT_Byte * limit)
{
	FT_Byte *p = start;
	FT_Error error = FT_Err_Ok;
	...
	while (p < limit)
	{
		...
		else if (v == 31)
		{
			...
			error = cff_decoder_parse_charstrings(&decoder, charstring_base, charstring_len, 1);
