...
static void cf2_computeDarkening(CF2_Fixed emRatio,
					 CF2_Fixed ppem,
					 CF2_Fixed stemWidth,
					 CF2_Fixed * darkenAmount, CF2_Fixed boldenAmount, FT_Bool stemDarkened, FT_Int * darkenParams)
{
	/*
	...
	*darkenAmount = 0;
	...
	if (stemDarkened)
	{
		...
		else if (scaledStem < cf2_intToFixed(x2))
		{
			FT_Int xdelta = x2 - x1;
			FT_Int ydelta = y2 - y1;
			FT_Pos x = stemWidthPer1000 - FT_DivFix(cf2_intToFixed(x1), ppem);
		  ...
		  Try_x3:
			{
				FT_Int xdelta = x3 - x2;
				FT_Int ydelta = y3 - y2;
				FT_Pos x = stemWidthPer1000 - FT_DivFix(cf2_intToFixed(x2), ppem);
		  ...
		  Try_x4:
			{
				FT_Int xdelta = x4 - x3;
				FT_Int ydelta = y4 - y3;
				FT_Pos x = stemWidthPer1000 - FT_DivFix(cf2_intToFixed(x3), ppem);
...
static void cf2_font_setup(CF2_Font font, const CF2_Matrix * transform)
{
	...
	if (hasVariations)
	{
		...
		if (cff_blend_check_vector(&subFont->blend, subFont->private_dict.vsindex, lenNormalizedV, normalizedV))
		{
...
cf2_getGlyphOutline(CF2_Font font, CF2_Buffer charstring, const CF2_Matrix * transform, CF2_F16Dot16 * glyphWidth)
{
	...
	while (1)
	{
		...
		cf2_interpT2CharString(font, charstring,
							   &font->outline.root, &translation, FALSE, 0, 0, &advWidth);
