...
FT_LOCAL_DEF(FT_Error) cff_size_select(FT_Size size, FT_ULong strike_index)
{
...
FT_LOCAL_DEF(FT_Error) cff_size_request(FT_Size size, FT_Size_Request req)
{
	CFF_Size cffsize = (CFF_Size) size;
	PSH_Globals_Funcs funcs;
...
static void remove_style(FT_String * family_name, const FT_String * style_name)
{
	...
	if (family_name_length > style_name_length)
	{
		...
		if (idx > style_name_length)
		{
			...
			idx = (FT_Int) (family_name_length - style_name_length - 1);

			/* also remove special characters     */
			/* between real family name and style */
			while (idx > 0 &&
				   (family_name[idx] == '-' ||
					family_name[idx] == ' ' || family_name[idx] == '_' || family_name[idx] == '+'))
				idx--;
...
FT_LOCAL_DEF(FT_Error) cff_face_init(FT_Stream stream, FT_Face cffface,	/* CFF_Face */
									 FT_Int face_index, FT_Int num_params, FT_Parameter * params)
{
	...
	/* now load and parse the CFF table in the file */
	{
...
#ifdef TT_CONFIG_OPTION_GX_VAR_SUPPORT
		{
			...
			if (FT_HAS_MULTIPLE_MASTERS(cffface) && mm && instance_index > 0)
			{
				...
				if (mm_var->namedstyle)
				{
					...
					error = mm->set_var_design(cffface, mm_var->num_axis, named_style->coords);
					if (error)
						goto Exit;
				}
			}
			...
			if (sub->has_font_matrix)
			{
				...
				if (top->has_font_matrix)
				{
					...
					sub->units_per_em = (FT_ULong)
						FT_MulDiv((FT_Long) sub->units_per_em, (FT_Long) top->units_per_em, scaling);
				}
			} else
			{
		...
		if (pure_cff)
		{
			...
			{
				const char *full = cff_index_get_sid_string(cff, dict->full_name);
				const char *fullp = full;
				const char *family = cffface->family_name;
			...
			{
				const char *cid_font_name = cff_index_get_sid_string(cff, dict->cid_font_name);
		...
		/*******************************************************************/
			...
			{
				const char *weight = cff_index_get_sid_string(cff, dict->weight);
	  ...
	  /*******************************************************************/
		...
		/* will use it, whatever be in the CFF part of the file.          */
		{
			...
			error = FT_CMap_New(&CFF_CMAP_UNICODE_CLASS_REC_GET, NULL, &cmaprec, NULL);
...
FT_LOCAL_DEF(FT_Error) cff_driver_init(FT_Module module)	/* CFF_Driver */
{
	...
	seed = (FT_UInt32) ((FT_Offset) (char *) &seed ^
						(FT_Offset) (char *) &module ^ (FT_Offset) (char *) module->memory);
