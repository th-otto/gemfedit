...
/*                                                                       */
FT_LOCAL_DEF(void) cff_decoder_init(CFF_Decoder *decoder,
	TT_Face face, CFF_Size size, CFF_GlyphSlot slot, FT_Bool hinting, FT_Render_Mode hint_mode)
{
...
FT_LOCAL_DEF(FT_Error) cff_decoder_prepare(CFF_Decoder * decoder, CFF_Size size, FT_UInt32 glyph_index)
{
...
FT_LOCAL_DEF(FT_Error) cff_check_points(CFF_Builder * builder, FT_Int count)
{
...
FT_LOCAL_DEF(void) cff_builder_add_point(CFF_Builder * builder, FT_Pos x, FT_Pos y, FT_Byte flag)
{
...
FT_LOCAL_DEF(FT_Error) cff_builder_add_point1(CFF_Builder * builder, FT_Pos x, FT_Pos y)
{
	FT_Error error;
...
FT_LOCAL_DEF(FT_Error) cff_builder_start_point(CFF_Builder * builder, FT_Pos x, FT_Pos y)
{
	FT_Error error = FT_Err_Ok;
...
FT_LOCAL_DEF(FT_Int) cff_lookup_glyph_by_stdcharcode(CFF_Font cff, FT_Int charcode)
{
	FT_UInt n;
	FT_UShort glyph_sid;
...
FT_LOCAL_DEF(FT_Error) cff_get_glyph_data(TT_Face face, FT_UInt32 glyph_index, FT_Byte ** pointer, FT_ULong * length)
{
...
FT_LOCAL_DEF(void) cff_free_glyph_data(TT_Face face, FT_Byte ** pointer, FT_ULong length)
{
...
static FT_Error cff_operator_seac(CFF_Decoder * decoder, FT_Pos asb, FT_Pos adx, FT_Pos ady, FT_Int bchar, FT_Int achar)
{
	...
	error = cff_get_glyph_data(face, (FT_UInt) bchar_index, &charstring, &charstring_len);
	if (!error)
	{
	...
	error = cff_get_glyph_data(face, (FT_UInt) achar_index, &charstring, &charstring_len);
	if (!error)
	{
		/* the seac operator must not be nested */
		decoder->seac = TRUE;
...
/*                                                                       */
FT_LOCAL_DEF(FT_Error) cff_decoder_parse_charstrings(CFF_Decoder *decoder,
	FT_Byte *charstring_base, FT_ULong charstring_len, FT_Bool in_dict)
{
	FT_Error error;
		...
		if (v >= 32 || v == 28)
		{
			...
			} else
			{
				...
				val = (FT_Int32) (((FT_UInt32) ip[0] << 24) |
								  ((FT_UInt32) ip[1] << 16) | ((FT_UInt32) ip[2] << 8) | (FT_UInt32) ip[3]);
				ip += 4;
			...
			*decoder->top++ = val;
			...
			if (!(val & 0xFFFFL))
				FT_TRACE4((" %d", (FT_Short) ((FT_UInt32) val >> 16)));
			else
		...
		} else
		{
			...
			case cff_op_vstemhm:
				...
				if (hinter)
					hinter->stems(hinter->hints,
								  (op == cff_op_hstem || op == cff_op_hstemhm), num_args / 2, args - (num_args & ~1));
				...
				if (hinter)
				{
					if (op == cff_op_hintmask)
						hinter->hintmask(hinter->hints,
										 (FT_UInt) builder->current->n_points, (FT_UInt) decoder->num_hints, ip);
					else
			...
			case cff_op_seac:
				...
				error = cff_operator_seac(decoder,
										  ...
										  (FT_Int) (args[3] >> 16), (FT_Int) (args[4] >> 16));
				...
				} else
				{
					...
					if (hinter)
					{
						...
						error = hinter->apply(hinter->hints,
											  builder->current,
											  (PSH_Globals) builder->hints_globals, decoder->hint_mode);
						if (error)
							goto Fail;
					}
			...
			case cff_op_random:
				...
				args[0] = (FT_Fixed) ((decoder->current_subfont->random & 0xFFFF) + 1);
				args++;
			...
			case cff_op_load:
				...
				/* in version 16-March-2000                               */
				{
					...
					if (reg_idx >= 0 && reg_idx <= 2 &&
						idx >= 0 && idx < CFF_MAX_TRANS_ELEMENTS && count >= 0 && count <= num_axes)
					{
						FT_Int end, i;
  ...
  MM_Error:
	FT_TRACE4(("cff_decoder_parse_charstrings:" " invalid opcode found in top DICT charstring\n"));
...
FT_LOCAL_DEF(FT_Error) cff_compute_max_advance(TT_Face face, FT_Int *max_advance)
{
	FT_Error error = FT_Err_Ok;
	CFF_Decoder decoder;
...
FT_LOCAL_DEF(FT_Error) cff_slot_load(CFF_GlyphSlot glyph, CFF_Size size, FT_UInt32 glyph_index, FT_Int32 load_flags)
{
	FT_Error error;
	CFF_Decoder decoder;
	...
	if (size)
	{
		...
		if (size->strike_index != 0xFFFFFFFFUL && sfnt->load_eblc && (load_flags & FT_LOAD_NO_BITMAP) == 0)
		{
			...
			error = sfnt->load_sbit_image(face,
										  size->strike_index,
										  glyph_index, (FT_UInt) load_flags, stream, &glyph->root.bitmap, &metrics);
...
#endif
		{
			error = cf2_decoder_parse_charstrings(&decoder, charstring, charstring_len);
