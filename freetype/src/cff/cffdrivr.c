...
/*                                                                       */
FT_CALLBACK_DEF(FT_Error) cff_get_kerning(FT_Face ttface,	/* TT_Face */
...
/*    FreeType error code.  0 means success.                             */
/*                                                                       */
FT_CALLBACK_DEF(FT_Error) cff_glyph_load(FT_GlyphSlot cffslot,	/* CFF_GlyphSlot */
										 FT_Size cffsize,	/* CFF_Size      */
										 FT_UInt32 glyph_index, FT_Int32 load_flags)
{
	FT_Error error;
...
cff_get_advances(FT_Face face, FT_UInt32 start, FT_UInt32 count, FT_Int32 flags, FT_Fixed * advances)
{
	FT_UInt nn;
	...
	if (FT_IS_SFNT(face))
	{
		...
		if (flags & FT_LOAD_VERTICAL_LAYOUT)
		{
			...
			if (!ttface->is_default_instance && !(ttface->variation_support & TT_FACE_FLAG_VAR_VADVANCE))
		...
		} else
		{
			...
			if (!ttface->is_default_instance && !(ttface->variation_support & TT_FACE_FLAG_VAR_HADVANCE))
				return FT_THROW(Unimplemented_Feature);
#endif
...
static FT_Error cff_get_glyph_name(FT_Face face_, FT_UInt32 glyph_index, FT_Pointer buffer, FT_UInt buffer_max)
{
	CFF_Face face = (CFF_Face) face_;
	CFF_Font font = (CFF_Font) face->extra.data;
	const FT_String *gname;
	FT_UShort sid;
	FT_Error error;
	...
	if (font->version_major == 2)
	{
		...
		FT_Service_GlyphDict service = (FT_Service_GlyphDict) ft_module_get_service(sfnt_module,
...
static FT_Error cff_ps_get_font_info(FT_Face face_, PS_FontInfoRec * afont_info)
{
	CFF_Face face = (CFF_Face)face_;
...
static const FT_Service_PsInfoRec cff_service_ps_info = {
	...
	NULL,	/* ps_get_font_extra   */
	cff_ps_has_glyph_names,	/* ps_has_glyph_names  */
	/* unsupported with CFF fonts */
	NULL,	/* ps_get_font_private */
	/* not implemented            */
 ...
 *
 */
static const char *cff_get_ps_name(FT_Face face_)
{
	CFF_Face face = (CFF_Face)face_;
	CFF_Font cff = (CFF_Font) face->extra.data;
	SFNT_Service sfnt = (SFNT_Service) face->sfnt;
	...
	if (FT_IS_SFNT(FT_FACE(face)) && sfnt)
	{
		FT_Library library = FT_FACE_LIBRARY(face);
		FT_Module sfnt_module = FT_Get_Module(library, "sfnt");
		FT_Service_PsFontName service = (FT_Service_PsFontName) ft_module_get_service(sfnt_module,
...
static FT_Error cff_get_cmap_info(FT_CharMap charmap, TT_CMapInfo * cmap_info)
{
	...
	if (cmap->clazz != &cff_cmap_encoding_class_rec && cmap->clazz != &cff_cmap_unicode_class_rec)
	{
		...
		FT_Service_TTCMaps service = (FT_Service_TTCMaps) ft_module_get_service(sfnt,
 ...
 *
 */
static FT_Error cff_get_ros(FT_Face face_, const char * *registry, const char * *ordering, FT_Int * supplement)
{
	CFF_Face face = (CFF_Face) face_;
	FT_Error error = FT_Err_Ok;
	CFF_Font cff = (CFF_Font) face->extra.data;
...
static FT_Error cff_get_is_cid(FT_Face face_, FT_Bool * is_cid)
{
	CFF_Face face = (CFF_Face) face_;
	FT_Error error = FT_Err_Ok;
	CFF_Font cff = (CFF_Font) face->extra.data;
...
static FT_Error cff_get_cid_from_glyph_index(FT_Face face_, FT_UInt32 glyph_index, FT_UInt32 * cid)
{
	CFF_Face face = (CFF_Face) face_;
	FT_Error error = FT_Err_Ok;
	CFF_Font cff;
 ...
 *
 */
static FT_Error cff_property_set(FT_Module module,	/* CFF_Driver */
...
static FT_Error cff_property_get(FT_Module module,	/* CFF_Driver */
 ...
 *
 */
static FT_Error cff_set_mm_blend(FT_Face face_, FT_UInt num_coords, FT_Fixed * coords)
{
	CFF_Face face = (CFF_Face)face_;
...
static FT_Error cff_get_mm_blend(FT_Face face_, FT_UInt num_coords, FT_Fixed * coords)
{
	CFF_Face face = (CFF_Face)face_;
	FT_Service_MultiMasters mm = (FT_Service_MultiMasters) face->mm;
...
static FT_Error cff_get_mm_var(FT_Face face_, FT_MM_Var * *master)
{
	CFF_Face face = (CFF_Face)face_;
	FT_Service_MultiMasters mm = (FT_Service_MultiMasters) face->mm;
...
static FT_Error cff_set_var_design(FT_Face face_, FT_UInt num_coords, FT_Fixed * coords)
{
	CFF_Face face = (CFF_Face)face_;
	FT_Service_MultiMasters mm = (FT_Service_MultiMasters) face->mm;
...
static FT_Error cff_get_var_design(FT_Face face_, FT_UInt num_coords, FT_Fixed * coords)
{
	CFF_Face face = (CFF_Face)face_;
	FT_Service_MultiMasters mm = (FT_Service_MultiMasters) face->mm;
...
static const FT_Service_MultiMastersRec cff_service_multi_masters = {
	...
	cff_get_mm_blend,	/* get_mm_blend   */
	cff_get_mm_var,	/* get_mm_var     */
	cff_set_var_design,	/* set_var_design */
