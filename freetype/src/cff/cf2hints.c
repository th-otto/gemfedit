...
static void cf2_hint_init(CF2_Hint hint,
			  ...
			  size_t indexStemHint, const CF2_Font font, CF2_Fixed hintOrigin, CF2_Fixed scale, FT_Bool bottom)
{
	CF2_Fixed width;
...
{
	return (FT_Bool) ((hint->flags & (CF2_PairBottom | CF2_PairTop)) != 0);
}
...
{
	return (FT_Bool) ((hint->flags & (CF2_PairTop | CF2_GhostTop)) != 0);
}


FT_LOCAL_DEF(FT_Bool) cf2_hint_isBottom(const CF2_Hint hint)
{
	return (FT_Bool) ((hint->flags & (CF2_PairBottom | CF2_GhostBottom)) != 0);
}
	...
	} else
	{
		...
		if (i == 0 && csCoord < hintmap->edge[0].csCoord)
		{
			...
			return ADD_INT32(FT_MulFix(SUB_INT32(csCoord,
												 hintmap->edge[0].csCoord), hintmap->scale), hintmap->edge[0].dsCoord);
		} else
		{
			/*
...
#if 0
			if (i == 0 || cf2_hint_isSynthetic(&hintmap->edge[i - 1]))
				downMinCounter = 0;

			if (j >= hintmap->count - 1 || cf2_hint_isSynthetic(&hintmap->edge[j + 1]))
				upMinCounter = 0;
#endif
...
static void cf2_hintmap_insertHint(CF2_HintMap hintmap, CF2_Hint bottomHintEdge, CF2_Hint topHintEdge)
{
	...
	if (cf2_hintmap_isValid(hintmap->initialHintMap) && !cf2_hint_isLocked(firstHintEdge))
	{
		if (isPair)
		{
			...
			CF2_Fixed midpoint = cf2_hintmap_map(hintmap->initialHintMap,
												 ADD_INT32(secondHintEdge->csCoord,
														   firstHintEdge->csCoord) / 2);
			CF2_Fixed halfWidth = FT_MulFix(SUB_INT32(secondHintEdge->csCoord,
													  firstHintEdge->csCoord) / 2,
											hintmap->scale);
 ...
 *
 */
FT_LOCAL_DEF(void) cf2_hintmap_build(CF2_HintMap hintmap,
				  CF2_ArrStack hStemHintArray,
				  CF2_ArrStack vStemHintArray, CF2_HintMask hintMask, CF2_Fixed hintOrigin, FT_Bool initialMap)
{
	FT_Byte *maskPtr;
	...
	for (i = 0, maskByte = 0x80; i < bitCount; i++)
	{
		if (maskByte & *maskPtr)
		{
			...
			if (cf2_hint_isLocked(&bottomHintEdge) ||
				cf2_hint_isLocked(&topHintEdge) || cf2_blues_capture(&font->blues, &bottomHintEdge, &topHintEdge))
			{
...
cf2_glyphpath_init(CF2_GlyphPath glyphpath, CF2_Font font, CF2_OutlineCallbacks callbacks, CF2_Fixed scaleY,
				   ...
				   CF2_Fixed hintOriginY, const CF2_Blues blues, const FT_Vector * fractionalTranslation)
{
	FT_ZERO(glyphpath);
 ...
 */
static void
cf2_glyphpath_hintPoint(CF2_GlyphPath glyphpath, CF2_HintMap hintmap, FT_Vector * ppt, CF2_Fixed x, CF2_Fixed y)
{
	...
	ppt->x = ADD_INT32(FT_MulFix(glyphpath->font->outerTransform.a, pt.x),
					   ADD_INT32(FT_MulFix(glyphpath->font->outerTransform.c, pt.y),
								 glyphpath->fractionalTranslation.x));
	ppt->y = ADD_INT32(FT_MulFix(glyphpath->font->outerTransform.b, pt.x),
...
#define CF2_CS_SCALE( x )         \
	...
	if (cf2_fixedAbs(intersection->x - ADD_INT32(u2->x, v1->x) / 2) >
		glyphpath->miterLimit || cf2_fixedAbs(intersection->y - ADD_INT32(u2->y, v1->y) / 2) > glyphpath->miterLimit)
		return FALSE;
...
static void cf2_glyphpath_pushPrevElem(CF2_GlyphPath glyphpath,
						   CF2_HintMap hintmap, FT_Vector * nextP0, const FT_Vector * nextP1, FT_Bool close)
{
	...
	case CF2_PathOpLineTo:
		...
		if (close)
		{
			...
			cf2_glyphpath_hintPoint(glyphpath,
									...
									&params.pt1, glyphpath->prevElemP1.x, glyphpath->prevElemP1.y);
		} else
		{
...
/* element                                                            */
static void cf2_glyphpath_pushMove(CF2_GlyphPath glyphpath, const FT_Vector * start)
{
	CF2_CallbackParamsRec params;
...
static void cf2_glyphpath_computeOffset(CF2_GlyphPath glyphpath,
							CF2_Fixed x1, CF2_Fixed y1, CF2_Fixed x2, CF2_Fixed y2, CF2_Fixed * x, CF2_Fixed * y)
{
	...
	*x = *y = 0;
	...
	glyphpath->callbacks->windingMomentum =
		ADD_INT32(glyphpath->callbacks->windingMomentum, cf2_getWindingMomentum(x1, y1, x2, y2));
 ...
 */
FT_LOCAL_DEF(void) cf2_glyphpath_moveTo(CF2_GlyphPath glyphpath, CF2_Fixed x, CF2_Fixed y)
{
	...
	if (!cf2_hintmap_isValid(&glyphpath->hintMap) || cf2_hintmask_isNew(glyphpath->hintMask))
		cf2_hintmap_build(&glyphpath->hintMap,
						  ...
						  glyphpath->vStemHintArray, glyphpath->hintMask, glyphpath->hintOriginY, FALSE);
...
FT_LOCAL_DEF(void) cf2_glyphpath_lineTo(CF2_GlyphPath glyphpath, CF2_Fixed x, CF2_Fixed y)
{
...
cf2_glyphpath_curveTo(CF2_GlyphPath glyphpath,
					  CF2_Fixed x1, CF2_Fixed y1, CF2_Fixed x2, CF2_Fixed y2, CF2_Fixed x3, CF2_Fixed y3)
{
	...
	cf2_glyphpath_computeOffset(glyphpath,
								glyphpath->currentCS.x, glyphpath->currentCS.y, x1, y1, &xOffset1, &yOffset1);
	...
	if (glyphpath->pathIsOpen)
	{
		...
		if (glyphpath->elemIsQueued)
			cf2_glyphpath_pushPrevElem(glyphpath,
									   &glyphpath->hintMap, &glyphpath->offsetStart0, &glyphpath->offsetStart1, TRUE);
