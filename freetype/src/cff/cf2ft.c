...
static void cf2_builder_moveTo(CF2_OutlineCallbacks callbacks, const CF2_CallbackParams params)
{
	...
	FT_UNUSED(params);						/* only used in debug mode */
...
static void cf2_builder_lineTo(CF2_OutlineCallbacks callbacks, const CF2_CallbackParams params)
{
	...
	if (!builder->path_begun)
	{
		...
		/* `path_begun'                                               */
		error = cff_builder_start_point(builder, params->pt0.x, params->pt0.y);
		if (error)
		{
			if (!*callbacks->error)
	...
	error = cff_builder_add_point1(builder, params->pt1.x, params->pt1.y);
	if (error)
	{
		if (!*callbacks->error)
			*callbacks->error = error;
...
static void cf2_builder_cubeTo(CF2_OutlineCallbacks callbacks, const CF2_CallbackParams params)
{
	...
	cff_builder_add_point(builder, params->pt1.x, params->pt1.y, 0);
	cff_builder_add_point(builder, params->pt2.x, params->pt2.y, 0);
	cff_builder_add_point(builder, params->pt3.x, params->pt3.y, 1);
}
...
static void cf2_getScaleAndHintFlag(CFF_Decoder *decoder, CF2_Fixed *x_scale, CF2_Fixed *y_scale, FT_Bool *hinted, FT_Bool *scaled)
{
...
static FT_UShort cf2_getUnitsPerEm(CFF_Decoder * decoder)
{
...
cf2_decoder_parse_charstrings(CFF_Decoder * decoder, FT_Byte * charstring_base, FT_ULong charstring_len)
{
	...
	{
		decoder->cff->cf2_instance.finalizer = (FT_Generic_Finalizer) cf2_free_instance;

		if (FT_ALLOC(decoder->cff->cf2_instance.data, sizeof(CF2_FontRec)))
...
FT_LOCAL_DEF(CFF_SubFont) cf2_getSubfont(CFF_Decoder * decoder)
{
...
FT_LOCAL_DEF(CFF_VStore) cf2_getVStore(CFF_Decoder * decoder)
{
	FT_ASSERT(decoder && decoder->cff);
...
FT_LOCAL_DEF(FT_UInt) cf2_getMaxstack(CFF_Decoder * decoder)
{
	FT_ASSERT(decoder && decoder->cff);
...
FT_LOCAL_DEF(FT_Error) cf2_getNormalizedVector(CFF_Decoder * decoder, FT_UInt * len, FT_Fixed * *vec)
{
	FT_ASSERT(decoder && decoder->builder.face);
	FT_ASSERT(vec && len);

	return cff_get_var_blend(&decoder->builder.face->root, len, NULL, vec, NULL);
}
#endif


/* get `y_ppem' from `CFF_Size' */
FT_LOCAL_DEF(CF2_Fixed) cf2_getPpemY(CFF_Decoder * decoder)
{
	FT_ASSERT(decoder && decoder->builder.face && decoder->builder.face->root.size);
	 ...
	 *
	 */
	return cf2_intToFixed(decoder->builder.face->root.size->metrics.y_ppem);
}
...
FT_LOCAL_DEF(CF2_Fixed) cf2_getStdVW(CFF_Decoder * decoder)
{
	FT_ASSERT(decoder && decoder->current_subfont);
...
FT_LOCAL_DEF(CF2_Fixed) cf2_getStdHW(CFF_Decoder * decoder)
{
	FT_ASSERT(decoder && decoder->current_subfont);
...
FT_LOCAL_DEF(void) cf2_getBlueValues(CFF_Decoder * decoder, size_t * count, FT_Pos * *data)
{
	FT_ASSERT(decoder && decoder->current_subfont);
...
FT_LOCAL_DEF(void) cf2_getOtherBlues(CFF_Decoder * decoder, size_t * count, FT_Pos * *data)
{
	FT_ASSERT(decoder && decoder->current_subfont);
...
FT_LOCAL_DEF(void) cf2_getFamilyBlues(CFF_Decoder * decoder, size_t * count, FT_Pos * *data)
{
	FT_ASSERT(decoder && decoder->current_subfont);
...
FT_LOCAL_DEF(void) cf2_getFamilyOtherBlues(CFF_Decoder * decoder, size_t * count, FT_Pos * *data)
{
	FT_ASSERT(decoder && decoder->current_subfont);
...
FT_LOCAL_DEF(CF2_Int) cf2_getLanguageGroup(CFF_Decoder * decoder)
{
	FT_ASSERT(decoder && decoder->current_subfont);
...
/* return 0 on success                                   */
FT_LOCAL_DEF(CF2_Int) cf2_initGlobalRegionBuffer(CFF_Decoder * decoder, CF2_Int subrNum, CF2_Buffer buf)
{
	CF2_UInt idx;
...
FT_LOCAL_DEF(FT_Error) cf2_getSeacComponent(CFF_Decoder * decoder, CF2_Int code, CF2_Buffer buf)
{
	CF2_Int gid;
	FT_Byte *charstring;
	FT_ULong len;
	...
	{
		gid = cff_lookup_glyph_by_stdcharcode(decoder->cff, (FT_Int) code);
		if (gid < 0)
			return FT_THROW(Invalid_Glyph_Format);
	}

	error = cff_get_glyph_data(decoder->builder.face, (CF2_UInt) gid, &charstring, &len);
...
FT_LOCAL_DEF(void) cf2_freeSeacComponent(CFF_Decoder * decoder, CF2_Buffer buf)
{
	FT_ASSERT(decoder);

	cff_free_glyph_data(decoder->builder.face, (FT_Byte **) & buf->start, (FT_ULong) (buf->end - buf->start));
}


FT_LOCAL_DEF(CF2_Int) cf2_initLocalRegionBuffer(CFF_Decoder * decoder, CF2_Int subrNum, CF2_Buffer buf)
{
	CF2_UInt idx;
	...
	buf->start = buf->ptr = decoder->locals[idx];
...
FT_LOCAL_DEF(CF2_Fixed) cf2_getDefaultWidthX(CFF_Decoder * decoder)
{
	FT_ASSERT(decoder && decoder->current_subfont);
...
FT_LOCAL_DEF(CF2_Fixed) cf2_getNominalWidthX(CFF_Decoder * decoder)
{
	FT_ASSERT(decoder && decoder->current_subfont);
