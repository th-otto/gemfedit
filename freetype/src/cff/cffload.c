...
static FT_Error cff_index_load_offsets(CFF_Index idx)
{
	...
	if (idx->count > 0 && !idx->offsets)
	{
		...
		if (FT_NEW_ARRAY(idx->offsets, idx->count + 1) ||
			FT_STREAM_SEEK(idx->start + idx->hdr_size) || FT_FRAME_ENTER(data_size))
			goto Exit;
...
static FT_Error cff_index_get_pointers(CFF_Index idx, FT_Byte *** table, FT_Byte ** pool, FT_ULong * pool_size)
{
	...
	*table = NULL;
	...
	if (idx->count > 0 && !FT_NEW_ARRAY(t, idx->count + 1) && (!pool || !FT_ALLOC(new_bytes, new_size)))
	{
		...
		if (cur_offset != 0)
		{
			FT_TRACE0(("cff_index_get_pointers:" " invalid first offset value %d set to zero\n", cur_offset));
			cur_offset = 0;
		}
...
cff_index_access_element(CFF_Index idx, FT_UInt32 element, FT_Byte ** pbytes, FT_ULong * pbyte_len)
{
	FT_Error error = FT_Err_Ok;
...
FT_LOCAL_DEF(void) cff_index_forget_element(CFF_Index idx, FT_Byte ** pbytes)
{
	if (idx->bytes == 0)
	{
		FT_Stream stream = idx->stream;
...
FT_LOCAL_DEF(FT_String *) cff_index_get_name(CFF_Font font, FT_UInt element)
{
...
FT_LOCAL_DEF(const FT_String *) cff_index_get_string(CFF_Font font, FT_UInt element)
{
	return (element < font->num_strings) ? (const FT_String *)font->strings[element] : NULL;
}


FT_LOCAL_DEF(const FT_String *) cff_index_get_sid_string(CFF_Font font, FT_UInt sid)
{
...
FT_LOCAL_DEF(FT_Byte) cff_fd_select_get(CFF_FDSelect fdselect, FT_UInt32 glyph_index)
{
	FT_Byte fd = 0;
...
FT_LOCAL_DEF(FT_UInt) cff_charset_cid_to_gindex(CFF_Charset charset, FT_UInt32 cid)
{
	FT_UInt result = 0;
...
static FT_Error cff_charset_load(CFF_Charset charset,
	FT_UInt num_glyphs, FT_Stream stream, FT_ULong base_offset, FT_ULong offset, FT_Bool invert)
{
	FT_Memory memory = stream->memory;
	FT_Error error = FT_Err_Ok;
	FT_UShort glyph_sid;
...
static FT_Error cff_vstore_load(CFF_VStoreRec * vstore, FT_Stream stream, FT_ULong base_offset, FT_ULong offset)
{
	...
	if (offset)
	{
		...
		if (FT_STREAM_SEEK(vsOffset + regionListOffset) ||
			FT_READ_USHORT(vstore->axisCount) || FT_READ_USHORT(vstore->regionCount))
			goto Exit;
...
FT_LOCAL_DEF(FT_Error) cff_blend_doBlend(CFF_SubFont subFont, CFF_Parser parser, FT_UInt numBlends)
{
	FT_UInt delta;
	FT_UInt base;
	FT_UInt i, j;
	...
	for (i = 0; i < numBlends; i++)
	{
		...
		for (j = 1; j < blend->lenBV; j++)
			sum = ADD_INT32(sum, FT_MulFix(*weight++, cff_parse_num(parser, &parser->stack[delta++]) * 65536L));
...
FT_LOCAL_DEF(FT_Error) cff_blend_build_vector(CFF_Blend blend, FT_UInt vsindex, FT_UInt lenNDV, FT_Fixed * NDV)
{
	FT_Error error = FT_Err_Ok;			/* for FT_REALLOC */
	...
	for (master = 0; master < len; master++)
	{
		...
		{
			FT_TRACE4((" cff_blend_build_vector:" " region index out of range\n"));
			error = FT_THROW(Invalid_File_Format);
			goto Exit;
		}
...
FT_LOCAL_DEF(FT_Bool) cff_blend_check_vector(CFF_Blend blend, FT_UInt vsindex, FT_UInt lenNDV, FT_Fixed * NDV)
{
	if (!blend->builtBV ||
		blend->lastVsindex != vsindex ||
		blend->lenNDV != lenNDV || (lenNDV && memcmp(NDV, blend->lastNDV, lenNDV * sizeof(*NDV)) != 0))
	{
...
FT_LOCAL_DEF(FT_Error) cff_get_var_blend(FT_Face face_,
...
FT_LOCAL_DEF(void) cff_done_blend(FT_Face face_)
{
	CFF_Face face = (CFF_Face)face_;
	FT_Service_MultiMasters mm = (FT_Service_MultiMasters) face->mm;
...
static FT_Error cff_encoding_load(CFF_Encoding encoding,
	CFF_Charset charset, FT_UInt num_glyphs, FT_Stream stream, FT_ULong base_offset, FT_ULong offset)
{
	FT_Error error = FT_Err_Ok;
	FT_UInt count;
	FT_UInt j;
...
FT_LOCAL_DEF(FT_Error) cff_load_private_dict(CFF_Font font, CFF_SubFont subfont, FT_UInt lenNDV, FT_Fixed * NDV)
{
	FT_Error error = FT_Err_Ok;
	...
	if (cff_parser_init(&parser,
						...
						priv, font->library, stackSize, top->num_designs, top->num_axes))
		goto Exit;
...
static FT_Error cff_subfont_load(CFF_SubFont subfont,
	CFF_Index idx,
	FT_UInt font_index, FT_Stream stream, FT_ULong base_offset, FT_UInt code, CFF_Font font, CFF_Face face)
{
	FT_Error error;
	CFF_ParserRec parser;
	FT_Byte *dict = NULL;
	...
	if (!cff2)
	{
		...
		if (face->root.internal->random_seed == -1)
		{
			...
			if (driver->random_seed)
			{
				do
				{
					driver->random_seed = (FT_Int32) cff_random((FT_UInt32) driver->random_seed);
...
FT_LOCAL_DEF(FT_Error) cff_font_load(FT_Library library,
	FT_Stream stream, FT_Int face_index, CFF_Font font, CFF_Face face, FT_Bool pure_cff, FT_Bool cff2)
{
...
#define FT_STRUCTURE  CFF_FontRec
	...
	} else
	{
		...
		if (FT_SET_ERROR(cff_index_init(&font->font_dict_index, stream, 0, cff2)) ||
		...
		{
			FT_ERROR(("cff_font_load:" " not enough entries in Top DICT index\n"));
			error = FT_THROW(Invalid_File_Format);
			goto Exit;
		}
	}
	...
	error = cff_subfont_load(&font->top_font,
							 ...
							 subfont_index,
							 stream, base_offset, cff2 ? CFF2_CODE_TOPDICT : CFF_CODE_TOPDICT, font, face);
	if (error)
		goto Exit;
	...
	if (dict->cid_registry != 0xFFFFU || cff2)
	{
		...
		for (idx = 0; idx < fd_index.count; idx++)
		{
			...
			error = cff_subfont_load(sub,
									 &fd_index,
									 idx,
									 stream, base_offset, cff2 ? CFF2_CODE_FONTDICT : CFF_CODE_TOPDICT, font, face);
			if (error)
				goto Fail_CID;
		}
		...
		if (!cff2 || fd_index.count > 1)
			error = CFF_Load_FD_Select(&font->fd_select,
									   font->charstrings_index.count, stream, base_offset + dict->cid_fd_select_offset);
	...
	if (!cff2 && font->num_glyphs > 0)
	{
		...
		if (dict->cid_registry == 0xFFFFU)
		{
			error = cff_encoding_load(&font->encoding,
									  &font->charset, font->num_glyphs, stream, base_offset, dict->encoding_offset);
			if (error)
				goto Exit;
		}
	}
