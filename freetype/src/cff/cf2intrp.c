...
FT_LOCAL_DEF(void) cf2_hintmask_init(CF2_HintMask hintmask, FT_Error * error)
{
	FT_ZERO(hintmask);
...
FT_LOCAL_DEF(void) cf2_hintmask_setNew(CF2_HintMask hintmask, FT_Bool val)
{
	hintmask->isNew = val;
}
...
FT_LOCAL_DEF(void) cf2_hintmask_setAll(CF2_HintMask hintmask, size_t bitCount)
{
	size_t i;
	CF2_UInt mask = (1 << (-(CF2_Int) bitCount & 7)) - 1;
...
static void cf2_doStems(const CF2_Font font, CF2_Stack opStack, CF2_ArrStack stemHintArray, CF2_Fixed * width, FT_Bool * haveWidth, CF2_Fixed hintOffset)
{
	...
	if (hasWidthArg && !*haveWidth)
		*width = cf2_stack_getReal(opStack, 0) + cf2_getNominalWidthX(font->decoder);
...
static void cf2_doFlex(CF2_Stack opStack,
   CF2_Fixed * curX,
   CF2_Fixed * curY, CF2_GlyphPath glyphPath, const FT_Bool * readFromStack, FT_Bool doConditionalLastRead)
{
	CF2_Fixed vals[14];
	CF2_UInt idx;
	FT_Bool isHFlex;
	...
	for (j = 0; j < 2; j++)
		cf2_glyphpath_curveTo(glyphPath, vals[j * 6 + 2],
							  vals[j * 6 + 3], vals[j * 6 + 4], vals[j * 6 + 5], vals[j * 6 + 6], vals[j * 6 + 7]);
 ...
 *
 */
FT_LOCAL_DEF(void) cf2_interpT2CharString(CF2_Font font,
	CF2_Buffer buf,
	...
	*charstring = *buf;					/* structure copy */
	...
	while (1)
	{
		...
		case cf2_cmdBLEND:
			{
				...
				if (cff_blend_check_vector(&font->blend, (FT_UInt) font->vsindex, (FT_UInt) font->lenNDV, font->NDV))
				{
					lastError = cff_blend_build_vector(&font->blend,
													   (FT_UInt) font->vsindex, (FT_UInt) font->lenNDV, font->NDV);
					if (lastError)
						goto exit;
				}
		...
		case cf2_cmdHSTEM:
			...
			cf2_doStems(font, opStack, &hStemHintArray, width, &haveWidth, 0);
		...
		case cf2_cmdVSTEM:
			...
			if (cf2_hintmask_isValid(&hintMask))
			{
				FT_TRACE4(("cf2_interpT2CharString:" " invalid vertical hint mask\n"));
				break;
			}
		...
		case cf2_cmdCALLSUBR:
			{
				...
				FT_TRACE4((op1 == cf2_cmdCALLGSUBR ? " callgsubr" : " callsubr"));
				...
				case cf2_escHFLEX:
					{
						...
						cf2_doFlex(opStack,
								   &curX, &curY, &glyphPath, readFromStack, FALSE /* doConditionalLastRead */ );
					}
					continue;
		...
		case cf2_cmdHINTMASK:
			...
			if (op1 == cf2_cmdHINTMASK)
			{
				...
				cf2_hintmask_read(&hintMask,
								  charstring, cf2_arrstack_size(&hStemHintArray) + cf2_arrstack_size(&vStemHintArray));
			} else
			{
				/*
...
#if 0
	/* ehm, no, we dont. A cmdENDCHAR is simulated at the end of the string */
	FT_TRACE4(("cf2_interpT2CharString:  charstring ends without ENDCHAR\n"));
#endif
