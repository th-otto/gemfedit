	...
	if (state->buf_clear || offset >= state->buf_size || state->free_ent >= state->free_bits)
	{
		if (state->free_ent >= state->free_bits)
		{
			...
			state->free_bits = state->num_bits < state->max_bits
				? (FT_UInt) ((1UL << num_bits) - 256) : state->max_free + 1;
		}
...
FT_LOCAL_DEF(void) ft_lzwstate_init(FT_LzwState state, FT_Stream source)
{
	FT_ZERO(state);
...
FT_LOCAL_DEF(FT_ULong) ft_lzwstate_io(FT_LzwState state, FT_Byte * buffer, FT_ULong out_size)
{
	FT_ULong result = 0;
	...
	case FT_LZW_PHASE_START:
		{
			...
			state->free_bits = state->num_bits < state->max_bits
				? (FT_UInt) ((1UL << state->num_bits) - 256) : state->max_free + 1;
