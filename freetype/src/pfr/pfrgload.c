...
FT_LOCAL_DEF(void) pfr_glyph_init(PFR_Glyph glyph, FT_GlyphLoader loader)
{
	FT_ZERO(glyph);
...
static FT_Error pfr_glyph_load_compound(PFR_Glyph glyph, FT_Byte * p, FT_Byte * limit)
{
	...
	if (org_count + count > glyph->max_subs)
	{
		...
		{
			error = FT_THROW(Invalid_Table);
			FT_ERROR(("pfr_glyph_load_compound:" " too many compound glyphs components\n"));
			goto Exit;
		}
...
static FT_Error
pfr_glyph_load_rec(PFR_Glyph glyph, FT_Stream stream, FT_ULong gps_offset, FT_ULong offset, FT_ULong size)
{
	FT_Error error;
	FT_Byte *p;
	FT_Byte *limit;
...
FT_LOCAL_DEF(FT_Error) pfr_glyph_load(PFR_Glyph glyph, FT_Stream stream, FT_ULong gps_offset, FT_ULong offset, FT_ULong size)
{
