...
static void pfr_lookup_bitmap_data(FT_Byte * base,
	FT_Byte * limit,
	FT_UInt count,
	FT_UInt * flags, FT_UInt char_code, FT_ULong * found_offset, FT_ULong * found_size)
{
	FT_UInt min, max;
	FT_UInt char_len;
	FT_Bool two = (*flags & PFR_BITMAP_2BYTE_CHARCODE) != 0;
	FT_Byte *buff;
  ...
  Found_Strike:
	...
	/* now look up the glyph's position within the file */
	{
		...
		pfr_lookup_bitmap_data(stream->cursor,
							   stream->limit,
							   strike->num_bitmaps, &strike->flags, character->char_code, &gps_offset, &gps_size);
			...
			/* Allocate and read bitmap data */
			{
				...
				if (!error)
					error = pfr_load_bitmap_bits(p,
												 stream->limit,
												 format,
												 (face->header.color_flags & PFR_FLAG_INVERT_BITMAP) != 0, &glyph->root.bitmap);
			}
		}
