...
FT_LOCAL_DEF(void) pfr_face_done(FT_Face pfrface)
{
...
FT_LOCAL_DEF(FT_Error) pfr_face_init(FT_Stream stream, FT_Face pfrface, FT_Int face_index, FT_Int num_params, FT_Parameter * params)
{
	PFR_Face face = (PFR_Face) pfrface;
	FT_Error error;
	...
	/* load the face */
	error = pfr_log_font_load(&face->log_font,
							  stream,
							  (FT_UInt) (face_index & 0xFFFF),
							  face->header.log_dir_offset, face->header.phy_font_max_size_high != 0);
	if (error)
		goto Exit;
	...
	/* now set up all root face fields */
	{
		...
		{
			FT_UInt n;
			FT_UInt count = phy_font->num_strikes;
			FT_Bitmap_Size *size;
			PFR_Strike strike;
...
FT_LOCAL_DEF(FT_Error) pfr_slot_init(FT_GlyphSlot pfrslot)
{
...
FT_LOCAL_DEF(void) pfr_slot_done(FT_GlyphSlot pfrslot)
{
	PFR_Slot slot = (PFR_Slot) pfrslot;
...
FT_LOCAL_DEF(FT_Error) pfr_slot_load(FT_GlyphSlot pfrslot, FT_Size pfrsize, FT_UInt32 gindex, FT_Int32 load_flags)
{
	PFR_Slot slot = (PFR_Slot) pfrslot;
	...
	if (!error)
	{
		...
		scaling = (load_flags & FT_LOAD_NO_SCALE) == 0;
...
FT_LOCAL_DEF(FT_Error) pfr_face_get_kerning(FT_Face pfrface, FT_UInt glyph1, FT_UInt glyph2, FT_Vector *kerning)
{
	PFR_Face face = (PFR_Face) pfrface;
	...
	/* now search the list of kerning items */
	{
	  ...
	  FoundPair:						/* we found an item, now parse it and find the value if any */
		if (FT_STREAM_SEEK((FT_ULong) item->offset) || FT_FRAME_ENTER(item->pair_count * item->pair_size))
			goto Exit;

		{
			...
			FT_Bool twobytes = (item->flags & PFR_KERN_2BYTE_CHAR) != 0;
			FT_Bool twobyte_adj = (item->flags & PFR_KERN_2BYTE_ADJ) != 0;
			FT_Byte *p;
			FT_UInt32 cpair;
