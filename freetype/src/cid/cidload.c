...
FT_LOCAL_DEF(FT_ULong) cid_get_offset(FT_Byte * *start, FT_Byte offsize)
{
	FT_ULong result;
	FT_Byte *p = *start;
...
FT_CALLBACK_DEF(FT_Error) parse_fd_array(FT_Face face_, void *parser_)
{
	...
	{
		FT_TRACE0(("parse_fd_array: adjusting FDArray size" " (from %d to %d)\n", num_dicts, stream->size / 100));
...
static FT_Error cid_read_subrs(CID_Face face)
{
	...
	for (n = 0; n < cid->num_dicts; n++, subr++)
	{
		...
		FT_UInt count;
		FT_UInt num_subrs = dict->num_subrs;
		FT_ULong data_len;
		FT_Byte *p;
...
FT_LOCAL_DEF(FT_Error) cid_face_open(CID_Face face, FT_Int face_index)
{
	CID_Loader loader;
	CID_Parser *parser;
	FT_Memory memory = face->root.memory;
	...
	{
		FT_ERROR(("cid_parse_dict:" " Invalid `FDBytes' or `GDBytes' value\n"));
		error = FT_THROW(Invalid_File_Format);
		goto Exit;
	}
	...
	for (n = 0; n < cid->num_dicts; n++)
	{
		...
		if (dict->sd_bytes > 4)
		{
			FT_ERROR(("cid_parse_dict:" " Values of `SDBytes' larger than 4" " are not supported\n"));
			error = FT_THROW(Invalid_File_Format);
			goto Exit;
		}
