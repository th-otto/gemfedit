...
static void gray_render_line(RAS_ARG_ TPos to_x, TPos to_y)
{
	...
	if (ey1 != ey2)
	{
		...
		do
		{
			...
			x2 = x + delta;
			gray_render_scanline(RAS_VAR_ ey1, x, (TCoord) (ONE_PIXEL - first), x2, first);
			x = x2;
...
static int gray_move_to(const FT_Vector * to, void *user)
{
...
static int gray_line_to(const FT_Vector * to, void *user)
{
	gray_PWorker worker = (gray_PWorker) user;
...
static int gray_conic_to(const FT_Vector * control, const FT_Vector * to, void *user)
{
	gray_PWorker worker = (gray_PWorker) user;
...
static int gray_cubic_to(const FT_Vector * control1, const FT_Vector * control2, const FT_Vector * to, void *user)
{
	gray_PWorker worker = (gray_PWorker) user;
	...
	for (; min < max_y; min = max)
	{
		...
		do
		{
			...
			/* memory management */
			{
				...
				cell_start = (ycount * sizeof(PCell) + sizeof(TCell) - 1) / sizeof(TCell);
...
static int gray_raster_render(FT_Raster raster, const FT_Raster_Params * params)
{
	...
	if (cbox.xMin < -0x1000000L || cbox.xMax > 0x1000000L || cbox.yMin < -0x1000000L || cbox.yMax > 0x1000000L)
		return FT_THROW(Invalid_Outline);
