...
FT_LOCAL_DEF(void) gxv_prop_validate(FT_Bytes table, FT_Face face, FT_Validator ftvalid)
{
	FT_Bytes p = table;
	FT_Bytes limit = 0;
	...
	if (format == 0)
	{
		FT_TRACE3(("(format 0, no per-glyph properties, " "remaining %d bytes are skipped)", limit - p));
		goto Exit;
	}
