...
static int gxv_compare_ushort_offset(const void *a_, const void *b_)
{
	const FT_UShort *a = (const FT_UShort *)a_;
	const FT_UShort *b = (const FT_UShort *)b_;
	if (*a < *b)
		return -1;
	if (*a > *b)
		return 1;
...
FT_LOCAL_DEF(void) gxv_set_length_by_ushort_offset(FT_UShort * offset,
	FT_UShort ** length,
	FT_UShort * buff, FT_UInt nmemb, FT_UShort limit, GXV_Validator gxvalid)
{
	FT_UInt i;
	...
	ft_qsort(buff, (nmemb + 1), sizeof(*buff), gxv_compare_ushort_offset);
...
FT_LOCAL_DEF(void) gxv_set_length_by_ulong_offset(FT_ULong * offset,
	FT_ULong ** length,
	FT_ULong * buff, FT_UInt nmemb, FT_ULong limit, GXV_Validator gxvalid)
{
	FT_UInt i;
...
FT_LOCAL_DEF(void) gxv_array_getlimits_ushort(FT_Bytes table, FT_Bytes limit, FT_UShort * min, FT_UShort * max, GXV_Validator gxvalid)
{
	FT_Bytes p = table;
...
static void gxv_BinSrchHeader_check_consistency(GXV_BinSrchHeader * binSrchHeader, GXV_Validator gxvalid)
{
	...
	entrySelector--;
	searchRange = searchRange * binSrchHeader->unitSize;
	rangeShift = binSrchHeader->nUnits * binSrchHeader->unitSize - searchRange;

	if (searchRange != binSrchHeader->searchRange ||
		entrySelector != binSrchHeader->entrySelector || rangeShift != binSrchHeader->rangeShift)
	{
		...
		GXV_TRACE(("originally: unitSize=%d, nUnits=%d, "
				   ...
				   binSrchHeader->searchRange, binSrchHeader->entrySelector, binSrchHeader->rangeShift));
...
FT_LOCAL_DEF(void) gxv_BinSrchHeader_validate(FT_Bytes table,
	FT_Bytes limit, FT_UShort * unitSize_p, FT_UShort * nUnits_p, GXV_Validator gxvalid)
{
	FT_Bytes p = table;
	...
	for (unit = 0, gid = 0; unit < nUnits; unit++)
	{
		...
		if (lastGlyph < firstGlyph)
		{
			GXV_TRACE(("reverse ordered range specification at unit %d:",
					   " lastGlyph %d < firstGlyph %d ", unit, lastGlyph, firstGlyph));
			GXV_SET_ERR_IF_PARANOID(FT_INVALID_GLYPH_ID);
...
static void gxv_LookupTable_fmt4_validate(FT_Bytes table, FT_Bytes limit, GXV_Validator gxvalid)
{
	...
	for (unit = 0, gid = 0; unit < nUnits; unit++)
	{
		...
		for (gid = firstGlyph; gid <= lastGlyph; gid++)
		{
			value = gxvalid->lookupfmt4_trans(gid - firstGlyph, &base_value, limit, gxvalid);
...
static void gxv_LookupTable_fmt6_validate(FT_Bytes table, FT_Bytes limit, GXV_Validator gxvalid)
{
	...
	for (unit = 0, prev_glyph = 0; unit < nUnits; unit++)
	{
		...
		if (gxv_glyphid_validate(glyph, gxvalid))
			GXV_TRACE((" endmarker found within defined range" " (entry %d < nUnits=%d)\n", unit, nUnits));
...
static void gxv_LookupTable_fmt8_validate(FT_Bytes table, FT_Bytes limit, GXV_Validator gxvalid)
{
	...
	gxv_glyphid_validate(firstGlyph, gxvalid);
	gxv_glyphid_validate(firstGlyph + glyphCount, gxvalid);
	...
	for (i = 0; i < glyphCount; i++)
	{
		GXV_LIMIT_CHECK(2);
		value = GXV_LOOKUP_VALUE_LOAD(p, gxvalid->lookupval_sign);
		gxvalid->lookupval_func(firstGlyph + i, &value, gxvalid);
	}
...
FT_LOCAL_DEF(void) gxv_LookupTable_validate(FT_Bytes table, FT_Bytes limit, GXV_Validator gxvalid)
{
	FT_Bytes p = table;
	FT_UShort format;

	static GXV_Validate_Func const fmt_funcs_table[] = {
...
FT_LOCAL_DEF(FT_Int) gxv_glyphid_validate(FT_UShort gid, GXV_Validator gxvalid)
{
	FT_Face face;
...
FT_LOCAL_DEF(void) gxv_ctlPoint_validate(FT_UShort gid, FT_UShort ctl_point, GXV_Validator gxvalid)
{
	FT_Face face;
	FT_Error error;
...
FT_LOCAL_DEF(void) gxv_sfntName_validate(FT_UShort name_index, FT_UShort min_index, FT_UShort max_index, GXV_Validator gxvalid)
{
	FT_SfntName name;
	FT_UInt i;
	FT_UInt nnames;
...
static void gxv_ClassTable_validate(FT_Bytes table,
	FT_UShort * length_p, FT_UShort stateSize, FT_Byte * maxClassID_p, GXV_Validator gxvalid)
{
	FT_Bytes p = table;
	...
	*maxClassID_p = 3;					/* Classes 0, 2, and 3 are predefined */
	...
	gxv_glyphid_validate(firstGlyph + nGlyphs, gxvalid);
...
static void gxv_StateArray_validate(FT_Bytes table,
	FT_UShort * length_p,
	FT_Byte maxClassID,
	FT_UShort stateSize, FT_Byte * maxState_p, FT_Byte * maxEntry_p, GXV_Validator gxvalid)
{
	FT_Bytes p = table;
	FT_Bytes limit = table + *length_p;
	FT_Byte clazz;
	...
	for (entry = 0; entry <= maxEntry; entry++)
	{
		...
		if (gxvalid->statetable.entry_validate_func)
			gxvalid->statetable.entry_validate_func(state,
													flags, &glyphOffset, statetable_table, statetable_limit, gxvalid);
	}
...
FT_LOCAL_DEF(void) gxv_StateTable_subtable_setup(FT_UShort table_size,
...
FT_LOCAL_DEF(void) gxv_StateTable_validate(FT_Bytes table, FT_Bytes limit, GXV_Validator gxvalid)
{
	FT_UShort stateSize;
	...
	setup_func((FT_UShort) (limit - table),
			   classTable, stateArray, entryTable, &classTable_length, &stateArray_length, &entryTable_length, gxvalid);
	...
	if (stateArray != 0)
	{
		gxv_StateArray_validate(table + stateArray,
								&stateArray_length, maxClassID, stateSize, &maxState, &maxEntry, gxvalid);
	} else
	{
#if 0
	...
	if (entryTable != 0)
		gxv_EntryTable_validate(table + entryTable,
								&entryTable_length,
								maxEntry, stateArray, stateArray_length, maxClassID, table, limit, gxvalid);
...
FT_LOCAL_DEF(void) gxv_XStateTable_subtable_setup(FT_ULong table_size,
	FT_ULong classTable,
	FT_ULong stateArray,
	FT_ULong entryTable,
...
static GXV_LookupValueDesc gxv_XClassTable_lookupfmt4_transit(FT_UShort relative_gindex,
	GXV_LookupValueCPtr base_value_p, FT_Bytes lookuptbl_limit, GXV_Validator gxvalid)
{
	FT_Bytes p;
	FT_Bytes limit;
	FT_UShort offset;
...
static void gxv_XStateArray_validate(FT_Bytes table,
	...
	FT_ULong stateSize, FT_UShort * maxState_p, FT_UShort * maxEntry_p, GXV_Validator gxvalid)
{
	FT_Bytes p = table;
	FT_Bytes limit = table + *length_p;
	FT_UShort clazz;
	...
	for (entry = 0; entry <= maxEntry; entry++)
	{
		...
		if (gxvalid->xstatetable.entry_validate_func)
			gxvalid->xstatetable.entry_validate_func(state,
													 flags,
													 &glyphOffset, xstatetable_table, xstatetable_limit, gxvalid);
	}
...
FT_LOCAL_DEF(void) gxv_XStateTable_validate(FT_Bytes table, FT_Bytes limit, GXV_Validator gxvalid)
{
	/* StateHeader members */
	...
	if (entryTable != 0)
		gxv_XEntryTable_validate(table + entryTable,
								 &entryTable_length,
								 maxEntry, stateArray_length, gxvalid->xstatetable.maxClassID, table, limit, gxvalid);
...
FT_LOCAL_DEF(void) gxv_odtect_add_range(FT_Bytes start, FT_ULong length, const FT_String * name, GXV_odtect_Range odtect)
{
...
FT_LOCAL_DEF(void) gxv_odtect_validate(GXV_odtect_Range odtect, GXV_Validator gxvalid)
{
	FT_UInt i, j;
