...
static void gxv_just_check_max_gid(FT_UShort gid, const FT_String * msg_tag, GXV_Validator gxvalid)
{
	...
	GXV_TRACE(("just table includes too large %s" " GID=%d > %d (in maxp)\n", msg_tag, gid, gxvalid->face->num_glyphs));
...
static void gxv_just_wdp_entry_validate(FT_Bytes table, FT_Bytes limit, GXV_Validator gxvalid)
{
	...
	{
		GXV_TRACE(("just table includes non-zero value" " in unused justClass higher bits" " of WidthDeltaPair"));
...
static void gxv_just_actSubrecord_type0_validate(FT_Bytes table, FT_Bytes limit, GXV_Validator gxvalid)
{
	...
	{
		GXV_TRACE(("just table includes invalid range spec:" " lowerLimit(%d) > upperLimit(%d)\n"));
		GXV_SET_ERR_IF_PARANOID(FT_INVALID_DATA);
	}
...
static void
gxv_just_pcTable_LookupValue_entry_validate(FT_UShort glyph, GXV_LookupValueCPtr value_p, GXV_Validator gxvalid)
{
	FT_UNUSED(glyph);
...
static void gxv_just_justClassTable_validate(FT_Bytes table, FT_Bytes limit, GXV_Validator gxvalid)
{
	...
	if (subFeatureFlags)
		GXV_TRACE(("  justClassTable: nonzero value (0x%08x)" " in unused subFeatureFlags\n", subFeatureFlags));
...
static void gxv_just_justData_validate(FT_Bytes table, FT_Bytes limit, GXV_Validator gxvalid)
{
	...
	if (justClassTableOffset)
	{
		...
		gxv_odtect_add_range(gxvalid->root->base + justClassTableOffset,
							 gxvalid->subtable_length, "just_justClassTable", odtect);
	}
...
FT_LOCAL_DEF(void) gxv_just_validate(FT_Bytes table, FT_Face face, FT_Validator ftvalid)
{
	FT_Bytes p = table;
	FT_Bytes limit = 0;
