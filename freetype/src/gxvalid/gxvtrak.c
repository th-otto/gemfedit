...
static void gxv_trak_trackData_validate(FT_Bytes table, FT_Bytes limit, GXV_Validator gxvalid)
{
	...
	gxv_odtect_add_range(gxvalid->root->base
						 ...
						 - GXV_TRAK_DATA(trackValueOffset_min) + nSizes * 2, "trackValue array", odtect);
...
FT_LOCAL_DEF(void) gxv_trak_validate(FT_Bytes table, FT_Face face, FT_Validator ftvalid)
{
	FT_Bytes p = table;
	FT_Bytes limit = 0;
