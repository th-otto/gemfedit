...
FT_LOCAL_DEF(FT_Error) T1_Size_Request(FT_Size t1size,	/* T1_Size */
									   FT_Size_Request req)
{
	T1_Size size = (T1_Size) t1size;
	PSH_Globals_Funcs funcs = T1_Size_Get_Globals_Funcs(size);
	...
	if (funcs)
		funcs->set_scale((PSH_Globals) t1size->internal->module_data,
						 size->root.metrics.x_scale, size->root.metrics.y_scale, 0, 0);
...
/*                                                                       */
FT_LOCAL_DEF(FT_Error) T1_Face_Init(FT_Stream stream, FT_Face t1face,	/* T1_Face */
									FT_Int face_index, FT_Int num_params, FT_Parameter * params)
{
	...
	/* set up root face fields */
	{
		...
		root->face_flags |= FT_FACE_FLAG_SCALABLE |
			FT_FACE_FLAG_HORIZONTAL | FT_FACE_FLAG_GLYPH_NAMES | FT_FACE_FLAG_HINTER;
...
/*    FreeType error code.  0 means success.                             */
/*                                                                       */
FT_LOCAL_DEF(FT_Error) T1_Driver_Init(FT_Module driver)
{
	FT_UNUSED(driver);
