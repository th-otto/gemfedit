...
/*                                                                       */
FT_LOCAL_DEF(FT_Error) tt_glyphzone_new(FT_Memory memory, FT_UShort maxPoints, FT_Short maxContours, TT_GlyphZone zone)
{
	FT_Error error;
	...
	if (FT_NEW_ARRAY(zone->org, maxPoints) ||
		...
		FT_NEW_ARRAY(zone->tags, maxPoints) || FT_NEW_ARRAY(zone->contours, maxContours))
	{
...
#define TRICK_SFNT_ID_prep  2
	...
	for (i = 0; i < face->num_tables; i++)
	{
		...
		/* cant use switch here because "Tag" is FT_ULong */
		if (face->dir_tables[i].Tag == TTAG_cvt)
		{
			k = TRICK_SFNT_ID_cvt;
			has_cvt = TRUE;
		} else if (face->dir_tables[i].Tag == TTAG_fpgm)
		{
			k = TRICK_SFNT_ID_fpgm;
			has_fpgm = TRUE;
		} else if (face->dir_tables[i].Tag == TTAG_prep)
		{
...

	{
		...
		if (FT_HAS_MULTIPLE_MASTERS(ttface) && instance_index > 0)
		{
			...
			if (face->blend->mmvar->namedstyle)
			{
				...
				error = TT_Set_Var_Design((FT_Face) face, face->blend->mmvar->num_axis, named_style->coords);
				if (error)
					goto Exit;
...
/*    FreeType error code.  0 means success.                             */
/*                                                                       */
FT_LOCAL_DEF(FT_Error) tt_size_run_fpgm(TT_Size size, FT_Bool pedantic)
{
...
/*    FreeType error code.  0 means success.                             */
/*                                                                       */
FT_LOCAL_DEF(FT_Error) tt_size_run_prep(TT_Size size, FT_Bool pedantic)
{
	TT_Face face = (TT_Face) size->root.face;
	TT_ExecContext exec;
	FT_Error error;
...
FT_LOCAL_DEF(FT_Error) tt_size_ready_bytecode(TT_Size size, FT_Bool pedantic)
{
...
/*                                                                       */
FT_LOCAL_DEF(FT_Error) tt_size_reset(TT_Size size, FT_Bool only_height)
{
	TT_Face face;
...
/*    FreeType error code.  0 means success.                             */
/*                                                                       */
FT_LOCAL_DEF(FT_Error) tt_driver_init(FT_Module ttdriver)
{
#ifdef TT_USE_BYTECODE_INTERPRETER
...
/*                                                                       */
FT_LOCAL_DEF(void) tt_driver_done(FT_Module ttdriver)
{
	FT_UNUSED(ttdriver);
}
