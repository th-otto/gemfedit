...
/*                                                                       */
FT_LOCAL_DEF(void) TT_Get_HMetrics(TT_Face face, FT_UInt32 idx, FT_Short * lsb, FT_UShort * aw)
{
...
/*                                                                       */
FT_LOCAL_DEF(void) TT_Get_VMetrics(TT_Face face, FT_UInt32 idx, FT_Pos yMax, FT_Short * tsb, FT_UShort * ah)
{
...
static FT_Error tt_get_metrics(TT_Loader loader, FT_UInt32 glyph_index)
{
...
static void tt_get_metrics_incr_overrides(TT_Loader loader, FT_UInt32 glyph_index)
{
	TT_Face face = loader->face;
	FT_Short left_bearing = 0;
	FT_Short top_bearing = 0;
	FT_UShort advance_width = 0;
	FT_Short advance_height = 0;
		...
		face->root.internal->incremental_interface->funcs->get_glyph_metrics)
	{
		...
		error =
			face->root.internal->incremental_interface->funcs->get_glyph_metrics(face->root.internal->
																				 incremental_interface->object,
...
TT_Access_Glyph_Frame(TT_Loader loader, FT_UInt32 glyph_index, FT_ULong offset, FT_ULong byte_count)
{
	FT_Error error;
	FT_Stream stream = loader->stream;
...
static FT_Error TT_Process_Simple_Glyph(TT_Loader loader)
{
	...
	if (!loader->face->is_default_instance)
	{
		...
		if (!(loader->face->variation_support & TT_FACE_FLAG_VAR_HADVANCE))
			loader->linear = (FT_Int) (outline->points[n_points - 3].x - outline->points[n_points - 4].x);
		if (!(loader->face->variation_support & TT_FACE_FLAG_VAR_VADVANCE))
			loader->vadvance = (FT_Int) (outline->points[n_points - 1].x - outline->points[n_points - 2].x);
...
/*                                                                       */
static FT_Error
TT_Process_Composite_Component(TT_Loader loader, FT_SubGlyph subglyph, FT_UInt start_point, FT_UInt num_base_points)
{
	FT_GlyphLoader gloader = loader->gloader;
	FT_Outline current;
	FT_Bool have_scale;
	...
	have_scale = (subglyph->flags & (WE_HAVE_A_SCALE | WE_HAVE_AN_XY_SCALE | WE_HAVE_A_2X2)) != 0;
	...
	} else
	{
...
#endif
		{
			...
			/*                                                                 */
			int a = subglyph->transform.xx > 0 ? subglyph->transform.xx : -subglyph->transform.xx;
			int b = subglyph->transform.yx > 0 ? subglyph->transform.yx : -subglyph->transform.yx;
			int c = subglyph->transform.xy > 0 ? subglyph->transform.xy : -subglyph->transform.xy;
			int d = subglyph->transform.yy > 0 ? subglyph->transform.yy : -subglyph->transform.yy;
			int m = a > b ? a : b;
			int n = c > d ? c : d;
...
/*    TT_Loader object.                                                  */
/*                                                                       */
static FT_Error load_truetype_glyph(TT_Loader loader, FT_UInt32 glyph_index, FT_UInt recurse_count, FT_Bool header_only)
{
	FT_Error error = FT_Err_Ok;
	...
	{
		error =
			face->root.internal->incremental_interface->funcs->get_glyph_data(face->root.internal->
																			  incremental_interface->object,
		...
		offset = tt_face_get_location(&face->root, glyph_index, &loader->byte_len);
	...
	if (loader->byte_len > 0)
	{
		...
		error = face->access_glyph_frame(loader, glyph_index, face->glyf_offset + offset, loader->byte_len);
		if (error)
			goto Exit;
	...
	if (loader->byte_len == 0 || loader->n_contours == 0)
	{
		...
		if (!loader->face->is_default_instance)
		{
			...
			/* if we don't have HVAR and VVAR, respectively        */
			if (!(loader->face->variation_support & TT_FACE_FLAG_VAR_HADVANCE))
				loader->linear = (FT_Int) (loader->pp2.x - loader->pp1.x);
			if (!(loader->face->variation_support & TT_FACE_FLAG_VAR_VADVANCE))
				loader->vadvance = (FT_Int) (loader->pp4.x - loader->pp3.x);
		}
#endif /* TT_CONFIG_OPTION_GX_VAR_SUPPORT */
	...
	else if (loader->n_contours < 0)
	{
		...
		{
			FT_TRACE1(("TT_Load_Composite_Glyph:" " infinite recursion detected\n"));
			error = FT_THROW(Invalid_Composite);
			goto Exit;
		}
		...
		if (!face->is_default_instance)
		{
			...
			if (FT_NEW_ARRAY(points, outline.n_points) ||
				FT_NEW_ARRAY(tags, outline.n_points) || FT_NEW_ARRAY(contours, outline.n_points))
				goto Exit1;
  ...
  Exit:
	...
	if (glyph_data_loaded)
		face->root.internal->incremental_interface->funcs->free_glyph_data(face->root.internal->incremental_interface->
																		   object, &glyph_data);
...
static FT_Error compute_glyph_metrics(TT_Loader loader, FT_UInt32 glyph_index)
{
	TT_Face face = loader->face;
	...
	if (
		   ...
		   IS_HINTED(loader->load_flags) && !(loader->load_flags & FT_LOAD_COMPUTE_METRICS))
	{
		FT_Byte *widthp;
		...
		if (driver->interpreter_version == TT_INTERPRETER_VERSION_38)
		{
			...
			ignore_x_mode = FT_LOAD_TARGET_MODE(loader->load_flags) != FT_RENDER_MODE_MONO;

			if (widthp &&
				((ignore_x_mode && loader->exec->compatible_widths) || !ignore_x_mode || SPH_OPTION_BITMAP_WIDTHS))
...
static FT_Error load_sbit_image(TT_Size size, TT_GlyphSlot glyph, FT_UInt32 glyph_index, FT_Int32 load_flags)
{
	TT_Face face;
	SFNT_Service sfnt;
	FT_Stream stream;
	...
	error = sfnt->load_sbit_image(face,
								  size->strike_index,
								  glyph_index, (FT_UInt) load_flags, stream, &glyph->bitmap, &sbit_metrics);
	if (!error)
	{
...
static FT_Error
tt_loader_init(TT_Loader loader, TT_Size size, TT_GlyphSlot glyph, FT_Int32 load_flags, FT_Bool glyf_table_only)
{
	TT_Face face;
	FT_Stream stream;
	...
	if (IS_HINTED(load_flags) && !glyf_table_only)
	{
		...
		if (driver->interpreter_version == TT_INTERPRETER_VERSION_40)
		{
			...
			exec->vertical_lcd_lean = subpixel_hinting_lean && (load_flags & FT_LOAD_TARGET_LCD_V);
		} else
		{
		...
		if (driver->interpreter_version == TT_INTERPRETER_VERSION_38)
		{
			subpixel_hinting = (FT_LOAD_TARGET_MODE(load_flags) != FT_RENDER_MODE_MONO) && SPH_OPTION_SET_SUBPIXEL;

			if (subpixel_hinting)
				grayscale = FALSE;
			else
#if SPH_OPTION_SET_GRAYSCALE
			{
				grayscale = TRUE;
...
#else /* 0 */
			exec->compatible_widths = FT_LOAD_TARGET_MODE(load_flags) != TT_LOAD_COMPATIBLE_WIDTHS;
			exec->symmetrical_smoothing = FT_LOAD_TARGET_MODE(load_flags) != TT_LOAD_SYMMETRICAL_SMOOTHING;
			exec->bgr = FT_LOAD_TARGET_MODE(load_flags) != TT_LOAD_BGR;
			exec->vertical_lcd = FT_LOAD_TARGET_MODE(load_flags) != TT_LOAD_VERTICAL_LCD;
			exec->subpixel_positioned = FT_LOAD_TARGET_MODE(load_flags) != TT_LOAD_SUBPIXEL_POSITIONED;
			exec->gray_cleartype = FT_LOAD_TARGET_MODE(load_flags) != TT_LOAD_GRAY_CLEARTYPE;
#endif /* 0 */
...
#ifdef TT_SUPPORT_SUBPIXEL_HINTING_MINIMAL
		if (driver->interpreter_version == TT_INTERPRETER_VERSION_40)
			grayscale = !subpixel_hinting_lean && FT_LOAD_TARGET_MODE(load_flags) != FT_RENDER_MODE_MONO;
		else
#endif
		...
		if (driver->interpreter_version == TT_INTERPRETER_VERSION_38)
		{
			...
			{
				FT_TRACE4(("tt_loader_init: grayscale hinting change," " re-executing `prep' table\n"));
...

		{
			...
			if (driver->interpreter_version == TT_INTERPRETER_VERSION_40)
			{
				...
				{
					FT_TRACE4(("tt_loader_init: grayscale subpixel hinting change," " re-executing `prep' table\n"));
...
{
	FT_List_Finalize(&loader->composites, (FT_List_Destructor) NULL, loader->face->root.memory, NULL);
}
...
/*                                                                       */
FT_LOCAL_DEF(FT_Error) TT_Load_Glyph(TT_Size size, TT_GlyphSlot glyph, FT_UInt32 glyph_index, FT_Int32 load_flags)
{
	FT_Error error;
	TT_LoaderRec loader;
		...
		} else
		{
			if (FT_IS_SCALABLE(glyph->face))
			{
				...
				if (!glyph->metrics.horiAdvance && glyph->linearHoriAdvance)
					glyph->metrics.horiAdvance = FT_MulFix(glyph->linearHoriAdvance, size->metrics->x_scale);
				if (!glyph->metrics.vertAdvance && glyph->linearVertAdvance)
					glyph->metrics.vertAdvance = FT_MulFix(glyph->linearVertAdvance, size->metrics->y_scale);
			}
