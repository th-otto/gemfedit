...
/*                                                                       */
static FT_Short *ft_var_readpackeddeltas(FT_Stream stream, FT_ULong size, FT_ULong delta_cnt)
{
...
static FT_Error ft_var_load_item_variation_store(TT_Face face, FT_ULong offset, GX_ItemVarStore itemStore)
{
	...
	for (i = 0; i < itemStore->dataCount; i++)
	{
		...
		if (FT_READ_USHORT(varData->itemCount) ||
			FT_READ_USHORT(shortDeltaCount) || FT_READ_USHORT(varData->regionIdxCount))
			goto Exit;
...
static FT_Error
ft_var_load_delta_set_index_mapping(TT_Face face, FT_ULong offset, GX_DeltaSetIdxMap map, GX_ItemVarStore itemStore)
{
	FT_Stream stream = FT_FACE_STREAM(face);
	FT_Memory memory = stream->memory;
...
static FT_Error ft_var_load_hvvar(TT_Face face, FT_Bool vertical)
{
	...
	if (widthMap_offset)
	{
		error = ft_var_load_delta_set_index_mapping(face,
													table_offset + widthMap_offset,
													&table->widthMap, &table->itemStore);
		if (error)
			goto Exit;
	}
		...
		for (j = 0; j < itemStore->axisCount; j++, axis++)
		{
			...
			else
			{
				if (face->blend->normalizedcoords[j] == axis->peakCoord)
					...
					axisScalar =
						FT_DivFix(axis->endCoord - face->blend->normalizedcoords[j], axis->endCoord - axis->peakCoord);
			}
...
static FT_Error tt_hvadvance_adjust(TT_Face face, FT_UInt gindex, FT_Int * avalue, FT_Bool vertical)
{
	...
	FT_TRACE5(("%s value %d adjusted by %d units (%s)\n",
			   vertical ? "vertical height" : "horizontal width", *avalue, delta, vertical ? "VVAR" : "HVAR"));
...
FT_LOCAL_DEF(FT_Error) tt_hadvance_adjust(FT_Face face_, FT_UInt gindex, FT_Int * avalue)
{
...
FT_LOCAL_DEF(FT_Error) tt_vadvance_adjust(FT_Face face_, FT_UInt gindex, FT_Int * avalue)
{
	TT_Face face = (TT_Face) face_;
...
static FT_Short *ft_var_get_value_pointer(TT_Face face, FT_ULong mvar_tag)
{
	...
	GX_GASP_CASE(0)
		...
		GX_VALUE_CASE(VDSC, vertical.Descender) GX_VALUE_CASE(VLGP, vertical.Line_Gap) GX_VALUE_CASE(XHGT, os2.sxHeight)
	else
	{
		/* ignore unknown tag */
		p = NULL;
	...
	for (; value < limit; value++)
	{
		...
		if (p)
			...
			FT_TRACE1(("ft_var_load_mvar: Ignoring unknown tag `%c%c%c%c'\n",
					   ...
					   (FT_Char) (value->tag >> 16), (FT_Char) (value->tag >> 8), (FT_Char) (value->tag)));
#endif
	}
...
/*    face :: The font face.                                             */
/*                                                                       */
FT_LOCAL_DEF(void) tt_apply_mvar(FT_Face face_)
{
	TT_Face face = (TT_Face) face_;
	GX_Blend blend = face->blend;
	GX_Value value, limit;
	...
	for (; value < limit; value++)
	{
		...
		if (p)
		{
			FT_TRACE5(("value %c%c%c%c (%d units) adjusted by %d units (MVAR)\n",
					   ...
					   (FT_Char) (value->tag >> 8), (FT_Char) (value->tag), value->unmodified, delta));
...
#define FT_STRUCTURE  GX_GVar_Head
	...
	{
		FT_TRACE1(("ft_var_load_gvar:" " invalid number of global coordinates\n"));
		error = FT_THROW(Invalid_Table);
		goto Exit;
	}
...
static FT_Fixed ft_var_apply_tuple(GX_Blend blend,
	FT_UShort tupleIndex, FT_Fixed * tuple_coords, FT_Fixed * im_start_coords, FT_Fixed * im_end_coords)
{
	FT_UInt i;
	...
	for (i = 0; i < blend->num_axis; i++)
	{
		...
		if (!(tupleIndex & GX_TI_INTERMEDIATE_TUPLE))
			FT_TRACE6(("      intermediate coordinates %d (%.5f, %.5f):\n",
					   i, im_start_coords[i] / 65536.0, im_end_coords[i] / 65536.0));
		...
		} else
		{
			...
			} else
			{
				FT_TRACE6(("      intermediate tuple range [%.5f;%.5f] fits\n",
						   im_start_coords[i] / 65536.0, im_end_coords[i] / 65536.0));
				apply = FT_MulDiv(apply,
								  im_end_coords[i] - blend->normalizedcoords[i], im_end_coords[i] - tuple_coords[i]);
			}
		}
	}
...
static void ft_var_to_normalized(TT_Face face, FT_UInt num_coords, FT_Fixed * coords, FT_Fixed * normalized)
{
	...
	{
		FT_TRACE2(("ft_var_to_normalized:" " only using first %d of %d coordinates\n", mmvar->num_axis, num_coords));
	...
	{
		FT_TRACE5(("normalized design coordinates" " before applying `avar' data:\n"));
				...
				if (normalized[i] < av->correspondence[j].fromCoord)
				{
					...
					normalized[i] =
						FT_MulDiv(normalized[i] - av->correspondence[j - 1].fromCoord,
								  ...
								  av->correspondence[j - 1].fromCoord) + av->correspondence[j - 1].toCoord;
					break;
				}
			}
		}
...
static void ft_var_to_design(TT_Face face, FT_UInt num_coords, FT_Fixed * coords, FT_Fixed * design)
{
	...
	{
		FT_TRACE2(("ft_var_to_design:" " only using first %d of %d coordinates\n", blend->num_axis, num_coords));
		nc = blend->num_axis;
	}
				...
				if (design[i] < av->correspondence[j].toCoord)
				{
					design[i] =
						FT_MulDiv(design[i] - av->correspondence[j - 1].toCoord,
								  ...
								  av->correspondence[j].toCoord -
								  av->correspondence[j - 1].toCoord) + av->correspondence[j - 1].fromCoord;
...
/*    FreeType error code.  0 means success.                             */
/*                                                                       */
FT_LOCAL_DEF(FT_Error) TT_Get_MM_Var(FT_Face face_, FT_MM_Var * *master)
{
	TT_Face face = (TT_Face) face_;
...
#define FT_STRUCTURE  GX_FVar_Axis
	...
	if (!face->blend)
	{
		...
		usePsName = fvar_head.instanceSize == 6 + 4 * fvar_head.axisCount;
		...
		face->blend->mmvar_len =
			...
			num_instances * fvar_head.axisCount * sizeof(FT_Fixed) + fvar_head.axisCount * 5;
		...
		axis_flags = (FT_UShort *) & (mmvar[1]);
		mmvar->axis = (FT_Var_Axis *) & (axis_flags[fvar_head.axisCount]);
		mmvar->namedstyle = (FT_Var_Named_Style *) & (mmvar->axis[fvar_head.axisCount]);

		next_coords = (FT_Fixed *) & (mmvar->namedstyle[num_instances]);
		...
		for (i = 0; i < fvar_head.axisCount; i++)
		{
			...
			*axis_flags = axis_rec.flags;
			...
			{
				FT_TRACE2(("TT_Get_MM_Var:" " invalid \"%s\" axis record; disabling\n", a->name));
			...
			FT_TRACE5(("  \"%s\":"
					   ...
					   " flags=0x%04X\n",
					   a->name, a->minimum / 65536.0, a->def / 65536.0, a->maximum / 65536.0, *axis_flags));
		...
		if (FT_NEW_ARRAY(face->blend->normalized_stylecoords, fvar_head.axisCount * num_instances))
			goto Exit;
		...
		for (i = 0; i < fvar_head.instanceCount; i++, ns++)
		{
			...
			if (FT_FRAME_ENTER((usePsName ? 6L : 4L) + 4L * fvar_head.axisCount))
				goto Exit;
			...
			ft_var_to_normalized(face, fvar_head.axisCount, ns->coords, nsc);
	...
	if (master)
	{
		...
		axis_flags = (FT_UShort *) & (mmvar[1]);
		mmvar->axis = (FT_Var_Axis *) & (axis_flags[mmvar->num_axis]);
...
static FT_Error tt_set_mm_blend(TT_Face face, FT_UInt num_coords, FT_Fixed * coords, FT_Bool set_design_coords)
{
	...
	if (set_design_coords)
		ft_var_to_design(face,
						 all_design_coords ? blend->num_axis : num_coords, blend->normalizedcoords, blend->coords);
...
/*    FreeType error code.  0 means success.                             */
/*                                                                       */
FT_LOCAL_DEF(FT_Error) TT_Set_MM_Blend(FT_Face face_, FT_UInt num_coords, FT_Fixed * coords)
{
	TT_Face face = (TT_Face) face_;
...
/*    FreeType error code.  0 means success.                             */
/*                                                                       */
FT_LOCAL_DEF(FT_Error) TT_Get_MM_Blend(FT_Face face_, FT_UInt num_coords, FT_Fixed * coords)
{
	TT_Face face = (TT_Face) face_;
	FT_Error error = FT_Err_Ok;
	GX_Blend blend;
...
/*    FreeType error code.  0 means success.                             */
/*                                                                       */
FT_LOCAL_DEF(FT_Error) TT_Set_Var_Design(FT_Face face_, FT_UInt num_coords, FT_Fixed * coords)
{
	TT_Face face = (TT_Face) face_;
	FT_Error error = FT_Err_Ok;
	GX_Blend blend;
	...
	if (num_coords > mmvar->num_axis)
	{
		FT_TRACE2(("TT_Set_Var_Design:" " only using first %d of %d coordinates\n", mmvar->num_axis, num_coords));
		num_coords = mmvar->num_axis;
	}
	...
	FT_MEM_COPY(blend->coords, coords, num_coords * sizeof(FT_Fixed));
...
/*    FreeType error code.  0~means success.                             */
/*                                                                       */
FT_LOCAL_DEF(FT_Error) TT_Get_Var_Design(FT_Face face_, FT_UInt num_coords, FT_Fixed * coords)
{
	TT_Face face = (TT_Face) face_;
	FT_Error error = FT_Err_Ok;
	GX_Blend blend;
...
/*                                                                       */
FT_LOCAL_DEF(FT_Error) tt_face_vary_cvt(TT_Face face, FT_Stream stream)
{
	FT_Error error;
	FT_Memory memory = stream->memory;
	...
	if (FT_NEW_ARRAY(tuple_coords, blend->num_axis) ||
		FT_NEW_ARRAY(im_start_coords, blend->num_axis) || FT_NEW_ARRAY(im_end_coords, blend->num_axis))
		goto FExit;
	...
	{
		FT_TRACE2(("tt_face_vary_cvt:" " invalid CVT variation array header\n"));
	...
	for (i = 0; i < (tupleCount & 0xFFF); i++)
	{
		...
		else if (localpoints == ALL_POINTS)
		{
			...
			for (j = 0; j < face->cvt_size; j++)
			{
				...
				face->cvt[j] = (FT_Short) (orig_cvt + FT_MulFix(deltas[j], apply));
		...
		else
		{
			...
			for (j = 0; j < point_count; j++)
			{
				...
				face->cvt[pindex] = (FT_Short) (orig_cvt + FT_MulFix(deltas[j], apply));
...
static void
tt_interpolate_deltas(FT_Outline * outline, FT_Vector * out_points, FT_Vector * in_points, FT_Bool * has_delta)
{
	FT_Int first_point;
	FT_Int end_point;
...
TT_Vary_Apply_Glyph_Deltas(TT_Face face, FT_UInt32 glyph_index, FT_Outline * outline, FT_UInt n_points)
{
	FT_Stream stream = face->root.stream;
	FT_Memory memory = stream->memory;
	GX_Blend blend = face->blend;
	...
	{
		FT_TRACE2(("TT_Vary_Apply_Glyph_Deltas:" " no variation data for this glyph\n"));
		return FT_Err_Ok;
	}
	...
	if (FT_STREAM_SEEK(blend->glyphoffsets[glyph_index]) ||
		FT_FRAME_ENTER(blend->glyphoffsets[glyph_index + 1] - blend->glyphoffsets[glyph_index]))
		goto Fail1;
	...
	/* rough sanity test */
	if (offsetToData + (tupleCount & GX_TC_TUPLE_COUNT_MASK) * 4 > blend->gvar_size)
	{
		FT_TRACE2(("TT_Vary_Apply_Glyph_Deltas:" " invalid glyph variation array header\n"));
	...
	for (i = 0; i < (tupleCount & GX_TC_TUPLE_COUNT_MASK); i++)
	{
		...
		{
			FT_TRACE2(("TT_Vary_Apply_Glyph_Deltas:" " invalid tuple index\n"));
		...
		else if (points == ALL_POINTS)
		{
			...
			for (j = 0; j < n_points; j++)
			{
				...
				} else
				{
					...
					if (j == (n_points - 3) && !(face->variation_support & TT_FACE_FLAG_VAR_HADVANCE))
						outline->points[j].x += delta_x;

					else if (j == (n_points - 2) && !(face->variation_support & TT_FACE_FLAG_VAR_LSB))
						outline->points[j].x += delta_x;

					else if (j == (n_points - 1) && !(face->variation_support & TT_FACE_FLAG_VAR_VADVANCE))
						outline->points[j].y += delta_y;

					else if (j == (n_points - 0) && !(face->variation_support & TT_FACE_FLAG_VAR_TSB))
						outline->points[j].y += delta_y;
				}
				...
				if (delta_x || delta_y)
				{
					FT_TRACE7(("      %d: (%d, %d) -> (%d, %d)\n",
							   j,
							   outline->points[j].x - delta_x,
							   outline->points[j].y - delta_y, outline->points[j].x, outline->points[j].y));
					count++;
				}
#endif
			}
...
/*                                                                       */
FT_LOCAL_DEF(FT_Error) tt_get_var_blend(FT_Face face_,
	FT_UInt * num_coords, FT_Fixed * *coords, FT_Fixed * *normalizedcoords, FT_MM_Var * *mm_var)
{
	TT_Face face = (TT_Face) face_;
...
/*                                                                       */
FT_LOCAL_DEF(void) tt_done_blend(FT_Face face_)
{
	TT_Face face = (TT_Face) face_;
	FT_Memory memory = FT_FACE_MEMORY(face);
	GX_Blend blend = face->blend;
