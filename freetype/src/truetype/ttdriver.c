...
static FT_Error tt_property_get(FT_Module module, const char *property_name, void *value)
{
	FT_Error error = FT_Err_Ok;
	TT_Driver driver = (TT_Driver) module;
...
static FT_Error tt_get_advances(FT_Face ttface, FT_UInt32 start, FT_UInt32 count, FT_Int32 flags, FT_Fixed * advances)
{
	FT_UInt nn;
	TT_Face face = (TT_Face) ttface;
	...
	if (flags & FT_LOAD_VERTICAL_LAYOUT)
	{
		...
		if (!face->is_default_instance && !(face->variation_support & TT_FACE_FLAG_VAR_VADVANCE))
	...
	} else
	{
		...
		if (!face->is_default_instance && !(face->variation_support & TT_FACE_FLAG_VAR_HADVANCE))
			return FT_THROW(Unimplemented_Feature);
#endif
...
/*    FreeType error code.  0 means success.                             */
/*                                                                       */
static FT_Error tt_glyph_load(FT_GlyphSlot ttslot, FT_Size ttsize, FT_UInt32 glyph_index, FT_Int32 load_flags)
{
	...
	if (glyph_index >= (FT_UInt32) face->num_glyphs && !face->internal->incremental_interface)
#else
...
FT_CALLBACK_TABLE_DEF const FT_Driver_ClassRec tt_driver_class = {
	{
