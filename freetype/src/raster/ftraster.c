...
static void Vertical_Sweep_Span(RAS_ARGS Short y, FT_F26Dot6 x1, FT_F26Dot6 x2, PProfile left, PProfile right)
{
	...
	FT_TRACE7(("  y=%d x=[%.12f;%.12f], drop-out=%d",
			   y, x1 / (double) ras.precision, x2 / (double) ras.precision, dropOutControl));
	...
	if (e1 > e2)
	{
		...
		if (e1 == e2 + ras.precision)
		{
			...
			case 5:					/* smart drop-outs excluding stubs  */
				...
				/* upper stub test */
				if (left->next == right &&
					left->height <= 0 && !(left->flags & Overshoot_Top && x2 - x1 >= ras.precision_half))
					goto Exit;

				/* lower stub test */
				if (right->next == left &&
					left->start == y && !(left->flags & Overshoot_Bottom && x2 - x1 >= ras.precision_half))
					goto Exit;
...
static int ft_black_new(FT_Memory memory, FT_Raster * araster_)
{
...
static void ft_black_done(FT_Raster raster_)
{
...
static int ft_black_render(FT_Raster raster, const FT_Raster_Params * params)
{
	...
	/* reject too large outline coordinates */
	{
		...
		for (; vec < limit; vec++)
		{
			if (vec->x < -0x1000000L || vec->x > 0x1000000L || vec->y < -0x1000000L || vec->y > 0x1000000L)
				return FT_THROW(Invalid);
		}
	}
