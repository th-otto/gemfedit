...
static FT_Error ft_raster1_init(FT_Module render_)
{
...
static FT_Error
ft_raster1_transform(FT_Renderer render, FT_GlyphSlot slot, const FT_Matrix * matrix, const FT_Vector * delta)
{
...
static FT_Error ft_raster1_render(FT_Renderer render, FT_GlyphSlot slot, FT_Render_Mode mode, const FT_Vector * origin)
{
	...
	if ((FT_ULong) width > FT_USHORT_MAX || (FT_ULong) height > FT_USHORT_MAX)
	{
		error = FT_THROW(Invalid_Argument);
		goto Exit;
	}
