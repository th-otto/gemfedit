...
FT_LOCAL_DEF(FT_Error) T42_Face_Init(FT_Stream stream, FT_Face t42face, FT_Int face_index, FT_Int num_params, FT_Parameter * params)
{
...
FT_LOCAL_DEF(FT_Error) T42_Size_Request(FT_Size t42size, FT_Size_Request req)
{
...
FT_LOCAL_DEF(FT_Error) T42_Size_Select(FT_Size t42size, FT_ULong strike_index)
{
	T42_Size size = (T42_Size) t42size;
	T42_Face face = (T42_Face) t42size->face;
	FT_Error error;
...
FT_LOCAL_DEF(FT_Error) T42_GlyphSlot_Load(FT_GlyphSlot glyph, FT_Size size, FT_UInt32 glyph_index, FT_Int32 load_flags)
{
	FT_Error error;
	...
	glyph_index = (FT_UInt32) ft_strtol((const char *) t42face->type1.charstrings[glyph_index], NULL, 10);
