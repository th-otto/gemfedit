...
{
	return hint1->org_pos + hint1->org_len >= hint2->org_pos && hint2->org_pos + hint2->org_len >= hint1->org_pos;
}
...
static FT_Error psh_hint_table_init(PSH_Hint_Table table,
	PS_Hint_Table hints, PS_Mask_Table hint_masks, PS_Mask_Table counter_masks, FT_Memory memory)
{
	FT_UInt count;
	FT_Error error;
	...
	if (FT_NEW_ARRAY(table->sort, 2 * count) ||
		FT_NEW_ARRAY(table->hints, count) || FT_NEW_ARRAY(table->zones, 2 * count + 1))
		goto Exit;
...
static void psh_hint_align(PSH_Hint hint, PSH_Globals globals, FT_Int dimension, PSH_Glyph glyph)
{
	...
	if (!psh_hint_is_fitted(hint))
	{
		...
		if (dimension == 1)
			psh_blues_snap_stem(&globals->blues, hint->org_pos + hint->org_len, hint->org_pos, &align);
  ...
  *  of stems
  */
static void psh_hint_align_light(PSH_Hint hint, PSH_Globals globals, FT_Int dimension, PSH_Glyph glyph)
{
	PSH_Dimension dim = &globals->dimension[dimension];
	FT_Fixed scale = dim->scale_mult;
	FT_Fixed delta = dim->scale_delta;
		...
		default:
			{
				...
				else					/* len > 64 */
				{
					...
					{
						FT_Fixed side_delta = psh_hint_snap_stem_side_delta(pos, len);
...
static void psh_hint_table_find_strong_points(PSH_Hint_Table table,
	PSH_Point point, FT_UInt count, FT_Int threshold, FT_Int major_dir)
{
	PSH_Hint *sort = table->sort;
		...
		else if (psh_point_is_extremum(point))
		{
			...
			if (!point->hint)
			{
				for (nn = 0; nn < num_hints; nn++)
				{
					...
					if (org_u >= hint->org_pos && org_u <= hint->org_pos + hint->org_len)
					{
						point->hint = hint;
						break;
					}
			...
			} else
			{
				...
				} else
				{
					...
					if (u == before->org_u)
						...
						point->cur_u = before->cur_u +
							FT_MulDiv(u - before->org_u, after->cur_u - before->cur_u, after->org_u - before->org_u);
				}
			}
			psh_point_set_fitted(point);
		}
...
FT_Error ps_hints_apply(PS_Hints ps_hints, FT_Outline * outline, PSH_Globals globals, FT_Render_Mode hint_mode)
{
	 ...
	 */
	{
		...
		glyph->do_horz_snapping = hint_mode == FT_RENDER_MODE_MONO || hint_mode == FT_RENDER_MODE_LCD;

		glyph->do_vert_snapping = hint_mode == FT_RENDER_MODE_MONO || hint_mode == FT_RENDER_MODE_LCD_V;
