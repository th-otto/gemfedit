...
static FT_Error _ft_face_scale_advances(FT_Face face, FT_Fixed * advances, FT_UInt32 count, FT_Int32 flags)
{
	FT_Fixed scale;
	FT_UInt nn;
...
FT_EXPORT_DEF(FT_Error) FT_Get_Advance(FT_Face face, FT_UInt32 gindex, FT_Int32 flags, FT_Fixed * padvance)
{
...
FT_EXPORT_DEF(FT_Error) FT_Get_Advances(FT_Face face, FT_UInt32 start, FT_UInt32 count, FT_Int32 flags, FT_Fixed * padvances)
{
	FT_Face_GetAdvancesFunc func;
	...
	for (nn = 0; nn < count; nn++)
	{
		...
		padvances[nn] = (flags & FT_LOAD_VERTICAL_LAYOUT)
			? face->glyph->advance.y * 1024 : face->glyph->advance.x * 1024;
	}
