...
FT_BASE_DEF(FT_Error) FT_Raccess_Get_HeaderInfo(FT_Library library,
	FT_Stream stream, FT_Long rfork_offset, FT_Long * map_offset, FT_Long * rdata_pos)
{
	FT_Error error;
	unsigned char head[16];
	unsigned char head2[16];
	FT_Long map_pos;
	FT_Long map_len;
	FT_Long rdata_len;
	int allzeros;
...
FT_BASE_DEF(FT_Error) FT_Raccess_Get_DataOffsets(FT_Library library,
	FT_Stream stream,
	FT_Long map_offset,
	FT_Long rdata_pos, FT_Long tag, FT_Bool sort_by_res_id, FT_Long ** offsets, FT_Long * count)
{
	FT_Error error;
	int i, j, cnt, subcnt;
	...
	for (i = 0; i < cnt; i++)
	{
		...
		FT_TRACE2(("Resource tags: %c%c%c%c\n",
				   ...
				   (char) (0xFF & (tag_internal >> 8)), (char) (0xFF & (tag_internal >> 0))));
		...
		if (tag_internal == tag)
		{
			*count = subcnt + 1;
			...
			if (sort_by_res_id)
			{
				ft_qsort(ref,
						 (size_t) * count,
						 sizeof(FT_RFork_Ref), (int (*)(const void *, const void *)) ft_raccess_sort_ref_by_id);
...
static FT_Error raccess_guess_apple_generic(FT_Library library,
							FT_Stream stream, char *base_file_name, FT_Int32 magic, FT_Long * result_offset);
...
FT_BASE_DEF(void) FT_Raccess_Guess(FT_Library library,
	FT_Stream stream, char *base_name, char **new_names, FT_Long * offsets, FT_Error * errors)
{
	FT_Int i;
 ...
 */
FT_LOCAL_DEF(FT_Bool) ft_raccess_rule_by_darwin_vfs(FT_Library library, FT_UInt rule_index)
{
