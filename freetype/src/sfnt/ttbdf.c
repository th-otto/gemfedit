...
FT_LOCAL_DEF(FT_Error) tt_face_find_bdf_prop(FT_Face face_, const char *property_name, BDF_PropertyRec * aprop)
{
		...
		if ((type & 0x10) != 0)
		{
			...
			if (name_offset < bdf->strings_size &&
				...
				ft_strncmp(property_name,
						   (const char *) bdf->strings + name_offset, bdf->strings_size - name_offset) == 0)
			{
				switch (type & 0x0F)
				{
				case 0x00:				/* string */
