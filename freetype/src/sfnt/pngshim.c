...
ANONYMOUS_STRUCT_DUMMY(png_struct_def)
ANONYMOUS_STRUCT_DUMMY(png_info_def)
...
static void read_data_from_FT_Stream(png_structp png, png_bytep data, png_size_t length)
{
	...
	if (FT_FRAME_ENTER((FT_ULong) length))
	{
	...
	FT_Bool metrics_only)
{
	...
	if (error ||
		(!populate_map_and_metrics &&
		 ((FT_Int) imgWidth != metrics->width || (FT_Int) imgHeight != metrics->height)))
		goto DestroyExit;
