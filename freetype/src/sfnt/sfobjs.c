...
FT_LOCAL_DEF(FT_Error) tt_face_get_name(TT_Face face, FT_UShort nameid, FT_String ** name)
{
	...
	if (rec && convert)
	{
		if (!rec->string)
		{
			...
			if (FT_QNEW_ARRAY(rec->string, rec->stringLength) ||
				FT_STREAM_SEEK(rec->stringOffset) || FT_STREAM_READ(rec->string, rec->stringLength))
			{
				FT_FREE(rec->string);
				rec->stringLength = 0;
				result = NULL;
...
FT_CALLBACK_DEF(int) compare_offsets(const void *a, const void *b)
{
	...
	for (nn = 0; nn < woff.num_tables; nn++)
	{
		...
		} else
		{
			...
			error = FT_Gzip_Uncompress(memory,
									   sfnt + table->OrigOffset, &output_len, stream->cursor, table->CompLength);
#else
...
#define FT_STRUCTURE  TTC_HeaderRec
  ...
  retry:
	...
	if (tag != 0x00010000UL &&
		tag != TTAG_ttcf &&
		tag != TTAG_OTTO &&
		tag != TTAG_true && tag != TTAG_typ1 && tag != TTAG_0xA5kbd && tag != TTAG_0xA5lst && tag != 0x00020000UL)
	{
...
sfnt_init_face(FT_Stream stream, TT_Face face, FT_Int face_instance_index, FT_Int num_params, FT_Parameter * params)
{
...
#ifdef TT_CONFIG_OPTION_GX_VAR_SUPPORT
	{
		...
		if (face->goto_table(face, TTAG_fvar, stream, &fvar_len) ||
			fvar_len < 20 || FT_READ_ULONG(version) || FT_READ_USHORT(offset) || FT_STREAM_SKIP(2) /* reserved */  ||
			FT_READ_USHORT(num_axes) ||
			FT_READ_USHORT(axis_size) || FT_READ_USHORT(num_instances) || FT_READ_USHORT(instance_size))
		{
			version = 0;
			offset = 0;
			num_axes = 0;
		...
		if (face->goto_table(face, TTAG_glyf, stream, 0) &&
			face->goto_table(face, TTAG_CFF2, stream, 0) && !face->goto_table(face, TTAG_CFF, stream, 0))
			num_instances = 0;
...
#ifdef TT_CONFIG_OPTION_POSTSCRIPT_NAMES
		if (psnames_error == FT_Err_Ok && face->postscript.FormatType != 0x00030000L)
