...
FT_LOCAL_DEF(FT_Error) tt_face_load_sbit(TT_Face face, FT_Stream stream)
{
	FT_Error error;
	FT_ULong table_size;
...
FT_LOCAL_DEF(FT_Error) tt_face_set_sbit_strike(TT_Face face, FT_Size_Request req, FT_ULong * astrike_index)
{
	return FT_Match_Size((FT_Face) face, req, 0, astrike_index);
}


FT_LOCAL_DEF(FT_Error) tt_face_load_strike_metrics(TT_Face face, FT_ULong strike_index, FT_Size_Metrics * metrics)
{
	...
	case TT_SBIT_TABLE_TYPE_CBLC:
		{
...
#if 0
			else;						/* if we have a negative descender, simply use it */
#endif
			...
			metrics->x_scale = FT_MulDiv(metrics->x_ppem, 64 * 0x10000L, face->header.Units_Per_EM);
			metrics->y_scale = FT_MulDiv(metrics->y_ppem, 64 * 0x10000L, face->header.Units_Per_EM);
...
typedef FT_Error(*TT_SBitDecoder_LoadFunc) (TT_SBitDecoder decoder,
											FT_Byte * p,
											FT_Byte * plimit, FT_Int x_pos, FT_Int y_pos, FT_UInt recurse_count);


static FT_Error tt_sbit_decoder_load_byte_aligned(TT_SBitDecoder decoder,
	FT_Byte * p, FT_Byte * limit, FT_Int x_pos, FT_Int y_pos, FT_UInt recurse_count)
{
	FT_Error error = FT_Err_Ok;
	FT_Byte *line;
	FT_ULong pitch;
	FT_Int width, height, line_bits, h;
	...
	{
		FT_TRACE1(("tt_sbit_decoder_load_byte_aligned:" " invalid bitmap dimensions\n"));
		error = FT_THROW(Invalid_File_Format);
		goto Exit;
	}
	...
	FT_Byte * p, FT_Byte * limit, FT_Int x_pos, FT_Int y_pos, FT_UInt recurse_count)
{
	...
	FT_Byte *line;
	FT_ULong pitch;
	FT_Int width, height, line_bits, h, nbits;
	FT_UInt bit_height, bit_width;
	FT_Bitmap *bitmap;
	FT_UShort rval;
	...
	if (x_pos < 0 || (FT_UInt) (x_pos + width) > bit_width || y_pos < 0 || (FT_UInt) (y_pos + height) > bit_height)
	{
		FT_TRACE1(("tt_sbit_decoder_load_bit_aligned:" " invalid bitmap dimensions\n"));
		error = FT_THROW(Invalid_File_Format);
		goto Exit;
	}
...
static FT_Error tt_sbit_decoder_load_png(TT_SBitDecoder decoder,
	FT_Byte * p, FT_Byte * limit, FT_Int x_pos, FT_Int y_pos, FT_UInt recurse_count)
{
	FT_Error error = FT_Err_Ok;
	FT_ULong png_len;
	...
	error = Load_SBit_Png(decoder->face->root.glyph,
						  ...
						  y_pos,
						  decoder->bit_depth,
						  decoder->metrics, decoder->stream->memory, p, (FT_UInt) png_len, FALSE, FALSE);
	...
	FT_Int x_pos, FT_Int y_pos, FT_UInt recurse_count, FT_Bool metrics_only)
{
	...
	FT_ULong image_end = 0;
	FT_ULong image_offset;

	/* arbitrary recursion limit */
	if (recurse_count > 100)
	{
		FT_TRACE4(("tt_sbit_decoder_load_image:" " recursion depth exceeded\n"));
		goto Failure;
	}
  ...
  FoundRange:
	...
	return tt_sbit_decoder_load_bitmap(decoder,
									   image_format, image_start, image_end, x_pos, y_pos, recurse_count, metrics_only);
  ...
  NoBitmap:
	if (recurse_count)
	{
		FT_TRACE4(("tt_sbit_decoder_load_image:" " missing subglyph sbit with glyph index %d\n", glyph_index));
		return FT_THROW(Invalid_Composite);
	}

	FT_TRACE4(("tt_sbit_decoder_load_image:" " no sbit found for glyph index %d\n", glyph_index));
	return FT_THROW(Missing_Bitmap);
}


static FT_Error tt_face_load_sbix_image(TT_Face face,
	FT_ULong strike_index,
	FT_UInt32 glyph_index,
	FT_Stream stream, FT_Bitmap * map, TT_SBit_MetricsRec * metrics, FT_Bool metrics_only)
{
  ...
  retry:
	...
	} else if (graphicType == FT_MAKE_TAG('p', 'n', 'g', ' '))
	{
		...
		error = Load_SBit_Png(face->root.glyph,
							  ...
							  0,
							  32,
							  metrics, stream->memory, stream->cursor, glyph_end - glyph_start - 8, TRUE, metrics_only);
#else
		error = FT_THROW(Unimplemented_Feature);
#endif
	} else if (graphicType == FT_MAKE_TAG('j', 'p', 'g', ' ') || graphicType == FT_MAKE_TAG('t', 'i', 'f', 'f') || graphicType == FT_MAKE_TAG('r', 'g', 'b', 'l'))	/* used on iOS 7.1 */
	{
		error = FT_THROW(Unknown_File_Format);
	} else
	{
...
FT_LOCAL(FT_Error) tt_face_load_sbit_image(TT_Face face,
	FT_ULong strike_index,
	FT_UInt32 glyph_index,
	FT_UInt load_flags, FT_Stream stream, FT_Bitmap * map, TT_SBit_MetricsRec * metrics)
{
	FT_Error error = FT_Err_Ok;
	...
	case TT_SBIT_TABLE_TYPE_CBLC:
		{
			...
			if (!error)
			{
				error = tt_sbit_decoder_load_image(decoder,
												   glyph_index,
												   0, 0, 0, (load_flags & FT_LOAD_BITMAP_METRICS_ONLY) != 0);
	...
	case TT_SBIT_TABLE_TYPE_SBIX:
		error = tt_face_load_sbix_image(face,
										strike_index,
										glyph_index,
										stream, map, metrics, (load_flags & FT_LOAD_BITMAP_METRICS_ONLY) != 0);
		break;
