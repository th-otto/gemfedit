...
FT_LOCAL_DEF(void) af_loader_init(AF_Loader loader, AF_GlyphHints hints)
{
	FT_ZERO(loader);
...
FT_LOCAL_DEF(FT_Error) af_loader_reset(AF_Loader loader, AF_Module module, FT_Face face)
{
...
FT_LOCAL_DEF(FT_Error) af_loader_load_glyph(AF_Loader loader, AF_Module module, FT_Face face, FT_UInt32 glyph_index, FT_Int32 load_flags)
{
	FT_Error error;
	...
	case FT_GLYPH_FORMAT_OUTLINE:
		...
		if (writing_system_class->style_hints_apply)
			writing_system_class->style_hints_apply(glyph_index, hints, &gloader->base.outline, style_metrics);
  ...
  Hint_Metrics:
	{
		...
		if (scaler.render_mode != FT_RENDER_MODE_LIGHT &&
			(FT_IS_FIXED_WIDTH(slot->face) ||
			 (af_face_globals_is_digit(loader->globals, glyph_index) && style_metrics->digits_have_same_width)))
		{
 ...
 */
FT_LOCAL_DEF(FT_Int32) af_loader_compute_darkening(AF_Loader loader, FT_Face face, FT_Pos standard_width)
{
	...
	else if (scaled_stem < af_intToFixed(x2))
	{
		FT_Int xdelta = x2 - x1;
		FT_Int ydelta = y2 - y1;
		FT_Fixed x = stem_width_per_1000 - FT_DivFix(af_intToFixed(x1), ppem);
	  ...
	  Try_x3:
		{
			FT_Int xdelta = x3 - x2;
			FT_Int ydelta = y3 - y2;
			FT_Fixed x = stem_width_per_1000 - FT_DivFix(af_intToFixed(x2), ppem);
	  ...
	  Try_x4:
		{
			FT_Int xdelta = x4 - x3;
			FT_Int ydelta = y4 - y3;
			FT_Fixed x = stem_width_per_1000 - FT_DivFix(af_intToFixed(x3), ppem);
