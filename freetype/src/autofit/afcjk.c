...
FT_LOCAL_DEF(void) af_cjk_metrics_init_widths(AF_CJKMetrics metrics, FT_Face face)
{
		...
		if (*blue_shoot != *blue_ref)
		{
			...
			if (AF_CJK_IS_TOP_BLUE(bs) ^ under_ref)
			{
				*blue_ref = *blue_shoot = (shoot + ref) / 2;
				...
				FT_TRACE5(("  [reference smaller than overshoot," " taking mean value]\n"));
			}
		}
...
FT_LOCAL_DEF(FT_Error) af_cjk_metrics_init(AF_StyleMetrics metrics_, FT_Face face)
{
...
FT_LOCAL_DEF(void) af_cjk_metrics_scale(AF_StyleMetrics metrics_, AF_Scaler scaler)
{
	AF_CJKMetrics metrics = (AF_CJKMetrics)metrics_;
...
FT_LOCAL_DEF(void) af_cjk_get_standard_widths(AF_StyleMetrics metrics_, FT_Pos * stdHW, FT_Pos * stdVW)
{
	AF_CJKMetrics metrics = (AF_CJKMetrics)metrics_;
	if (stdHW)
...
FT_LOCAL_DEF(FT_Error) af_cjk_hints_init(AF_GlyphHints hints, AF_StyleMetrics metrics_)
{
	AF_CJKMetrics metrics = (AF_CJKMetrics)metrics_;
	FT_Render_Mode mode;
...
static FT_Pos
af_cjk_compute_stem_width(AF_GlyphHints hints, AF_Dimension dim, FT_Pos width, FT_UInt base_flags, FT_UInt stem_flags)
{
...
static void af_cjk_align_linked_edge(AF_GlyphHints hints, AF_Dimension dim, AF_Edge base_edge, AF_Edge stem_edge)
{
	...
	FT_TRACE5(("  CJKLINK: edge %d @%d (opos=%.2f) linked to %.2f,"
			   ...
			   stem_edge->opos / 64.0, stem_edge->pos / 64.0, dist / 64.0, fitted_width / 64.0));
}
	...
	for (edge = edges; edge < edge_limit; edge++)
	{
		...
		if (dim != AF_DIMENSION_VERT && !anchor)
		{
			...
			if (fixedpitch)
			{
				...
				delta1 = delta;
				delta1 += af_hint_normal_stem(hints, left, left->link, delta1, 0);
		...
		if (before >= edges || after < edge_limit)
		{
			...
			else
			{
				if (after->fpos == before->fpos)
					...
					edge->pos = before->pos +
						FT_MulDiv(edge->fpos - before->fpos, after->pos - before->pos, after->fpos - before->fpos);
			}
		}
	}
...
FT_LOCAL_DEF(FT_Error) af_cjk_hints_apply(FT_UInt32 glyph_index, AF_GlyphHints hints, FT_Outline *outline, AF_StyleMetrics metrics_)
{
	AF_CJKMetrics metrics = (AF_CJKMetrics)metrics_;
	FT_Error error;
	int dim;
