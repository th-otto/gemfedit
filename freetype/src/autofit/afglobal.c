...
FT_LOCAL_ARRAY_DEF(char *) af_style_names[] =
{
	...
	for (ss = 0; AF_STYLE_CLASSES_GET[ss]; ss++)
	{
		...
		if (style_class->coverage == AF_COVERAGE_DEFAULT)
		{
			...
			for (range = script_class->script_uni_nonbase_ranges; range->first != 0; range++)
			{
				...
				if (gindex != 0 &&
					gindex < (FT_ULong) globals->glyph_count && (gstyles[gindex] & AF_STYLE_MASK) == (FT_UShort) ss)
...
FT_LOCAL_DEF(FT_Error) af_face_globals_new(FT_Face face, AF_FaceGlobals * aglobals, AF_Module module)
{
	FT_Error error;
	FT_Memory memory;
...
FT_LOCAL_DEF(FT_Error) af_face_globals_get_metrics(AF_FaceGlobals globals, FT_UInt32 gindex, FT_UInt options, AF_StyleMetrics * ametrics)
{
...
FT_LOCAL_DEF(FT_Bool) af_face_globals_is_digit(AF_FaceGlobals globals, FT_UInt32 gindex)
{
