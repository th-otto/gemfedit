...
FT_LOCAL_DEF(void) af_latin_metrics_init_widths(AF_LatinMetrics metrics, FT_Face face)
{
			...
			for (i = 0; i < num_idx; i++)
			{
				...
				if (best_point >= 0)
				{
					...
					if (num_idx == 1)
						...
						FT_TRACE5(("  component %d of cluster starting with U+%04lX:" " best_y = %5ld", i, ch, best_y));
#endif
		...
		if (*blue_shoot != *blue_ref)
		{
			...
			if ((AF_LATIN_IS_TOP_BLUE(bs) || AF_LATIN_IS_SUB_TOP_BLUE(bs)) ^ over_ref)
			{
				*blue_ref = *blue_shoot = (shoot + ref) / 2;
				...
				FT_TRACE5(("  [overshoot smaller than reference," " taking mean value]\n"));
			}
		}
...
FT_LOCAL_DEF(void) af_latin_metrics_check_digits(AF_LatinMetrics metrics, FT_Face face)
{
	FT_Bool started = 0;
	FT_Bool same_width = 1;
	FT_Fixed advance = 0;
	FT_Fixed old_advance = 0;
...
FT_LOCAL_DEF(FT_Error) af_latin_metrics_init(AF_StyleMetrics metrics_, FT_Face face)
{
				...
				if (dim == AF_DIMENSION_VERT)
				{
					...
					if (dist == 0)
					{
						FT_TRACE5(("af_latin_metrics_scale_dim:"
								   ...
								   "\n",
								   af_style_names[metrics->root.style_class->style],
								   scale / 65536.0, new_scale / 65536.0, (fitted - scaled) * 100 / scaled));
	...
	if (dim == AF_DIMENSION_VERT)
	{
		...
		for (nn = 0; nn < axis->blue_count; nn++)
		{
			...
			FT_TRACE5(("  reference %d: %d scaled to %.2f%s\n"
					   ...
					   nn,
					   blue->shoot.org,
					   blue->shoot.fit / 64.0, blue->flags & AF_LATIN_BLUE_ACTIVE ? "" : " (inactive)"));
		}
#endif
	}
}
...
FT_LOCAL_DEF(void) af_latin_metrics_scale(AF_StyleMetrics metrics_, AF_Scaler scaler)
{
	AF_LatinMetrics metrics = (AF_LatinMetrics) metrics_;
...
/* metrics class.                                             */
FT_LOCAL_DEF(void) af_latin_get_standard_widths(AF_StyleMetrics metrics_, FT_Pos *stdHW, FT_Pos *stdVW)
{
	AF_LatinMetrics metrics = (AF_LatinMetrics) metrics_;
	if (stdHW)
...
FT_LOCAL_DEF(FT_Error) af_latin_hints_compute_segments(AF_GlyphHints hints, AF_Dimension dim)
{
...
FT_LOCAL_DEF(void) af_latin_hints_link_segments(AF_GlyphHints hints, FT_UInt width_count, AF_WidthRec * widths, AF_Dimension dim)
{
	AF_AxisHints axis = &hints->axis[dim];
	AF_Segment segments = axis->segments;
...
FT_LOCAL_DEF(FT_Error) af_latin_hints_compute_edges(AF_GlyphHints hints, AF_Dimension dim)
{
	AF_AxisHints axis = &hints->axis[dim];
	FT_Error error = FT_Err_Ok;
	FT_Memory memory = hints->memory;
	...
	for (seg = segments; seg < segment_limit; seg++)
	{
		...
		if (!found)
		{
			...
			error = af_axis_hints_new_edge(axis, seg->pos,
										   (AF_Direction) seg->dir, top_to_bottom_hinting, memory, &edge);
			if (error)
				goto Exit;
...
FT_LOCAL_DEF(FT_Error) af_latin_hints_detect_features(AF_GlyphHints hints, FT_UInt width_count, AF_WidthRec * widths, AF_Dimension dim)
{
	FT_Error error;
...
static FT_Error af_latin_hints_init(AF_GlyphHints hints, AF_StyleMetrics metrics_)
{
	AF_LatinMetrics metrics = (AF_LatinMetrics) metrics_;
	FT_Render_Mode mode;
...
static FT_Pos af_latin_compute_stem_width(AF_GlyphHints hints,
							AF_Dimension dim, FT_Pos width, FT_Pos base_delta, FT_UInt base_flags, FT_UInt stem_flags)
{
	AF_LatinMetrics metrics = (AF_LatinMetrics) hints->metrics;
	AF_LatinAxis axis = &metrics->axis[dim];
	FT_Pos dist = width;
	...
	for (edge = edges; edge < edge_limit; edge++)
	{
		...
		if (!anchor)
		{
			...
			FT_TRACE5(("  ANCHOR: edge %d (opos=%.2f) and %d (opos=%.2f)"
					   ...
					   edge2 - edges, edge2->opos / 64.0, edge->pos / 64.0, edge2->pos / 64.0));
		...
		for (edge = edges; edge < edge_limit; edge++)
		{
			...
			if (delta < 64 + 16)
			{
				...
				FT_TRACE5(("  SERIF: edge %d (opos=%.2f) serif to %d (opos=%.2f)"
						   " aligned to %.2f\n",
						   edge - edges, edge->opos / 64.0,
						   edge->serif - edges, edge->serif->opos / 64.0, edge->pos / 64.0));
			} else if (!anchor)
			{
				edge->pos = FT_PIX_ROUND(edge->opos);
				anchor = edge;
			...
			} else
			{
				...
				if (before >= edges && before < edge && after < edge_limit && after > edge)
				{
					if (after->opos == before->opos)
						...
						edge->pos = before->pos +
							FT_MulDiv(edge->opos - before->opos, after->pos - before->pos, after->opos - before->opos);
...
static FT_Error af_latin_hints_apply(FT_UInt32 glyph_index, AF_GlyphHints hints, FT_Outline *outline, AF_StyleMetrics metrics_)
{
	AF_LatinMetrics metrics = (AF_LatinMetrics) metrics_;
	FT_Error error;
	int dim;
...
#ifdef AF_CONFIG_OPTION_USE_WARPER
		if (dim == AF_DIMENSION_HORZ &&
			metrics->root.scaler.render_mode == FT_RENDER_MODE_NORMAL && AF_HINTS_DO_WARP(hints))
		{
			AF_WarperRec warper;
			FT_Fixed scale;
			FT_Pos delta;
