...
FT_LOCAL_DEF(FT_Error) af_latin2_hints_compute_segments(AF_GlyphHints hints, AF_Dimension dim);

FT_LOCAL_DEF(void) af_latin2_hints_link_segments(AF_GlyphHints hints, AF_Dimension dim);
...
FT_LOCAL_DEF(void) af_latin2_metrics_init_widths(AF_LatinMetrics metrics, FT_Face face)
{
...
FT_LOCAL_DEF(void) af_latin2_metrics_check_digits(AF_LatinMetrics metrics, FT_Face face)
{
	FT_UInt i;
	FT_Bool started = FALSE;
	FT_Bool same_width = TRUE;
	FT_Fixed advance;
	...
	for (i = 0x30; i <= 0x39; i++)
	{
		...
		if (FT_Get_Advance(face, glyph_index,
						   FT_LOAD_NO_SCALE | FT_LOAD_NO_HINTING | FT_LOAD_IGNORE_TRANSFORM, &advance))
			continue;
...
FT_LOCAL_DEF(FT_Error) af_latin2_metrics_init(AF_StyleMetrics metrics_, FT_Face face)
{
...
FT_LOCAL_DEF(void) af_latin2_metrics_scale(AF_StyleMetrics metrics_, AF_Scaler scaler)
{
	AF_LatinMetrics metrics = (AF_LatinMetrics) metrics_;
...
/* metrics class.                                             */
FT_LOCAL_DEF(void) af_latin2_get_standard_widths(AF_StyleMetrics metrics_, FT_Pos *stdHW, FT_Pos *stdVW)
{
	AF_LatinMetrics metrics = (AF_LatinMetrics) metrics_;
	if (stdHW)
...
FT_LOCAL_DEF(FT_Error) af_latin2_hints_compute_edges(AF_GlyphHints hints, AF_Dimension dim)
{
	AF_AxisHints axis = &hints->axis[dim];
	FT_Error error = FT_Err_Ok;
	FT_Memory memory = hints->memory;
...
FT_LOCAL_DEF(FT_Error) af_latin2_hints_detect_features(AF_GlyphHints hints, AF_Dimension dim)
{
	FT_Error error;
...
static FT_Error af_latin2_hints_init(AF_GlyphHints hints, AF_StyleMetrics metrics_)
{
	AF_LatinMetrics metrics = (AF_LatinMetrics) metrics_;
	FT_Render_Mode mode;
...
static FT_Pos af_latin2_compute_stem_width(AF_GlyphHints hints,
							 AF_Dimension dim, FT_Pos width, FT_UInt base_flags, FT_UInt stem_flags)
{
		...
		} else
		{
		  ...
		  AlignStem:
			...
			FT_TRACE5(("STEM1: %d (opos=%.2f) to %d (opos=%.2f)"
					   ...
					   edge->pos / 64.0, edge2->pos / 64.0, org_len / 64.0, cur_len / 64.0));
...
static FT_Error af_latin2_hints_apply(FT_UInt32 glyph_index, AF_GlyphHints hints, FT_Outline *outline, AF_StyleMetrics metrics_)
{
	AF_LatinMetrics metrics = (AF_LatinMetrics) metrics_;
	FT_Error error;
	int dim;
