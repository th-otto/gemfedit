...
FT_LOCAL_DEF(FT_Error) af_axis_hints_new_segment(AF_AxisHints axis, FT_Memory memory, AF_Segment * asegment)
{
...
FT_LOCAL(FT_Error) af_axis_hints_new_edge(AF_AxisHints axis, FT_Int fpos, AF_Direction dir, FT_Bool top_to_bottom_hinting, FT_Memory memory, AF_Edge * anedge)
{
	FT_Error error = FT_Err_Ok;
	AF_Edge edge = NULL;
	AF_Edge edges;
	...
	for (dimension = 1; dimension >= 0; dimension--)
	{
		...
		if (axis->num_segments)
			AF_DUMP(("  index   pos   delta   dir   from   to "
					  "  link  serif  edge" "  height  extra     flags\n"));
		else
			AF_DUMP(("  (none)\n"));
		...
		for (seg = segments; seg < limit; seg++)
			AF_DUMP(("  %5d  %5d  %5d  %5s  %4d  %4d"
					 ...
					 seg->height,
					 seg->height - (seg->max_coord - seg->min_coord), af_edge_flags_to_string(seg->flags)));
		AF_DUMP(("\n"));
	}
}
#ifdef __cplusplus
	...
	for (dimension = 1; dimension >= 0; dimension--)
	{
		...
		for (edge = edges; edge < limit; edge++)
			AF_DUMP(("  %5d  %7.2f  %5s  %4s  %5s"
					 ...
					 edge->blue_edge ? 'y' : 'n',
					 edge->opos / 64.0, edge->pos / 64.0, af_edge_flags_to_string(edge->flags)));
		AF_DUMP(("\n"));
	}
}
#ifdef __cplusplus
...
FT_LOCAL_DEF(AF_Direction) af_direction_compute(FT_Pos dx, FT_Pos dy)
{
...
FT_LOCAL_DEF(void) af_glyph_hints_init(AF_GlyphHints hints, FT_Memory memory)
{
...
FT_LOCAL_DEF(void) af_glyph_hints_rescale(AF_GlyphHints hints, AF_StyleMetrics metrics)
{
...
FT_LOCAL_DEF(FT_Error) af_glyph_hints_reload(AF_GlyphHints hints, FT_Outline * outline)
{
	FT_Error error = FT_Err_Ok;
	AF_Point points;
				...
				} else if (point->out_dir == point->in_dir)
				{
...

					{
						...
						if (ft_corner_is_flat(point->fx - prev_v->fx,
											  point->fy - prev_v->fy, next_u->fx - point->fx, next_u->fy - point->fy))
						{
...
FT_LOCAL_DEF(void) af_glyph_hints_save(AF_GlyphHints hints, FT_Outline * outline)
{
...
FT_LOCAL_DEF(void) af_glyph_hints_align_edge_points(AF_GlyphHints hints, AF_Dimension dim)
{
	AF_AxisHints axis = &hints->axis[dim];
	AF_Segment segments = axis->segments;
...
FT_LOCAL_DEF(void) af_glyph_hints_align_strong_points(AF_GlyphHints hints, AF_Dimension dim)
{
	AF_Point points = hints->points;
	AF_Point point_limit = points + hints->num_points;
	AF_AxisHints axis = &hints->axis[dim];
...
FT_LOCAL_DEF(void) af_glyph_hints_align_weak_points(AF_GlyphHints hints, AF_Dimension dim)
{
	AF_Point points = hints->points;
	AF_Point point_limit = points + hints->num_points;
	AF_Point *contour = hints->contours;
...
FT_LOCAL_DEF(void) af_glyph_hints_scale_dim(AF_GlyphHints hints, AF_Dimension dim, FT_Fixed scale, FT_Pos delta)
{
	AF_Point points = hints->points;
