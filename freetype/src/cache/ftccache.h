...
#define FTC_NODE_TOP_FOR_HASH( cache, hash )                      \
        ( ( cache )->buckets +                                    \
            ( ( ( ( hash ) &   ( cache )->mask ) < ( cache )->p ) \
              ...
              : ( ( hash ) &   ( cache )->mask ) ) )
#else
FT_LOCAL(FTC_Node *) ftc_get_top_node_for_hash(FTC_Cache cache, FT_Offset hash);
#define FTC_NODE_TOP_FOR_HASH( cache, hash )             \
...
#ifndef FTC_INLINE
FT_LOCAL(FT_Error) FTC_Cache_Lookup(FTC_Cache cache, FT_Offset hash, FT_Pointer query, FTC_Node * anode);
#endif

FT_LOCAL(FT_Error) FTC_Cache_NewNode(FTC_Cache cache, FT_Offset hash, FT_Pointer query, FTC_Node * anode);
 ...
 */
FT_LOCAL(void) FTC_Cache_RemoveFaceID(FTC_Cache cache, FTC_FaceID face_id);
