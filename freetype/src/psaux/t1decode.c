...
t1_decoder_parse_charstrings(T1_Decoder decoder, FT_Byte * charstring_base, FT_UInt charstring_len)
{
	...
	seed = (FT_Fixed) (((FT_Offset) (char *) &seed ^
						(FT_Offset) (char *) &decoder ^ (FT_Offset) (char *) &charstring_base) & FT_ULONG_MAX);
	...
	while (ip < limit)
	{
	  ...
	  /*********************************************************************/
		...
		case 255:						/* four bytes integer */
			...
			if (value > 32000 || value < -32000)
			{
				if (large_int)
				{
					FT_ERROR(("t1_decoder_parse_charstrings:" " no `div' after large integer\n"));
				} else
					large_int = TRUE;
			} else
			{
		...
		default:
			if (ip[-1] >= 32)
			{
				...
				else
				{
					...
					if (ip[-2] < 251)
						value = (((FT_Int32) ip[-2] - 247) * 256) + ip[-1] + 108;
					else
						value = -((((FT_Int32) ip[-2] - 251) * 256) + ip[-1] + 108);
				}
			...
			{
				FT_ERROR(("t1_decoder_parse_charstrings:" " invalid byte (%d)\n", ip[-1]));
				goto Syntax_Error;
			}
		}
	  ...
	  /*********************************************************************/
		...
		/***********************************************************/
			...
			case 19:
				...
				/*    of BuildCharArray with WeightVector               */
				{
					...
					ft_memcpy(&decoder->buildchar[idx],
							  blend->weight_vector, blend->num_designs * sizeof(blend->weight_vector[0]));
				}
				break;
			...
			default:
			  ...
			  Unexpected_OtherSubr:
				FT_ERROR(("t1_decoder_parse_charstrings:" " invalid othersubr [%d %d]\n", arg_cnt, subr_no));
				goto Syntax_Error;
			}
		...
		} else							/* general operator */
		{
			...
			default:
				FT_ERROR(("t1_decoder_parse_charstrings:" " unhandled opcode %d\n", op));
				goto Syntax_Error;
			}
...
FT_LOCAL_DEF(FT_Error) t1_decoder_parse_glyph(T1_Decoder decoder, FT_UInt32 glyph)
{
