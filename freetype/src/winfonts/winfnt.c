		...
		} else if (ne_header.magic == WINFNT_PE_MAGIC)
		{
			...
			for (i = 0; i < pe32_header.number_of_sections; i++)
			{
				...
				FT_TRACE2(("name %.8s, va %04lx, size %04lx, offset %04lx\n",
						   ...
						   pe32_section.size_of_raw_data, pe32_section.pointer_to_raw_data));
		  ...
		  Found_rsrc_section:
			...
			for (i = 0; i < root_dir.number_of_named_entries + root_dir.number_of_id_entries; i++)
			{
				...
				if (FT_STREAM_SEEK(pe32_section.pointer_to_raw_data +
								   dir_entry1.offset) || FT_STREAM_READ_FIELDS(winpe_rsrc_dir_fields, &name_dir))
					goto Exit;
				...
				for (j = 0; j < name_dir.number_of_named_entries + name_dir.number_of_id_entries; j++)
				{
					...
					if (FT_STREAM_SEEK(pe32_section.pointer_to_raw_data +
									   dir_entry2.offset) || FT_STREAM_READ_FIELDS(winpe_rsrc_dir_fields, &lang_dir))
						goto Exit;
							  ...
							  FT_Int face_instance_index, FT_Int num_params, FT_Parameter * params)
{
	...
	/* with relevant information                   */
	{
		...
		if (font->header.pixel_width != 0 && !(font->header.pitch_and_family & 1))
