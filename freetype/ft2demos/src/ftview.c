...
static FT_Error Render_Stroke(int num_indices, int offset)
{
	...
	radius = (FT_Fixed) (status.radius * (status.ptsize * status.res / 72));
	...
	for (i = offset; i < num_indices; i++)
	{
		...
		error = FT_Load_Glyph(face, glyph_idx, handle->load_flags | FT_LOAD_NO_BITMAP);
...
static FT_Error Render_Fancy(int num_indices, int offset)
{
	int start_x, start_y, step_y, x, y;
	int i, have_topleft;
	FT_Size size;
	...
	for (i = offset; i < num_indices; i++)
	{
		...
		} else if (slot->format == FT_GLYPH_FORMAT_BITMAP)
		{
			...
			error = FT_Bitmap_Embolden(slot->library, &slot->bitmap, xstr, ystr);
			if (error)
				goto Next;
		} else
		{
...
static FT_Error Render_All(int num_indices, int offset)
{
	int start_x, start_y, step_y, x, y;
	int i, have_topleft;
	FT_Size size;
...
static FT_Error Render_Text(int num_indices, int offset)
{
	int start_x, start_y, step_y, x, y;
	FT_Size size;
	int have_topleft;
...
static FT_Error Render_Waterfall(int first_size, int offset)
{
	int start_x, start_y, step_y, x, y;
...
static void event_help(void)
{
	...
	sprintf(buf, "FreeType Glyph Viewer - part of the FreeType %s test suite", version);
...
static int event_cff_hinting_engine_change(unsigned int delta)
{
	unsigned int new_cff_hinting_engine = 0;

	if (delta)
		new_cff_hinting_engine = (status.cff_hinting_engine + delta + N_CFF_HINTING_ENGINES) % N_CFF_HINTING_ENGINES;
...
static int event_tt_interpreter_version_change(void)
{
...
static int event_warping_change(void)
{
...
static int event_bold_change(double xdelta, double ydelta)
{
	...
	return (old_xbold_factor == status.xbold_factor && old_ybold_factor == status.ybold_factor) ? 0 : 1;
}


static int event_radius_change(double delta)
{
...
static int event_slant_change(double delta)
{
...
static int event_size_change(int delta)
{
...
static int event_index_change(int delta)
{
...
{
	if (delta)
		status.render_mode = (status.render_mode + delta + N_RENDER_MODES) % N_RENDER_MODES;
}


static int event_encoding_change(void)
{
...
static int event_font_change(int delta)
{
	int num_indices;

	if (status.font_idx + delta >= handle->num_fonts || status.font_idx + delta < 0)
		return 0;
...
static int Process_Event(grEvent *event)
{
	...
	case grKEY('l'):
	case grKEY('k'):
		status.lcd_idx = (status.lcd_idx + (event->key == grKEY('l') ? 1 : -1) + N_LCD_IDXS) % N_LCD_IDXS;
	...
	{
	case grKEY('L'):
		FTC_Manager_RemoveFaceID(handle->cache_manager, handle->scaler.face_id);
		...
		case FT_LCD_FILTER_LEGACY1:
			FT_Library_SetLcdFilter(handle->library, (FT_LcdFilter) status.lcd_filter);
			break;
		default:
...
static void write_header(FT_Error error_code)
{
	...
	FTDemo_Draw_Header(handle, display, status.ptsize, status.res,
					   ...
					   status.render_mode != RENDER_MODE_WATERFALL ? status.topleft : -1, error_code);
	...
	/* render mode */
	{
		...
		sprintf(buf, "%d: %s", status.render_mode + 1, render_mode);
		grWriteCellString(display->bitmap, 0, (line++) * HEADER_HEIGHT, buf, display->fore_color);
	}

	if (status.render_mode == RENDER_MODE_FANCY)
	{
		/* x emboldening */
		sprintf(buf, " x: % .3f", status.xbold_factor);
		grWriteCellString(display->bitmap, 0, (line++) * HEADER_HEIGHT, buf, display->fore_color);

		/* y emboldening */
		sprintf(buf, " y: % .3f", status.ybold_factor);
		grWriteCellString(display->bitmap, 0, (line++) * HEADER_HEIGHT, buf, display->fore_color);

		/* slanting */
		sprintf(buf, " s: % .3f", status.slant);
		grWriteCellString(display->bitmap, 0, (line++) * HEADER_HEIGHT, buf, display->fore_color);
	}
	...
	{
		/* stroking radius */
		sprintf(buf, " radius: %.3f", status.radius);
		grWriteCellString(display->bitmap, 0, (line++) * HEADER_HEIGHT, buf, display->fore_color);
	}
	...
	/* anti-aliasing */
	{
		...
		grWriteCellString(display->bitmap, 0, (line++) * HEADER_HEIGHT, lcd_mode, display->fore_color);
	}

	/* hinting */
	sprintf(buf, "hinting: %s", handle->hinted ? "on" : "off");
	grWriteCellString(display->bitmap, 0, (line++) * HEADER_HEIGHT, buf, display->fore_color);

	if (handle->hinted)
	{
		/* auto-hinting */
		sprintf(buf, " forced auto: %s", (handle->autohint || handle->lcd_mode == LCD_MODE_LIGHT) ? "on" : "off");
		grWriteCellString(display->bitmap, 0, (line++) * HEADER_HEIGHT, buf, display->fore_color);
	}
	...
	if (!handle->autohint && handle->lcd_mode != LCD_MODE_LIGHT)
	{
		...
		FTC_Manager_LookupFace(handle->cache_manager, handle->scaler.face_id, &face);
		module = &face->driver->root;
		...
		if (hinting_engine)
		{
			sprintf(buf, "engine: %s", hinting_engine);
			grWriteCellString(display->bitmap, 0, (line++) * HEADER_HEIGHT, buf, display->fore_color);
		}
	}
	...
	if (handle->lcd_mode == LCD_MODE_AA && handle->autohint)
	{
		sprintf(buf, "warping: %s", status.warping ? "on" : "off");
		grWriteCellString(display->bitmap, 0, (line++) * HEADER_HEIGHT, buf, display->fore_color);
	}
	...
	/* embedded bitmaps */
	sprintf(buf, "bitmaps: %s", handle->use_sbits == 2 ? "color" : handle->use_sbits == 1 ? "gray" : "off");
	grWriteCellString(display->bitmap, 0, (line++) * HEADER_HEIGHT, buf, display->fore_color);

	/* cache */
	sprintf(buf, "cache: %s", handle->use_sbits_cache ? "on" : "off");
	grWriteCellString(display->bitmap, 0, (line++) * HEADER_HEIGHT, buf, display->fore_color);
	...
	if (handle->lcd_mode >= LCD_MODE_RGB)
	{
		sprintf(buf, "filter: %s",
				...
				status.lcd_filter == 2 ? "light" : status.lcd_filter == 3 ? "legacy" : "custom");
		grWriteCellString(display->bitmap, 0, (line++) * HEADER_HEIGHT, buf, display->fore_color);
		...
		if (status.lcd_filter < 0)
		{
			...
			for (i = 0; i < 5; i++)
			{
				sprintf(buf, " %s0x%02X%s", fwi == i ? "[" : " ", fw[i], fwi == i ? "]" : " ");
				grWriteCellString(display->bitmap, 0, (line++) * HEADER_HEIGHT, buf, display->fore_color);
			}
		}
	}
...
int main(int argc, char *argv[])
{
	...
	for (i = 0; i < 3; i++)
	{
		error = FT_Property_Set(handle->library, "truetype", "interpreter-version", &versions[i]);
		if (!error)
			status.tt_interpreter_versions[status.num_tt_interpreter_versions++] = versions[i];
	...
	grSetTitle(display->surface, "FreeType Glyph Viewer - press ? for help");
	...
	do
	{
		...
		{
		case RENDER_MODE_ALL:
			error = Render_All(handle->current_font->num_indices, status.offset);
			break;

		case RENDER_MODE_FANCY:
			error = Render_Fancy(handle->current_font->num_indices, status.offset);
			break;

		case RENDER_MODE_STROKE:
			error = Render_Stroke(handle->current_font->num_indices, status.offset);
			break;
