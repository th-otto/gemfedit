...
static int test_load_advances(btimer_t * timer, FT_Face face, void *user_data)
{
	...
	advances = (FT_Fixed *) calloc(sizeof(FT_Fixed), (size_t) face->num_glyphs);

	TIMER_START(timer);

	FT_Get_Advances(face, first_index, (unsigned int) face->num_glyphs - first_index, (FT_Int32) flags, advances);
...
static void get_charset(FT_Face face, bcharset_t * charset)
{
	...
	charset->code = (FT_ULong *) calloc((size_t) face->num_glyphs, sizeof(FT_ULong));
	if (!charset->code)
		return;
...
static FT_Error get_face(FT_Face * face)
{
	...
	if (preload)
	{
		...
		error = FT_New_Memory_Face(lib, memory_file, (FT_Long) memory_size, face_index, face);
	} else
	{
...
static void usage(void)
{
	...
	if (num_tt_interpreter_versions == 2)
		sprintf(interpreter_versions, "%d and %d", tt_interpreter_versions[0], tt_interpreter_versions[1]);
	else
		sprintf(interpreter_versions,
				"%d, %d, and %d", tt_interpreter_versions[0], tt_interpreter_versions[1], tt_interpreter_versions[2]);

	/* we expect that at least one hinting engine is available */
	if (num_cff_hinting_engines == 1)
		sprintf(hinting_engines, "`%s'", cff_hinting_engine_names[cff_hinting_engines[0]]);
	else
	...
	fprintf(stderr,
			...
			interpreter_versions, dflt_tt_interpreter_version, CACHE_SIZE);
	fprintf(stderr,
...
int main(int argc, char **argv)
{
	...
	for (;;)
	{
		...
		case 'H':
			...
			for (j = 0; j < num_cff_hinting_engines; j++)
			{
				...
				{
					FT_Property_Set(lib, "cff", "hinting-engine", &j);
					break;
				}
			}
		...
		case 'I':
			...
			for (j = 0; j < num_tt_interpreter_versions; j++)
			{
				...
				{
					FT_Property_Set(lib, "truetype", "interpreter-version", &version);
					break;
				}
			}

			if (j == num_tt_interpreter_versions)
				fprintf(stderr, "warning: couldn't set TT interpreter version\n");
			break;
	...
	if (size)
	{
		...
		} else
		{
			...
			fprintf(stderr, "using size of first bitmap strike (%dpx)\n", size);
