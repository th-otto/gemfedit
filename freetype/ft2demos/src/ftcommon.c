...
FTDemo_Display *FTDemo_Display_New(grPixelMode mode, int width, int height)
{
	...
	display->fore_color = grFindColor(display->bitmap, 0x00, 0x00, 0x00, 0xff);
	display->back_color = grFindColor(display->bitmap, 0xff, 0xff, 0xff, 0xff);
	display->warn_color = grFindColor(display->bitmap, 0xff, 0x00, 0x00, 0xff);
...
static FT_Error my_face_requester(FTC_FaceID face_id, FT_Library lib, FT_Pointer request_data, FT_Face *aface)
{
	...
	if (font->file_address != NULL)
	{
		error = FT_New_Memory_Face(lib,
								   ...
								   (FT_Long) font->file_size, font->face_index, aface);
	} else
	{
...
FTDemo_Handle *FTDemo_New(void)
{
	...
	error = FTC_Manager_New(handle->library, 0, 0, 0, my_face_requester, 0, &handle->cache_manager);
	if (error)
...
FT_Error FTDemo_Install_Font(FTDemo_Handle *handle, const char *filepath, FT_Bool outline_only, FT_Bool no_instances)
{
	...
	for (i = 0; i < num_faces; i++)
	{
		...
		{
			error = FT_New_Face(handle->library, filename, (j << 16) + i, &face);
			if (error)
				continue;
			...
			{
				handle->max_fonts = 16;
				handle->fonts = (PFont *) calloc((size_t) handle->max_fonts, sizeof(PFont));
...
void FTDemo_Set_Current_Size(FTDemo_Handle *handle, int pixel_size)
{
	...
	if (!error && !FT_IS_SCALABLE(face))
	{
		...
		int j = 0;
		int c;
		int d = abs((int) face->available_sizes[j].y_ppem - pixel_size * 64);
...
void FTDemo_Set_Current_Charsize(FTDemo_Handle *handle, int char_size, int resolution)
{
	...
	if (!error && !FT_IS_SCALABLE(face))
	{
		...
		int j = 0;
		int c;
		int d = abs((int) face->available_sizes[j].y_ppem - pixel_size);
...
void FTDemo_Draw_Header(FTDemo_Handle *handle, FTDemo_Display *display, int ptsize, int res, int idx, int error_code)
{
	...
	grWriteCellString(display->bitmap, DIM_X - 8 * 11, line * HEADER_HEIGHT, buf, display->fore_color);
	...
	grBitmap *target, int *left, int *top, int *x_advance, int *y_advance, FT_Glyph *aglyf)
{
	...
	*aglyf = NULL;
	...
	case FT_PIXEL_MODE_LCD:
		target->mode = handle->lcd_mode == LCD_MODE_RGB ? gr_pixel_mode_lcd : gr_pixel_mode_lcd2;
		target->grays = source->num_grays;
		break;

	case FT_PIXEL_MODE_LCD_V:
		target->mode = handle->lcd_mode == LCD_MODE_VRGB ? gr_pixel_mode_lcdv : gr_pixel_mode_lcdv2;
		target->grays = source->num_grays;
		break;
...
FT_Error FTDemo_Draw_Index(FTDemo_Handle *handle, FTDemo_Display *display, unsigned int gindex, int *pen_x, int *pen_y)
{
	...
	error = FTDemo_Index_To_Bitmap(handle, gindex, &bit3, &left, &top, &x_advance, &y_advance, &glyf);
	if (error)
		return error;

	/* now render the bitmap into the display surface */
	grBlitGlyphToBitmap(display->bitmap, &bit3, *pen_x + left, *pen_y - top, display->fore_color);
	...
	FTDemo_Display *display, FT_Glyph glyph, int *pen_x, int *pen_y, grColor color)
{
	...
	error = FTDemo_Glyph_To_Bitmap(handle, glyph, &bit3, &left, &top, &x_advance, &y_advance, &glyf);
	if (error)
	{
		FT_Done_Glyph(glyph);
	...
	/* now render the bitmap into the display surface */
	grBlitGlyphToBitmap(display->bitmap, &bit3, *pen_x + left, *pen_y - top, color);
...
static FT_Error string_render_prepare(FTDemo_Handle *handle, FTDemo_String_Context *sc, FT_Vector *advances)
{
	...
	if (!sc->vertical && sc->kerning_degree)
	{
		...
		if (FT_Get_Track_Kerning(face, (FT_Fixed) handle->scaler.width << 10, -sc->kerning_degree, &track_kern))
			track_kern = 0;
		else
			track_kern = (FT_Pos) ((track_kern / 1024.0 * handle->scaler.x_res) / 72.0);
	}
	...
	for (i = 0; i < handle->string_length; i++)
	{
		...
		else
		{
			...
			if (prev_advance)
			{
				...
				if (sc->kerning_mode)
				{
					...
					if (handle->lcd_mode != LCD_MODE_LIGHT_SUBPIXEL && sc->kerning_mode > KERNING_MODE_NORMAL)
		...
		if (prev_advance)
		{
			if (handle->lcd_mode != LCD_MODE_LIGHT_SUBPIXEL && handle->hinted)
			{
	...
	for (n = 0; n < handle->string_length; n++)
	{
		...
		if (bbox.xMax > 0 && bbox.yMax > 0 && bbox.xMin < display->bitmap->width && bbox.yMin < display->bitmap->rows)
		{
			...
			if (!error)
			{
				...
				/* now render the bitmap into the display surface */
				grBlitGlyphToBitmap(display->bitmap, &bit3, left, top, display->fore_color);
...
FT_Encoding FTDemo_Make_Encoding_Tag(const char *s)
{
	int i;
	FT_Tag l = 0;
	...
	for (i = 0; i < 4; i++)
	{
		if (!s[i])
			break;
		l <<= 8;
		l += (FT_Tag)(unsigned char)s[i];
	}
	
	if (l == FT_MAKE_TAG(0, 0, 0, 0))
		return FT_ENCODING_NONE;
	if (l == FT_MAKE_TAG('s', 'y', 'm', 'b'))
		return FT_ENCODING_MS_SYMBOL;
	if (l == FT_MAKE_TAG('u', 'n', 'i', 'c'))
		return FT_ENCODING_UNICODE;
	if (l == FT_MAKE_TAG('s', 'j', 'i', 's'))
		return FT_ENCODING_SJIS;
	if (l == FT_MAKE_TAG('g', 'b', ' ', ' '))
		return FT_ENCODING_PRC;
	if (l == FT_MAKE_TAG('g', 'b', 0, 0))
		return FT_ENCODING_PRC;
	if (l == FT_MAKE_TAG('b', 'i', 'g', '5'))
		return FT_ENCODING_BIG5;
	if (l == FT_MAKE_TAG('w', 'a', 'n', 's'))
		return FT_ENCODING_WANSUNG;
	if (l == FT_MAKE_TAG('j', 'o', 'h', 'a'))
		return FT_ENCODING_JOHAB;
	if (l == FT_MAKE_TAG('A', 'D', 'O', 'B'))
		return FT_ENCODING_ADOBE_STANDARD;
	if (l == FT_MAKE_TAG('A', 'D', 'B', 'E'))
		return FT_ENCODING_ADOBE_EXPERT;
	if (l == FT_MAKE_TAG('A', 'D', 'B', 'C'))
		return FT_ENCODING_ADOBE_CUSTOM;
	if (l == FT_MAKE_TAG('l', 'a', 't', '1'))
		return FT_ENCODING_ADOBE_LATIN_1;
	if (l == FT_MAKE_TAG('l', 'a', 't', '2'))
		return FT_ENCODING_OLD_LATIN_2;
	if (l == FT_MAKE_TAG('a', 'r', 'm', 'n'))
		return FT_ENCODING_APPLE_ROMAN;

	if (l == FT_MAKE_TAG(-1, -1, -1, -1))
		return FT_ENCODING_ORDER;
