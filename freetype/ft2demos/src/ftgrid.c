...
static void grid_status_init(GridStatus st)
{
	st->width = DIM_X;
	st->height = DIM_Y;
...
static void grid_status_draw_grid(GridStatus st)
{
	...
	if (xy_incr >= 2)
	{
		...
		for (; x2 < st->disp_width; x2 += xy_incr)
			grFillVLine(st->disp_bitmap, x2, 0, st->disp_height, st->grid_color);

		for (x2 = x_org - xy_incr; x2 >= 0; x2 -= xy_incr)
			grFillVLine(st->disp_bitmap, x2, 0, st->disp_height, st->grid_color);

		for (; y2 < st->disp_height; y2 += xy_incr)
			grFillHLine(st->disp_bitmap, 0, y2, st->disp_width, st->grid_color);
...
static void grid_hint_draw_segment(GridStatus st, FT_Size size, AF_GlyphHints hints)
{
	...
	for (dimension = 1; dimension >= 0; dimension--)
	{
		...
		for (count = 0; count < num_seg; count++)
		{
			...
			af_glyph_hints_get_segment_offset(hints, dimension, count, &offset, &is_blue, &blue_offset);

			if (dimension == 0)			/* AF_DIMENSION_HORZ is 0 */
			{
				offset = FT_MulFix(offset, x_scale);
				pos = x_org + ((offset * st->scale) >> 6);
				grFillVLine(st->disp_bitmap, pos, 0, st->disp_height, st->segment_color);
			} else
			{
				...
				if (is_blue)
				{
					...
					if (blue_pos == pos)
						grFillHLine(st->disp_bitmap, 0, blue_pos, st->disp_width, st->blue_color);
					else
					{
						grFillHLine(st->disp_bitmap, 0, blue_pos, st->disp_width, st->blue_color);
						grFillHLine(st->disp_bitmap, 0, pos, st->disp_width, st->segment_color);
					}
				} else
					grFillHLine(st->disp_bitmap, 0, pos, st->disp_width, st->segment_color);
			}
				...
				double scale, int pen_x, int pen_y, FTDemo_Handle * handle, FTDemo_Display * display, grColor color)
{
	...
	FT_Outline_New(handle->library, (FT_UInt) outline->n_points, outline->n_contours, &transformed);
	...
	ft_bitmap_draw(&bitm, pen_x + (cbox.xMin >> 6), pen_y - (cbox.yMax >> 6), display, color);
...
static void grid_status_draw_outline(GridStatus st, FTDemo_Handle * handle, FTDemo_Display * display)
{
	...
	if (st->do_grid)
	{
		...
		grFillVLine(st->disp_bitmap,
					...
					((size->face->glyph->metrics.horiAdvance +
					  ...
					  size->face->glyph->rsb_delta) * st->scale >> 6),
					  0, st->disp_height,
					  st->axis_color);
	}
	...
	if (slot->format == FT_GLYPH_FORMAT_OUTLINE)
	{
		...
		if (st->work & DO_BITMAP)
		{
			...
			error = FTDemo_Glyph_To_Bitmap(handle, glyph, &bitg, &left, &top, &x_advance, &y_advance, &glyf);

			if (!error)
			{
				bitmap_scale(&bitg, scale);

				grBlitGlyphToBitmap(display->bitmap, &bitg, ox + left * scale, oy - top * scale, st->axis_color);
		...
		if (st->work & DO_OUTLINE)
		{
			...
			error = FTDemo_Draw_Glyph_Color(handle, display, glyph, &ox, &oy, st->outline_color);
			if (!error)
				FT_Done_Glyph(glyph);
		}
		...
		if (st->work & DO_DOTS)
		{
			for (nn = 0; nn < gimage->n_points; nn++)
				circle_draw(st->x_origin * 64 + gimage->points[nn].x,
							...
							128, handle, display, (gimage->tags[nn] & FT_CURVE_TAG_ON) ? st->on_color : st->off_color);
		}
			...
			for (; c < gimage->n_contours; c++)
			{
				for (;;)
				{
					...
					if (c == 0)
					{
						...
						{
							prev = n > 0 ? n - 1 : contours[c];
							next = n < contours[c] ? n + 1 : 0;
						}
					} else
					{
						prev = n > (contours[c - 1] + 1) ? n - 1 : contours[c];
						next = n < contours[c] ? n + 1 : contours[c - 1] + 1;
					}
					...
					if ((middle.x < 4096) && (middle.x > -4096) && (middle.y < 4096) && (middle.y > -4096))
					{
					...
					num_digits = snprintf(number_string, number_string_len, "%d", n);
					...
					grWriteCellString(display->bitmap,
									  ...
									  (middle.x > 0 ? (num_digits - 1) * 8 + 2 : 2),
									  st->y_origin - (((points[n].y - middle.y) >> 6) + 8 / 2),
									  number_string, (tags[n] & FT_CURVE_TAG_ON) ? st->on_color : st->off_color);
...
static const unsigned char *Text = (unsigned char *)
	...
	"&#~\"\'(-`_^@)=+\260 ABCDEFGHIJKLMNOPQRSTUVWXYZ " "$\243^\250*\265\371%!\247:/;.,?<>";
#endif
...
static void event_help(void)
{
	...
	sprintf(buf, "FreeType Glyph Grid Viewer - part of the FreeType %s test suite", version);
...
static void event_cff_hinting_engine_change(int delta)
{
	int new_cff_hinting_engine = 0;

	if (delta)
		new_cff_hinting_engine =
			((int) status.cff_hinting_engine + delta + N_CFF_HINTING_ENGINES) % N_CFF_HINTING_ENGINES;

	error = FT_Property_Set(handle->library, "cff", "hinting-engine", &new_cff_hinting_engine);
...
static void event_tt_interpreter_version_change(void)
{
	...
	error = FT_Property_Set(handle->library,
							"truetype",
							"interpreter-version", &status.tt_interpreter_versions[status.tt_interpreter_version_idx]);
...
static void event_warping_change(void)
{
	if (handle->lcd_mode == LCD_MODE_AA && handle->autohint)
	{
		FT_Bool new_warping_state = !status.warping;

		error = FT_Property_Set(handle->library, "autofitter", "warping", &new_warping_state);
		...
		status.header = status.warping ? "warping enabled" : "warping disabled";
	} else
	{
...
static void event_grid_reset(GridStatus st)
{
...
static void event_grid_translate(int dx, int dy)
{
...
static void event_grid_zoom(double zoom)
{
...
static void event_lcd_mode_change(int delta)
{
	const char *lcd_mode = NULL;

	handle->lcd_mode = (handle->lcd_mode + delta + N_LCD_MODES) % N_LCD_MODES;
...
static void event_lcd_filter_change(void)
{
...
static void event_index_change(int delta)
{
...
static void event_axis_change(int delta)
{
	FT_Error err;
	FT_Size size;
	FT_Var_Axis *a;
	...
	(void) FT_Set_Var_Design_Coordinates(size->face, status.used_num_axis, status.design_pos);
}
...
static void event_font_change(int delta)
{
	...
	for (n = 0; n < status.used_num_axis; n++)
	{
		...
		{
			status.design_pos[n] = n < status.requested_cnt ? status.requested_pos[n] : status.mm->axis[n].def;
		...
		{
			status.design_pos[n] = status.mm->namedstyle[instance_index].coords[n];
		} else
		{
		...
		if (is_GX)
		{
				...
				if (name.name_id == strid)
				{
					...
					if ((name.platform_id == TT_PLATFORM_MACINTOSH &&
						...
						(name.platform_id == TT_PLATFORM_MICROSOFT &&
						 (name.language_id & 0xFF) == TT_MS_LANGID_ENGLISH_GENERAL))
						break;
				}
			}
			...
			if (name.string)
			{
				...
				{
					len = put_unicode_be16_string_size(name.string, name.string_len, 0, 0);
					s = (char *) malloc(len);
					if (s)
					{
						put_unicode_be16_string(s, name.string, name.string_len, 0, 0);
						status.axis_name[n] = s;
					}
				}
			}
...
static int Process_Event(grEvent * event)
{
	...
	case grKEY('1'):
		if (handle->hinted &&
			(handle->autohint || handle->lcd_mode == LCD_MODE_LIGHT || handle->lcd_mode == LCD_MODE_LIGHT_SUBPIXEL))
		{
	...
	case grKEY('G'):
		...
		status.header = status.do_grid ? "grid drawing enabled" : "grid drawing disabled";
		break;
	...
	case grKEY('H'):
		if (!(handle->autohint || handle->lcd_mode == LCD_MODE_LIGHT || handle->lcd_mode == LCD_MODE_LIGHT_SUBPIXEL))
		{
			FT_Face face;
			FT_Module module;

			error = FTC_Manager_LookupFace(handle->cache_manager, handle->scaler.face_id, &face);
			if (!error)
			{
		...
		else
		{
			...
			status.header = status.do_horz_hints ? "horizontal hinting enabled" : "horizontal hinting disabled";
		}
#endif
		break;
	...
	case grKEY('V'):
		if (handle->autohint || handle->lcd_mode == LCD_MODE_LIGHT || handle->lcd_mode == LCD_MODE_LIGHT_SUBPIXEL)
		{
			...
			status.header = status.do_vert_hints ? "vertical hinting enabled" : "vertical hinting disabled";
		} else
	...
	case grKEY('B'):
		if (handle->autohint || handle->lcd_mode == LCD_MODE_LIGHT || handle->lcd_mode == LCD_MODE_LIGHT_SUBPIXEL)
		{
			...
			status.header = status.do_blue_hints ? "blue zone hinting enabled" : "blue zone hinting disabled";
		} else
	...
	case grKEY('s'):
		...
		status.header = status.do_segment ? "segment drawing enabled" : "segment drawing disabled";
		break;
#endif /* FT_DEBUG_AUTOFIT */
	...
	case grKeyRight:
		...
		break;
	case grKeyF7:
		event_index_change(-0x10);
		break;
	case grKeyF8:
		event_index_change(0x10);
		break;
	case grKeyF9:
		event_index_change(-0x100);
		break;
	case grKeyF10:
		event_index_change(0x100);
		break;
	case grKeyF11:
		event_index_change(-0x1000);
		break;
	case grKeyF12:
		event_index_change(0x1000);
		break;
	...
	case grKEY('i'):
		event_grid_translate(0, -1);
		break;
	case grKEY('k'):
		event_grid_translate(0, 1);
		break;
	case grKEY('j'):
		event_grid_translate(-1, 0);
		break;
	case grKEY('l'):
		event_grid_translate(1, 0);
		break;
	...
	case grKeyPageUp:
		event_grid_zoom(1.25);
		break;
	case grKeyPageDown:
		event_grid_zoom(1 / 1.25);
		break;
...
{
	FTDemo_Draw_Header(handle, display, status.ptsize, status.res, status.Num, error_code);

	if (status.header)
		grWriteCellString(display->bitmap, 0, 3 * HEADER_HEIGHT, status.header, display->fore_color);
...
int main(int argc, char *argv[])
{
	...
	FT_Property_Get(handle->library, "cff", "hinting-engine", &status.cff_hinting_engine);

	/* collect all available versions, then set again the default */
	FT_Property_Get(handle->library, "truetype", "interpreter-version", &dflt_tt_interpreter_version);
	for (n = 0; n < 3; n++)
	{
		error = FT_Property_Set(handle->library, "truetype", "interpreter-version", &versions[n]);
		if (!error)
			status.tt_interpreter_versions[status.num_tt_interpreter_versions++] = versions[n];
		if (versions[n] == dflt_tt_interpreter_version)
			status.tt_interpreter_version_idx = n;
	}
	FT_Property_Set(handle->library, "truetype", "interpreter-version", &dflt_tt_interpreter_version);

	FT_Property_Get(handle->library, "autofitter", "warping", &status.warping);

	FT_Library_SetLcdFilter(handle->library, status.lcd_filter);

	FT_Stroker_New(handle->library, &status.stroker);

	FT_Stroker_Set(status.stroker, 32, FT_STROKER_LINECAP_BUTT, FT_STROKER_LINEJOIN_BEVEL, 0x20000);
	...
	grSetTitle(display->surface, "FreeType Glyph Grid Viewer - press ? for help");

	for (; argc > 0; argc--, argv++)
	{
		error = FTDemo_Install_Font(handle, argv[0], 1, status.requested_cnt ? 1 : 0);
