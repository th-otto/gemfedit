...
static void do_ptrn(grBitmap * bitmap, int x, int y, int w, int h)
{
	...
	for (i = 0; i < h; i++)
	{
		...
		for (j = 0, k = 0; j < w; j++)
		{
			line[j] = (unsigned char) (0.5 + 255. * pow(p[k], 1. / (1. + 2. * j / w)));
			k++;
			if (k == 4)
				k = 0;
		}
...
static void event_gamma_grid(void)
{
	grEvent dummy_event;
	int g;
	int yside = 11;
...
int main(void)
{
	...
	do
	{
		...
		{
		case 0:
			grWriteCellString(display->bitmap, 236, 75, "Solid-Checkered Pattern", display->fore_color);
			Render_Bitmap(display->bitmap, &bit1, 20, 90, display->fore_color, 0);
			break;
		case 1:
			grWriteCellString(display->bitmap, 236, 75, "Grayscale Anti-Aliasing", display->fore_color);
			Render_Bitmap(display->bitmap, &bit2, 20, 96, display->fore_color, 0);
			break;
		case 2:
			grWriteCellString(display->bitmap, 236, 75, "Subpixel  Anti-Aliasing", display->fore_color);
		...
		for (i = 0; i <= 10; i++)
		{
			...
			grWriteCellString(display->bitmap, 9 + i * 60, 395, buf, display->fore_color);
		}

		grWriteCellString(display->bitmap, DIM_X / 2 - 20, 410, "Gamma", display->fore_color);
