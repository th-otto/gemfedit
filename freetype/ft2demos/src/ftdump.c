...
/* PanicZ */
static void PanicZ(FT_Library library, const char *message)
{
	const FT_String *str;
...
static void Print_Comma(const char *message)
{
	if (comma_flag)
		printf(", ");
...
static void usage(FT_Library library, char *execname)
{
	FT_Done_FreeType(library);
...
static const char *platform_id(int id)
{
	switch (id)
	{
...
static const char *name_id(int id)
{
	switch (id)
	{
...
static void Print_Sfnt_Names(FT_Face face)
{
	FT_SfntName name;
	FT_UInt num_names, i;
...
static void Print_Sfnt_Tables(FT_Face face)
{
	...
	for (i = 0; i < num_tables; i++)
	{
		...
		printf("  %2lu: %c%c%c%c %02X%02X%02X%02X...\n", i,
			   ...
			   (FT_Char) (tag >> 16),
			   (FT_Char) (tag >> 8),
			   (FT_Char) (tag), (FT_UInt) buffer[0], (FT_UInt) buffer[1], (FT_UInt) buffer[2], (FT_UInt) buffer[3]);
	}
}
...
static void Print_MM_Axes(FT_Face face)
{
	FT_MM_Var *mm;
		...
		if ((FT_Int16) len < 0)			/* composite */
		{
			...
			do
			{
				...
				loc += flags & FT_SUBGLYPH_FLAG_SCALE ? 2
					: flags & FT_SUBGLYPH_FLAG_XY_SCALE ? 4 : flags & FT_SUBGLYPH_FLAG_2X2 ? 8 : 0;
...
int main(int argc, char *argv[])
{
  ...
  Success:
	...
	printf("There %s %d %s in this file.\n",
		   ...
		   num_faces, num_faces == 1 ? (char *) "face" : (char *) "faces");
