...
static void gblitter_blitrgb24_gray_direct(GBlitter blitter, int color)
{
...
static void gblitter_blitrgb24_gray_cache(GBlitter blitter, int color)
{
	unsigned char r = (unsigned char) (color >> 16);
	unsigned char g = (unsigned char) (color >> 8);
	unsigned char b = (unsigned char) (color);
...
static int do_glyph(int arg)
{
	...
	if (gblitter_init_rgb24(&blit, (GBitmap) & glyph, dst_x, dst_y, SIZE_X, SIZE_Y, buffer, SIZE_X * 3))
		return 1;
...
static int do_glyph_color(int arg)
{
	GBlitterRec blit;
	int dst_x = RAND(SIZE_X);
	int dst_y = RAND(SIZE_Y);
...
void usage(void)
{
	...
	fprintf(stderr, "   -s seed  : specify random seed\n");
...
int main(int argc, char **argv)
{
	...
	while (argc > 1 && argv[1][0] == '-')
	{
		...
		case 't':
			argc--;
			argv++;
			if (argc < 1 || sscanf(argv[1], "%lf", &bench_time) != 1)
				usage();
			break;
	...
	if (TEST('a'))
		bench(do_glyph, 0, "direct white glyph", 0);

	chits = cmiss1 = cmiss2 = 0;
	memset(buffer, 0, sizeof(buffer));
	if (TEST('b'))
		bench(do_glyph, 1, "cache white glyph", 0);
	dump_cache_stats();

	memset(buffer, 0, sizeof(buffer));
	if (TEST('c'))
		bench(do_glyph_color, 0, "direct color glyph", 0);

	chits = cmiss1 = cmiss2 = 0;
	memset(buffer, 0, sizeof(buffer));
	if (TEST('d'))
		bench(do_glyph_color, 1, "cache color glyph", 0);
	dump_cache_stats();
