...
static FT_Error run_ot_validator(FT_Face face, const char *tables, int validation_level);
static FT_Error run_gx_validator(FT_Face face, const char *tables, int validation_level);
static FT_Error run_ckern_validator(FT_Face face, const char *dialect_request, int validation_level);
...
static ValidatorRec validators[] = {
	 ...
	 },
	{
	 GX_VALIDATE,
	 "gx",
	 ("FT_TrueTypeGX_Validate" " is disabled!  Recompile FreeType 2 with " "gxvalid" " module enabled.\n"),
	 ...
	 },
	{
	 CKERN_VALIDATE,
	 "ckern",
	 ("FT_ClassicKern_Validate" " is disabled!  Recompile FreeType 2 with " "gxvalid"	/* NOTE: classic kern validator is in gxvalid. */
	  " module enabled.\n"),
...
static void panic(int error, const char *message)
{
	fprintf(stderr, "%s\n  error = 0x%04x\n", message, error);
	exit(1);
}
...
static char *make_tag_chararray(char chararray[4], FT_UInt tag)
{
...
static void print_usage(FT_Library library_initializer)
{
	...
	{
		v = &validators[i];
		fprintf(stderr, " %s%s", v->symbol, v->is_implemented(library) ? "" : " (NOT COMPILED IN)");
	}
	fprintf(stderr, "\n" "\n");
	...
	for (i = 0; i < LAST_VALIDATE; i++)
	{
		...
		fprintf(stderr, "                Supported tables in %s validator:\n" "                 ", v->symbol);
...
static FT_Error try_load(FT_Face face, FT_ULong tag)
{
	FT_ULong length;
...
static FT_UInt parse_table_specs(const char *tables, const TableSpecRec spec[], int spec_count)
{
	...
	for (i = 0; i < len; i++)
	{
		...
		tag[i % 5] = tables[i];
		if ((i % 5) == 3)
			validation_flags |= find_validation_flag(FT_MAKE_TAG(tag[0], tag[1], tag[2], tag[3]), spec, spec_count);
	}
...
static FT_UInt list_face_tables(FT_Face face, const TableSpecRec spec[], int spec_count)
{
	FT_Error error;
...
static FT_UInt make_table_specs(FT_Face face, const char *request, const TableSpecRec spec[], int spec_count)
{
...
static void report_header(FT_UInt validation_flags, const TableSpecRec spec[], int spec_count)
{
	printf("[%s:%s] validation targets: ", execname, validators[validator].symbol);
	print_tables(stdout, validation_flags, spec, spec_count);
	printf("-------------------------------------------------------------------\n");
}


static void report_result(FT_Bytes data[], FT_UInt validation_flags, const TableSpecRec spec[], int spec_count)
{
	int i;
	int n_passes;
	int n_targets;
...
static FT_Error run_ot_validator(FT_Face face, const char *tables, int validation_level)
{
	...
	error = FT_OpenType_Validate(face, validation_flags, &data[0], &data[1], &data[2], &data[3], &data[4]);
...
static FT_Error run_gx_validator(FT_Face face, const char *tables, int validation_level)
{
	...
	error = FT_TrueTypeGX_Validate(face, validation_flags, data, N_GX_TABLE_SPEC);
...
static int list_gx_tables(FT_Face face)
{
	...
	validation_flags = list_face_tables(face, gx_table_spec, N_GX_TABLE_SPEC);
	return print_tables(stdout, validation_flags, gx_table_spec, N_GX_TABLE_SPEC);
}
...
static FT_Error run_ckern_validator(FT_Face face, const char *dialect_request, int validation_level)
{
	...
	if (strcmp(dialect_request, "ms:apple") == 0 || strcmp(dialect_request, "apple:ms") == 0)
...
int main(int argc, char **argv)
{
	...
	while (1)
	{
		...
		case 'V':
			...
			if (validation_level < 0 || validation_level > FT_VALIDATE_PARANOID)
			{
