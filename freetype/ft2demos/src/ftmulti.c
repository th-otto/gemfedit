...
static unsigned long make_tag(char *s)
{
	int i;
	unsigned long l = 0;
...
static void parse_design_coords(char *s)
{
	for (requested_cnt = 0; requested_cnt < MAX_MM_AXES && *s; requested_cnt++)
	{
...
static FT_Error LoadChar(unsigned int idx, int hint)
{
	int flags;
...
static FT_Error Render_All(unsigned int first_glyph, int pt_size)
{
...
#endif
	{
		if (!(error = LoadChar(i, hinted)))
		{
			...
			if (i <= first_glyph + 6)
			{
				LOG(("metrics[%02d] = [%x %x]\n", i, glyph->metrics.horiBearingX, glyph->metrics.horiAdvance));
...
static void Help(void)
{
	...
	sprintf(buf, "FreeType MM Glyph Viewer - part of the FreeType %s test suite", version);
...
static void cff_hinting_engine_change(int delta)
{
	...
	error = FT_Property_Set(library, "cff", "hinting-engine", &new_cff_hinting_engine);
	if (!error)
		cff_hinting_engine = (FT_UInt) new_cff_hinting_engine;
}


static void tt_interpreter_version_change(void)
{
	tt_interpreter_version_idx += 1;
	tt_interpreter_version_idx %= num_tt_interpreter_versions;

	FT_Property_Set(library, "truetype", "interpreter-version", &tt_interpreter_versions[tt_interpreter_version_idx]);
}
...
static int Process_Event(grEvent *event)
{
	...
	case grKEY('a'):
		antialias = !antialias;
		new_header = antialias ? (char *) "anti-aliasing is now on" : (char *) "anti-aliasing is now off";
		return 1;

	case grKEY('b'):
		use_sbits = !use_sbits;
		new_header = use_sbits
			? (char *) "embedded bitmaps are now used if available" : (char *) "embedded bitmaps are now ignored";
		return 1;
	...
	case grKEY('h'):
		hinted = !hinted;
		new_header = hinted ? (char *) "glyph hinting is now active" : (char *) "glyph hinting is now ignored";
		break;

	case grKEY(' '):
		render_mode ^= 1;
		new_header = render_mode ? (char *) "rendering all glyphs in font" : (char *) "rendering test text string";
		break;
...
int main(int argc, char *argv[])
{
	...
	for (n = 0; n < 3; n++)
	{
		error = FT_Property_Set(library, "truetype", "interpreter-version", &versions[n]);
		if (!error)
			tt_interpreter_versions[num_tt_interpreter_versions++] = versions[n];
  ...
  Display_Font:
	...
	for (;;)
	{
		...
		if (file_loaded >= 1)
		{
			...
			sprintf(Header, "PS name: %s", FT_Get_Postscript_Name(face));
			grWriteCellString(&bit, 0, 16, Header, fore_color);

			sprintf(Header, "axes:");
			{
				unsigned int limit = used_num_axis > MAX_MM_AXES / 2 ? MAX_MM_AXES / 2 : used_num_axis;
				...
				sprintf(Header, "at %d points, first glyph = %d, format = %s",
						...
						? (tt_ver == TT_INTERPRETER_VERSION_35
						   ...
						   : (tt_ver == TT_INTERPRETER_VERSION_38
							  ? "TrueType (v38)"
							  : "TrueType (v40)"))
						: (cff_hinting_engine == FT_CFF_HINTING_FREETYPE ? "CFF (FreeType)" : "CFF (Adobe)"));
			}
		} else
		{
			sprintf(Header, "%.100s: not an MM font file, or could not be opened", ft_basename(argv[file]));
		}
