...
static const FT_String *Cur_U_Line(TT_ExecContext exc)
{
	...
	} else if (op == 0x41)
	{
		...
		for (i = 0; i < n; i++)
		{
			sprintf(s, " $%02x%02x", (unsigned) CUR.code[CUR.IP + i * 2 + 2], (unsigned) CUR.code[CUR.IP + i * 2 + 3]);
			strncat(tempStr, s, 8);
		}
	...
	} else if ((op & 0xF8) == 0xB8)
	{
		...
		for (i = 0; i <= n; i++)
		{
			sprintf(s, " $%02x%02x", (unsigned) CUR.code[CUR.IP + i * 2 + 1], (unsigned) CUR.code[CUR.IP + i * 2 + 2]);
			strncat(tempStr, s, 8);
		}
	} else if (op == 0x39)				/* IP */
	{
...
static void handle_WS(TT_ExecContext exc, Storage * storage)
{
	...
	for (A = 0; A < curr->n_points; A++)
	{
		...
		if (diff)
		{
			...
			printf(temp,
				   ...
				   prev->tags[A] & FT_CURVE_TAG_TOUCH_X ? 'X' : ' ', prev->tags[A] & FT_CURVE_TAG_TOUCH_Y ? 'Y' : ' ');
			...
			printf(temp,
				   ...
				   curr->tags[A] & FT_CURVE_TAG_TOUCH_X ? 'X' : ' ', curr->tags[A] & FT_CURVE_TAG_TOUCH_Y ? 'Y' : ' ');
...
static void show_points_table(TT_GlyphZoneRec *zone, const FT_String *code_range, int n_points, FT_Bool is_twilight)
{
	...
	if (code_range[0] == 'g')
	{
		...
		printf(" idx " "orig. unscaled  " "   orig. scaled     " " current scaled     " "tags\n");
		printf("-----" "----------------" "--------------------" "--------------------" "----\n");
	} else
	...
	{
		printf("%3d%s ", A, is_twilight ? "T" : (A >= n_points - 4) ? "F" : " ");
		printf("(%5ld,%5ld) - ", zone->orus[A].x, zone->orus[A].y);
		if (use_float)
		{
			printf("(%7.2f,%7.2f) - ", zone->org[A].x / 64.0, zone->org[A].y / 64.0);
			printf("(%7.2f,%7.2f) - ", zone->cur[A].x / 64.0, zone->cur[A].y / 64.0);
		} else
		{
			printf("(%7ld,%7ld) - ", zone->org[A].x, zone->org[A].y);
			printf("(%7ld,%7ld) - ", zone->cur[A].x, zone->cur[A].y);
		}
		printf("%c%c%c\n",
			   zone->tags[A] & FT_CURVE_TAG_ON ? 'P' : 'C',
			   zone->tags[A] & FT_CURVE_TAG_TOUCH_X ? 'X' : ' ', zone->tags[A] & FT_CURVE_TAG_TOUCH_Y ? 'Y' : ' ');
	}
	printf("\n");
}
...
static FT_Error RunIns(TT_ExecContext exc)
{
	...
	save_pts.org = (FT_Vector *) malloc(2 * sizeof(FT_F26Dot6) * save_pts.n_points);
	save_pts.cur = (FT_Vector *) malloc(2 * sizeof(FT_F26Dot6) * save_pts.n_points);
	...
	save_twilight.org = (FT_Vector *) malloc(2 * sizeof(FT_F26Dot6) * save_twilight.n_points);
	save_twilight.cur = (FT_Vector *) malloc(2 * sizeof(FT_F26Dot6) * save_twilight.n_points);
	...
	do
	{
		...
		if (breakpoint.IP == CUR.IP && breakpoint.range == CUR.curRange)
		...
		do
		{
			...
			case 'V':
				...
				{
					/* 2.14 numbers */
					printf("freedom    (%.5f, %.5f)\n", CUR.GS.freeVector.x / 16384.0, CUR.GS.freeVector.y / 16384.0);
					printf("projection (%.5f, %.5f)\n", CUR.GS.projVector.x / 16384.0, CUR.GS.projVector.y / 16384.0);
					printf("dual       (%.5f, %.5f)\n", CUR.GS.dualVector.x / 16384.0, CUR.GS.dualVector.y / 16384.0);
					printf("\n");
				} else
				{
					printf("freedom    ($%04hx, $%04hx)\n", CUR.GS.freeVector.x, CUR.GS.freeVector.y);
					printf("projection ($%04hx, $%04hx)\n", CUR.GS.projVector.x, CUR.GS.projVector.y);
					printf("dual       ($%04hx, $%04hx)\n", CUR.GS.dualVector.x, CUR.GS.dualVector.y);
					printf("\n");
				}
				break;
					...
					else
					{
						...
						for (i = 0; i < CUR.storeSize; i++)
						{
							if (storage[i].initialized)
								printf("%3ldS  %8ld (%8.2f)\n", i, storage[i].value, storage[i].value / 64.0);
							else
				...
				else
				{
					...
					for (i = CUR.callTop; i > 0; i--)
					{
						...
						printf(" %3d      %4ld     f%04lx   f%04lx   %c%04lx\n",
							   ...
							   ? 'f' : (rec->Caller_Range == tt_coderange_cvt ? 'c' : 'g'), rec->Caller_IP - 1);
					}
					printf("\n");
				}
				break;
		...
		FT_MEM_COPY(save_pts.org, pts.org, pts.n_points * sizeof(FT_Vector));
		FT_MEM_COPY(save_pts.cur, pts.cur, pts.n_points * sizeof(FT_Vector));
		FT_MEM_COPY(save_pts.tags, pts.tags, pts.n_points);

		FT_MEM_COPY(save_twilight.org, twilight.org, twilight.n_points * sizeof(FT_Vector));
		FT_MEM_COPY(save_twilight.cur, twilight.cur, twilight.n_points * sizeof(FT_Vector));
		FT_MEM_COPY(save_twilight.tags, twilight.tags, twilight.n_points);

		FT_MEM_COPY(save_cvt, CUR.cvt, CUR.cvtSize * sizeof(FT_Long));

		FT_MEM_COPY(save_storage, storage, CUR.storeSize * sizeof(Storage));
		...
		case 'c':
			if (CUR.IP < CUR.codeSize)
			{
				...
				while (CUR.IP < CUR.codeSize)
				{
					...
					if (CUR.IP == breakpoint.IP && CUR.curRange == breakpoint.range)
						break;
				}
			}
			break;
		...
		case 'p':
			...
			{
				if (breakpoint.IP == last_IP && breakpoint.range == last_range)
				{
					breakpoint.IP = 0;
					breakpoint.range = 0;
				...
				} else
				{
					...
					printf("Breakpoint set to previous position (%c%04lx).\n",
						   ...
						   ? 'f' : (last_range == tt_coderange_cvt ? 'c' : 'g'), last_IP);
				}
			}
...
static void Usage(char *execname)
{
	...
	if (num_tt_interpreter_versions == 2)
		sprintf(versions, "%d and %d", tt_interpreter_versions[0], tt_interpreter_versions[1]);
	else
		sprintf(versions, "%d, %d, and %d",
				tt_interpreter_versions[0], tt_interpreter_versions[1], tt_interpreter_versions[2]);
...
int main(int argc, char **argv)
{
	...
	FT_Set_Debug_Hook(library, FT_DEBUG_HOOK_TRUETYPE, (FT_DebugHook_Func) RunIns);
	...
	while (!error)
	{
		...
		/* now load glyph */
		error = FT_Load_Glyph((FT_Face) face, (FT_UInt) glyph_index, FT_LOAD_NO_BITMAP);
		if (error && error != Quit && error != Restart)
