...
static const char *file_basename(const char *pathname)
{
...
static void check_face(FT_Face face, const char *filepathname, int idx)
{
	...
	printf("%15s : %20s : ", file_basename(filepathname), face->family_name ? face->family_name : "UNKNOWN FAMILY");
	...
	if (num_proportional > 0)
	{
		if (face_has_fixed_flag)
			printf("KO!  Tagged as fixed, but has %d `proportional' glyphs", num_proportional);
		else
