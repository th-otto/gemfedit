...
static void record_my_block(void *base, long size)
{
	if (size <= 0)
	{
...
static void forget_my_block(void *base)
{
...
FT_CALLBACK_DEF(void *) my_alloc(FT_Memory memory, long size)
{
	void *p = malloc(size);
...
FT_CALLBACK_DEF(void *) my_realloc(FT_Memory memory, long cur_size, long new_size, void *block)
{
	void *p;
