...
typedef void (*Display_drawFunc) (void *disp, DisplayMode mode, int x, int y, int width, int height, int pitch, void *buffer);
typedef void (*Display_textFunc) (void *disp, int x, int y, const char *msg);
...
static void render_state_init(RenderState state, Display display, FT_Library library)
{
	...
	FT_Property_Get(library, "cff", "hinting-engine", &cff_hinting_engine);

	/* collect all available versions, then set again the default */
	FT_Property_Get(library, "truetype", "interpreter-version", &dflt_tt_interpreter_version);
	for (i = 0; i < 3; i++)
	{
		error = FT_Property_Set(library, "truetype", "interpreter-version", &versions[i]);
		if (!error)
	...
	FT_Property_Set(library, "truetype", "interpreter-version", &dflt_tt_interpreter_version);

	FT_Property_Get(library, "autofitter", "warping", &warping);
	...
	state->columns[0].tt_interpreter_versions[0] = tt_interpreter_versions[0];
	state->columns[0].tt_interpreter_versions[1] = tt_interpreter_versions[1];
	state->columns[0].tt_interpreter_versions[2] = tt_interpreter_versions[2];
	state->columns[0].num_tt_interpreter_versions = num_tt_interpreter_versions;
	state->columns[0].tt_interpreter_version_idx = tt_interpreter_version_idx;
...
static void render_state_done(RenderState state)
{
...
static void render_state_set_resolution(RenderState state, unsigned int resolution)
{
	state->resolution = resolution;
}


static void render_state_set_size(RenderState state, double char_size)
{
	state->char_size = char_size;
}


static void render_state_set_face_index(RenderState state, int idx)
{
	state->face_index = idx;
}


static void _render_state_rescale(RenderState state)
{
	if (state->size)
		FT_Set_Char_Size(state->face, 0, (FT_F26Dot6) (state->char_size * 64.0), 0, state->resolution);
}


static void render_state_set_files(RenderState state, char **files, char *execname)
{
	FontFace faces = NULL;
	...
	for (; files[0] != NULL; files++)
	{
		...
		for (count = 0; count < num_subfonts; count++)
		{
			...
			if (error)
			{
				fprintf(stderr, "ftdiff: Opening `%s' failed with code 0x%X, skipping\n", files[0], error);
				break;
			}

			if (!FT_IS_SCALABLE(face))
			{
				fprintf(stderr, "ftdiff: font `%s' is not scalable, skipping\n", files[0]);
				goto Done;
			}
			...
			if (num_faces >= max_faces)
			{
				...
				faces = (FontFace) realloc(faces, max_faces * sizeof(faces[0]));
				if (faces == NULL)
...
static int render_state_set_file(RenderState state)
{
	const char *filepath;
	...
	{
		error = FT_New_Face(state->library, filepath, state->faces[state->face_index].index, &state->face);
		if (error)
			return -1;
...
/** RENDERING **/
static void render_state_draw(RenderState state, const char *text, int idx, int x, int y, int width, int height)
{
	...
	if (column->use_custom_lcd_filter)
		FT_Library_SetLcdFilterWeights(face->glyph->library, column->filter_weights);
	...
	if (rmode == HINT_MODE_AUTOHINT_LIGHT || rmode == HINT_MODE_AUTOHINT_LIGHT_SUBPIXEL)
	...
	while (1)
	{
		...
		if (rmode != HINT_MODE_AUTOHINT_LIGHT_SUBPIXEL && column->use_deltas)
		{
		...
		if ((rmode == HINT_MODE_UNHINTED ||
			 rmode == HINT_MODE_AUTOHINT_LIGHT_SUBPIXEL) && slot->format == FT_GLYPH_FORMAT_OUTLINE)
		{
		...
		if (column->use_cboxes)
		{
			if (slot->format == FT_GLYPH_FORMAT_OUTLINE)
			{
				...
				xmax = (x_origin >> 6) + slot->bitmap_left + (FT_Long) slot->bitmap.width;
		} else
		{
			if (rmode == HINT_MODE_UNHINTED || rmode == HINT_MODE_AUTOHINT_LIGHT_SUBPIXEL)
...

		{
			...
			state->display.disp_draw(state->display.disp, mode,
									 ...
									 y - slot->bitmap_top, (int) map->width, (int) map->rows, map->pitch, map->buffer);
		}
		if (rmode == HINT_MODE_UNHINTED || rmode == HINT_MODE_AUTOHINT_LIGHT_SUBPIXEL)
	...
	/* display footer on this column */
	{
		...
		if (rmode == HINT_MODE_BYTECODE)
		{
			...
			{
				switch (column->tt_interpreter_versions[column->tt_interpreter_version_idx])
				{
		...
		sprintf(temp, "%s%s", render_mode_names[column->hint_mode], extra);
		state->display.disp_text(disp, left, bottom + 5, temp);

		if (column->use_lcd_filter)
			msg = "LCD rendering";
		else
			msg = "gray rendering";
		state->display.disp_text(disp, left, bottom + HEADER_HEIGHT + 5, msg);
		...
		if (column->use_lcd_filter)
		{
			if (column->use_custom_lcd_filter)
			{
				...
				sprintf(temp,
						...
						fw[4],
						fwi == 4 ? "]" : " ");
				state->display.disp_text(disp, left, bottom + 2 * HEADER_HEIGHT + 5, temp);
			} else
			{
				...
				state->display.disp_text(disp, left, bottom + 2 * HEADER_HEIGHT + 5, msg);
			}
		} else
		{
			msg = "";
		...
		sprintf(temp, "%s %s %s",
				...
				: "-kern",
				column->use_deltas ? "+delta" : "-delta",
				column->use_cboxes ? "glyph boxes" : "adv. widths");
		state->display.disp_text(disp, left, bottom + 3 * HEADER_HEIGHT + 5, temp);

		if (state->col == idx)
			state->display.disp_text(disp, left, bottom + 4 * HEADER_HEIGHT + 5, "************************");
	}
}
...
static int adisplay_init(ADisplay display, grPixelMode mode, int width, int height)
{
	grSurface *surface;
	grBitmap bit;
...
static void adisplay_clear(ADisplay display)
{
...
static void adisplay_done(ADisplay display)
{
...
static void adisplay_draw_glyph(void *_display, DisplayMode mode, int x, int y, int width, int height, int pitch, void *buffer)
{
	...
	grBlitGlyphToBitmap(display->bitmap, &glyph, x, y, display->fore_color);
}


static void adisplay_draw_text(void *_display, int x, int y, const char *msg)
{
...
static void adisplay_change_gamma(ADisplay display, double delta)
{
	/* use epsilons */
	display->gamma += delta;
...
static void event_help(RenderState state)
{
	char buf[256];
	char version[64];
	...
	sprintf(buf, "FreeType Hinting Mode Comparator - part of the FreeType %s test suite", version);
...
static void event_change_gamma(RenderState state, double delta)
{
	ADisplay display = (ADisplay) state->display.disp;
...
static void event_change_face_index(RenderState state, int idx)
{
	if (idx < 0)
		idx = 0;
...
static int process_event(RenderState state, grEvent * event)
{
	...
	case grKEY('h'):
		column->hint_mode = (HintMode) ((column->hint_mode + 1) % HINT_MODE_MAX);
		break;
	...
	case grKEY('H'):
		{
			...
			if (column->hint_mode == HINT_MODE_BYTECODE)
			{
				if (!strcmp(module->clazz->module_name, "cff"))
				{
					...
					error = FT_Property_Set(state->library, "cff", "hinting-engine", &new_cff_hinting_engine);
					if (!error)
						column->cff_hinting_engine = new_cff_hinting_engine;
				} else if (!strcmp(module->clazz->module_name, "truetype"))
				{
					column->tt_interpreter_version_idx += 1;
					column->tt_interpreter_version_idx %= column->num_tt_interpreter_versions;
...
static void write_global_info(RenderState state)
{
	...
	sprintf(buf, "%.1fpt (%dppem) at %ddpi, gamma: %s",
			state->char_size, (int) (state->char_size * state->resolution / 72 + 0.5), state->resolution, gamma);
	grWriteCellString(adisplay->bitmap, 0, 5 + HEADER_HEIGHT, buf, adisplay->fore_color);
...
int main(int argc, char **argv)
{
	...
	/* Initialize display */
	if (adisplay_init(adisplay, gr_pixel_mode_rgb24, width, height) < 0)
	{
	...
	for (;;)
	{
		...
		render_state_draw(state, text, 0, column_x_start[0], column_y_start, column_width, column_height);
		render_state_draw(state, text, 1, column_x_start[1], column_y_start, column_width, column_height);
		render_state_draw(state, text, 2, column_x_start[2], column_y_start, column_width, column_height);
