...
static Translator syskey_translators[] = {
	{VK_F10, grKeyF10}
};
...
static void gr_win32_surface_done(grWin32Surface * surface)
{
...
static void gr_win32_surface_refresh_rectangle(grWin32Surface * surface, int x, int y, int w, int h)
{
	HDC hDC;
	int row_bytes, delta;
...
#ifdef SWIZZLE
	{
		...
		gr_swizzle_rect_rgb24(bitmap->buffer, bitmap->pitch,
							  ...
							  bitmap->width, bitmap->rows, 0, 0, bitmap->width, bitmap->rows);
	...
	hDC = GetDC(window);
	SetDIBits(hDC, surface->hbm, 0, bitmap->rows, surface->bgrBitmap.buffer, pbmi, DIB_RGB_COLORS);
...
static void gr_win32_surface_set_title(grWin32Surface * surface, const char *title)
{
...
static void gr_win32_surface_listen_event(grWin32Surface * surface, int event_mask, grEvent * grevent)
{
	MSG msg;
	HANDLE window = surface->window;
...
static int gr_win32_init_surface(grSurface *surface_, grBitmap * bitmap)
{
	...
	if (grNewBitmap(bitmap->mode, bitmap->grays, bitmap->width, bitmap->rows, &surface->bgrBitmap))
		return 0;
	...
	{
		if (grNewBitmap(bitmap->mode, bitmap->grays, bitmap->width, bitmap->rows, &surface->swizzle_bitmap))
			return 0;
	...
	case gr_pixel_mode_gray:
		...
		pbmi->bmiHeader.biClrUsed = bitmap->grays;
		{
			...
			{
				color->rgbRed = color->rgbGreen = color->rgbBlue = (unsigned char) (((count - x) * 255) / count);
				color->rgbReserved = 0;
			}
		}
		break;
...
LRESULT CALLBACK Message_Process(HWND handle, UINT mess, WPARAM wParam, LPARAM lParam)
{
	...
	case WM_PAINT:
		{
			...
			BitBlt(hDC, 0, 0, surface->window_width, surface->window_height, memDC, 0, 0, SRCCOPY);
