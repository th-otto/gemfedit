...
static void gblender_set_gamma_table(double gamma_value, unsigned short *gamma_ramp, unsigned char *gamma_ramp_inv)
{
	...
	} else
	{
		...
		/* voltage to linear */
		for (ii = 0; ii < 256; ii++)
			gamma_ramp[ii] = (unsigned short) (gmax * pow((double) ii / 255., gamma_value) + 0.5);

		/* linear to voltage */
		for (ii = 0; ii <= gmax; ii++)
			gamma_ramp_inv[ii] = (unsigned char) (255. * pow((double) ii / gmax, gamma_inv) + 0.5);
	}
}
...
/* clear the cache
 */
static void gblender_clear(GBlender blender)
{
	int nn;
...
void gblender_reset(GBlender blender)
{
	...
	if (blender->channels)
	{
		...
		blender->cache_r_cells = gblender_lookup_channel(blender, blender->cache_r_back, blender->cache_r_fore);
		...
		blender->cache_b_cells = gblender_lookup_channel(blender, blender->cache_b_back, blender->cache_b_fore);
	} else
	{
		blender->cache_back = 0;
		blender->cache_fore = 0xFFFFFF;
		blender->cache_cells = gblender_lookup(blender, blender->cache_back, blender->cache_fore);
	}
...
void gblender_init(GBlender blender, double gamma_value)
{
	blender->channels = 0;

	gblender_set_gamma_table(gamma_value, blender->gamma_ramp, blender->gamma_ramp_inv);
 ...
 */
static void gblender_reset_key(GBlender blender, GBlenderKey key)
{
...
GBlenderCell *gblender_lookup(GBlender blender, GBlenderPixel background, GBlenderPixel foreground)
{
	...
	do
	{
		...
		if (key->background == background && key->foreground == foreground)
			goto Exit;
  ...
  NewNode:
	...
	key->cells = blender->cells + idx * (GBLENDER_SHADE_COUNT * GBLENDER_CELL_SIZE);
...
static void gblender_reset_channel_key(GBlender blender, GBlenderChanKey key)
{
