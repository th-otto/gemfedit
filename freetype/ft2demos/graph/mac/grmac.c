...
static void refresh_rectangle(grSurface * surface, int x, int y, int w, int h)
{
	...
	if (theWindow)
	{
		...
		SetPort(windowPort);
		CopyBits((BitMap *) & thePixMap, GetPortBitMapForCopyBits(windowPort), &bounds, &bounds, srcCopy, visRgn);
		DisposeRgn(visRgn);
	}
}
...
static void listen_event(grSurface * surface, int event_mask, grEvent * grevent)
{
	grEvent our_grevent;
		...
		if (WaitNextEvent(everyEvent, &mac_event, 10, NULL))
		{
			...
			case updateEvt:
				if (theWindow && (WindowPtr) mac_event.message == theWindow)
				{
					...
					refresh_rectangle(surface, 0, 0, thePixMap.bounds.right, thePixMap.bounds.bottom);
					EndUpdate(theWindow);
				}
#ifdef __MWERKS__
				else
