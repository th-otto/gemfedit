...
static void enable_os2_iostreams(void)
{
	PTIB thread_block;
	PPIB process_block;
...
static void done_surface(grPMSurface * surface)
{
...
static const int pixel_mode_bit_count[] = {
	0,
	1,									/* mono  */
	4,									/* pal4  */
	8,									/* pal8  */
...
static void RunPMWindow(grPMSurface * surface);




static void refresh_rectangle(grPMSurface * surface, int x, int y, int w, int h)
{
	...
	GpiSetBitmapBits(surface->image_ps,
					 0, surface->root.bitmap.rows, surface->root.bitmap.buffer, surface->bitmap_header);
...
static void set_title(grPMSurface * surface, const char *title)
{
	ULONG rc;
...
static void listen_event(grPMSurface * surface, int event_mask, grEvent * grevent)
{
...
static int init_surface(grSurface *surface, grBitmap *bitmap)
{
	...
	surface->image_dc = DevOpenDC(gr_anchor, OD_MEMORY, (PSZ) "*", 0L, 0L, 0L);

	surface->image_ps = GpiCreatePS(gr_anchor,
									surface->image_dc, &sizl, PU_PELS | GPIT_MICRO | GPIA_ASSOC | GPIF_DEFAULT);
	...
	if (bitmap->mode == gr_pixel_mode_gray)
	{
		...
		{
			color->bBlue = color->bGreen = color->bRed = (((count - x) * 255) / count);
		}
	} else
	{
		RGB2 *color = bit->argbColor;

		color[0].bBlue = color[0].bGreen = color[0].bRed = 0;

		color[1].bBlue = color[1].bGreen = color[1].bRed = 255;
	}

	surface->os2_bitmap = GpiCreateBitmap(surface->image_ps, (PBITMAPINFOHEADER2) bit, 0L, NULL, NULL);

	GpiSetBitmap(surface->image_ps, surface->os2_bitmap);

	bit->cbFix = sizeof(BITMAPINFOHEADER2);
	GpiQueryBitmapInfoHeader(surface->os2_bitmap, (PBITMAPINFOHEADER2) bit);
	surface->bitmap_header = bit;
	...
	if (bitmap->mode == gr_pixel_mode_gray)
	{
		...
		GpiCreateLogColorTable(surface->image_ps,
							   ...
							   (LONG) LCOLF_CONSECRGB, (LONG) 0L, (LONG) count, (PLONG) palette);

		/* now, copy the color indexes to surface->shades */
		for (x = 0; x < count; x++)
			surface->shades[x] = GpiQueryColorIndex(surface->image_ps, 0, palette[x]);
	}
	...
	DosCreateThread(&surface->message_thread, (PFNTHREAD) RunPMWindow, (ULONG) surface, 0UL, 32920);
...
MRESULT EXPENTRY Message_Process(HWND handle, ULONG mess, MPARAM parm1, MPARAM parm2);
...
static void RunPMWindow(grPMSurface * surface)
{
	...
	if (!WinRegisterClass(surface->anchor, (PSZ) class_name, (PFNWP) Message_Process, CS_SIZEREDRAW, 0))
	{
	...
	class_flags = FCF_TITLEBAR | FCF_MINBUTTON | FCF_DLGBORDER | FCF_TASKLIST | FCF_SYSMENU;
	...
	surface->frame_window = WinCreateStdWindow(HWND_DESKTOP,
											   ...
											   WS_VISIBLE, 0, 0, &surface->client_window);
	if (!surface->frame_window)
	{
	...
	surface->title_window = WinWindowFromID(surface->frame_window, FID_TITLEBAR);
	...
	WinSetWindowPos(surface->frame_window,
					...
					WinQuerySysValue(HWND_DESKTOP, SV_CYDLGFRAME) * 2 + surface->root.bitmap.rows, SWP_SIZE | SWP_MOVE);
...
MRESULT EXPENTRY Message_Process(HWND handle, ULONG mess, MPARAM parm1, MPARAM parm2)
{
	...
	case WM_CREATE:
		...
		screen_ps = GpiCreatePS(surface->anchor, screen_dc, &sizl, PU_PELS | GPIT_MICRO | GPIA_ASSOC | GPIF_DEFAULT);
	...
	case WM_PAINT:
		...
		GpiBitBlt(screen_ps, surface->image_ps, 4L, surface->blit_points, ROP_SRCCOPY, BBO_AND);
