...
static void gr_fill_hline_mono(unsigned char *line, int x, int width, grColor color)
{
	int c1 = (x >> 3);
	int lmask = 0xFF >> (x & 7);
	int c2 = ((x + width - 1) >> 3);
...
static void gr_fill_hline_4(unsigned char *line, int x, int width, grColor color)
{
...
static void gr_fill_hline_8(unsigned char *line, int x, int width, grColor color)
{
	memset(line + x, color.value, (unsigned int) width);
}

static void gr_fill_hline_16(unsigned char *_line, int x, int width, grColor color)
{
...
static void gr_fill_hline_24(unsigned char *line, int x, int width, grColor color)
{
...
static void gr_fill_hline_32(unsigned char *line, int x, int width, grColor color)
{
	line += 4 * x;
...
typedef void (*grFillHLineFunc) (unsigned char *line, int x, int width, grColor color);
...
void grFillHLine(grBitmap * target, int x, int y, int width, grColor color)
{
	int delta;
	unsigned char *line;
...
void grFillVLine(grBitmap * target, int x, int y, int height, grColor color)
{
	int delta;
	unsigned char *line;
...
void grFillRect(grBitmap * target, int x, int y, int width, int height, grColor color)
{
	int delta;
	unsigned char *line;
	grFillHLineFunc hline_func;
