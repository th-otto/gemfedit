...
static OSErr get_folder_parent(FSSpec * fss, FSSpec * parent)
{
	CInfoPBRec rec;
	short err;
...
static OSErr FTMac_GetFullPath(FSSpec *fss, char *buf)
{
	short err;
...
static OSErr init_app_name(void)
{
...
static pascal OSErr handle_not(const AppleEvent *theAppleEvent, AppleEvent *reply, long refCon)
{
...
static pascal OSErr handle_open_app(const AppleEvent *theAppleEvent, AppleEvent *reply, long refCon)
{
#pragma unused (reply, refCon)
#if 0
...
static pascal OSErr handle_open_doc(const AppleEvent *theAppleEvent, AppleEvent *reply, long refCon)
{
#pragma unused (reply, refCon)
	OSErr err;
	AEDescList doclist;
